...
function check_user() {	
	...
	var  apipath_base_photo_dm='http://w05.yeapps.com/novivo/syncmobile_417_web_firm_01/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'

	//var  apipath_base_photo_dm='http://127.0.0.1:8000/novivo/syncmobile_417_web_firm_01/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'
...
//							--------------------------
							...
							//alert (localStorage.base_url+'check_user_pharma?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&version=13')
...
//													}
													...
													for (var l=0; l < linkStrList.length; l++){
														...
														//alert (linkPath);
														if(path_value!=''){
																//Nazma Azam 2020-03-05 start design
														...
														//	linkStr_combo+='<li style="border-bottom-style:solid; border-color:#CBE4E4;border-bottom-width:thin"><table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr><td><a href="#" onclick="'+linkPath+'">'+pathName+'</a></td></tr></table></li>';
															...
															linkStr_combo+='<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr align="center" style="width:100%"><td  align="center"><input type="submit" id="btn_link_path"  align="middle" onClick="'+linkPath+'"  style="width:100%; height:40px; background-color:#26917C; color:#FFF; font-size:18px;border-radius:15px;" value="'+pathName+'"/></td></tr></table>';

											//Nazma Azam 2020-03-05 end design
	...
	$.ajax(localStorage.base_url+'lastThreeVisit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_pending='+repId,{
								...
								success:function(data, status,xhr){	
									 if (status!='success'){
										...
										//alert(resultArray[0])
										if (resultArray[0]=='FAILED'){
											$("#err_repInfo").html(resultArray[1]);	
											//alert(resultArray[1])
											$("#wait_image_repInfo").hide();		
...
//alert (localStorage.doctor_flag)
	if (localStorage.doctor_flag==1){
		...
		//Nazma Azam 2020-03-16 start
		//alert('1');
		//Nazma Azam 2020-03-16 end
...
function lscVisitSubmit(){	
	...
	//	Nazma 2020-03-05 start
	var day_1 = currentDate_1.getDate();if(parseInt(day_1)<10)	{day_1="0" +day_1};
	//var day_2 = currentDate_1.getDate()+1;if(parseInt(day_1)<10)	{day_1="0" +day_1};
	//	Nazma 2020-03-05 end
	...
	//Nazma Azam 2020-03-05 start tomorrow 
	
 var tomorrow = new Date();
 tomorrow.setDate(tomorrow.getDate() + 1);
 var str_tomorrow = (tomorrow.toLocaleString().substring(0,tomorrow.toLocaleString().indexOf(':')-3)).replace(',','');
	//alert(today_1)
	//alert(productOrderStr)
	/*if ( ((delivery_date=='') || (delivery_date<today_1))&& (productOrderStr.length < 10))
	
	{
		//alert('a');
		delivery_date=str_tomorrow}*/


	if ((delivery_date=='') || (delivery_date<today_1))
	...
	//Nazma Azam 2020-03-05 end tomorrow 
	//alert(delivery_date)
	//else{
...
function visitSave(){
	...
	tomorrow.setDate(tomorrow.getDate() + 1);
	var str_tomorrow = (tomorrow.toLocaleString().substring(0,tomorrow.toLocaleString().indexOf(':')-3)).replace(',','');
	if (delivery_date_save=='')
	
	{
		//alert('a');
		delivery_date_save=str_tomorrow}
	//Nazma Azam 2020-03-05 end tomorrow 
...
function getvisitSave_data(){
	...
	for (var i=0; i < saved_dataArray.length -1 ; i++){
		...
		localStorage.visit_market_show+'<fdfd>'+localStorage.visit_client_show
			//Nazma Azam 2020-03-11 start design
	   saved_data_list=saved_data_list+'<li  style="border-bottom-style:solid; border-color:#CBE4E4;border-bottom-width:thin"><table width="100%" border="0"><tr><td width="2%"></td><td width="80%" onClick="set_save_data('+i+')"  >'+'<font style="font-size:16px ;color:#306161"><b>'+visit_client_show+'</b></font>( '+visit_market_show+' )'+'</br><font style="color:#804040"> '+visit_time_show+'</font></td><td>' +'<input type="submit" style="width:100%; height:30px; width:40px; background-color:#09C; color:#FFF; font-size:15px;border-radius:5px;" onClick="cancelSave('+i+')" value="X"  >' +'</font>' +'</td> </tr></table></li>'
	  // alert (client_id);
		//Nazma Azam 2020-03-11 end design 												
	}
...
function page_outstanding() {
	...
	//alert (localStorage.visit_client);
	//	Nazma 2020-03-05 start
	//alert (localStorage.report_url+'client_outstanding_report?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&client='+localStorage.visit_client);
	//	Nazma 2020-03-05 end
...
function page_invoice() {
	...
	//alert(localStorage.report_url+'client_invoice_report?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&client='+localStorage.visit_client)
						//Nazma Azam 2020-03-05 end
...
function page_clientOrder() {
	...
	$("#error_clientOrder").html('');
		//	Nazma 2020-03-05 start
	//alert (localStorage.report_url+'client_order_report?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&client='+localStorage.visit_client);
	//	Nazma 2020-03-05 end
...
function poultryImage() {
	//navigator.camera.getPicture(onSuccessProfile, onFailProfile, { quality: 10,
		//destinationType: Camera.DestinationType.FILE_URI });
   navigator.camera.getPicture(onSuccess_poultryImage, onFail_poultryImage, { 
   		quality: 90,
		targetWidth: 400,
		destinationType: Camera.DestinationType.FILE_URI,correctOrientation: true ,
		saveToPhotoAlbum: true});
...
function poultry_submit() {
	...
	//Nazma Azam 2020-03-07 start
	var poultryPhoto=$("#poultryPhoto").val();
	var now = $.now();
	var imageName=localStorage.user_id+'_'+now+'_poultryVisit.jpg';
	//Nazma Azam 2020-03-07 end
	getLocationInfo_ready()
...
//	$("#doctor_prof").val(localStorage.report_url+'doc_info_submit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&route='+market_Id+'&docId='+visitDocId+'&dName='+dName+'&dSpaciality='+dSpaciality+'&dDegree='+dDegree+'&dDOB='+dDOB+'&dMDay='+dMDay+'&dMobile='+dMobile+'&dCAddress='+dCAddress+'&dCategory='+dCategory+'&dDist='+dDist+'&dThana='+dThana)
		...
		//Nazma Azam 2020-03-07 end
								...
								if (resultArray[0]=='FAILED'){
									...
									//Nazma Azam 2020-03-07 start
									uploadPhoto(poultryPhoto, imageName);	
									//Nazma Azam 2020-03-07 end	
									var result_string=resultArray[1];
								// ===============2019-02-01 novivo2019 start ================
									$("#addPName").val('');
...
// ===============2019-02-01 novivo2019 end ================
  ...
  var chPhoto=$("#"+imageText).val();
	//Nazma Azam 2020-03-07 start
  var cattlePhoto=$("#cattlePhoto").val();

  var now = $.now();
  var imageName=localStorage.user_id+'_'+now.toString()+'_cattleVisit.jpg'; 
    	//Nazma Azam 2020-03-07 end
...
// Nazma Azam 2020-03-07 start
 ...
 //var imageName=localStorage.user_id+'C_'+now+'.jpg';
  ...
  $.ajax(localStorage.base_url+'cattleSubmit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&latitude='+latitude+'&longitude='+longitude+'&farm_name='+encodeURI(farm_name)+'&tr_cattle='+tr_cattle+'&owner_name='+encodeURI(owner_name)+'&address='+address+'&mobileC='+mobileC+'&add_dob='+add_dob+'&anniversary='+anniversary+'&chemist_medicine='+chemist_medicine+'&farm_manager='+farm_manager+'&farm_consultant='+farm_consultant+'&addCCategory='+addCCategory+'&birds_animal='+birds_animal+'&rearing='+rearing+'&feeding='+feeding+'&watering='+watering+'&brooding='+brooding+'&ponds_bigha='+ponds_bigha+'&cattle_farm_id_text='+cattle_farm_id_text+'&imageName='+imageName,
 // Nazma Azam 2020-03-07 end
  {
	  ...
	  //Nazma Azam 2019-02-01 end
								...
								if (resultArray[0]=='FAILED'){
									...
									//Nazma Azam 2020-03-07 start
									uploadPhoto(cattlePhoto, imageName);
									//Nazma Azam 2020-03-07 end		
									var result_string=resultArray[1];
									// ===============2019-02-01 novivo2019 start================
		...
		$.ajax(localStorage.base_url+'aqua_info_submit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&addCNameA='+addCNameA+'&addAOName='+addAOName+'&tr_aqua='+tr_aqua+'&addressAq='+addressAq+'&mobileA='+mobileA+'&addCDOBa='+addCDOBa+'&anniversaryA='+anniversaryA+'&chemist_medicine_a='+chemist_medicine_a+'&managerA='+managerA+'&consultantA='+consultantA+'&addCCategoryA='+addCCategoryA+'&birds_animal_a='+birds_animal_a+'&rearingA='+rearingA+'&feedingA='+feedingA+'&wateringA='+wateringA+'&broodingA='+broodingA+'&addCPhoneA='+addCPhoneA+'&latitude='+latitude+'&longitude='+longitude+'&aqua_farm_id_text='+aqua_farm_id_text+'&imageName='+imageName,{
			...
			//Nazma Azam 2020-03-07 end
								...
								if (resultArray[0]=='FAILED'){
									...
									//Nazma Azam 2020-03-07 start	
								uploadPhoto(aquaPhoto, imageName)
																	//Nazma Azam 2020-03-07 end	
