...
#         strValueMorningE=strValueMorningE+'<'+sDate+'>'+route_name
        ...
        if ((status=='Done') or (status=='Submitted')):
            ...
            docStrE='Doctor'
            clStrE='Client'
            rowDoctor = db((db.sm_doctor_day_plan.cid==cid)  &(db.sm_doctor_day_plan.rep_id==session.rep_id) & (db.sm_doctor_day_plan.area_id==route_id) & (db.sm_doctor_day_plan.plan_date==sDate)  &(db.sm_doctor_day_plan.visit_time=='Evening') ).select(db.sm_doctor_day_plan.doc_name,db.sm_doctor_day_plan.doc_id, orderby=db.sm_doctor_day_plan.doc_name)
#             return db._lastsql
            for rowDoctor in rowDoctor:
                docInfo = str(rowDoctor.doc_name)+'-'+str(rowDoctor.doc_id) 
...
#         return rowDoctor
        ...
        for rowClient in rowClient:
            clInfo = str(rowClient.client_name)+'-'+str(rowClient.client_id) 
            clStr=clStr+clInfo+','
        routeShow=routeName+'|'+route_id+' <d>'+docStr+' '+clStr+'</span>'
