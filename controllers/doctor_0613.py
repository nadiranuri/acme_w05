...
def doctor_add():
    ...
    form = SQLFORM(db.sm_doctor,
                  fields=['doc_id', 'doc_name', 'specialty', 'mobile', 'des', 'status', 'attached_institution', 'designation', 'dob', 'doctors_category', 'field1'],
                  submit_button='Save'
                  )
...
def doctor_edit():
    ...
    form = SQLFORM(db.sm_doctor,
                  record=record,
                  deletable=True,
                  fields=['doc_name', 'specialty', 'mobile', 'des', 'status', 'attached_institution', 'designation', 'dob', 'doctors_category', 'field1'],
    ...
    if btn_upload == 'Upload':
        ...
        for i in range(total_row):
            ...
            if len(coloum_list) == 10:
                ...
                chamberAddress = str(coloum_list[4]).strip()
                attachInstitute = str(coloum_list[5]).strip()
                designation = str(coloum_list[6]).strip()
                dob = str(coloum_list[7]).strip()                
                ...
                if not(doc_id == '' or doc_name == ''):
                    ...
                    if validMobile == True:
                        ...
                        if dobFlag==True:
                            try:
                                ...
                                if(duplicate_doc == False):
                                    if doc_id not in excelList:
                                        ...
                                        # Create insert list
                                        ins_dict = {'cid':c_id, 'doc_id':doc_id, 'doc_name':doc_name, 'specialty':specialty,'field1':degree,'des':chamberAddress, 'attached_institution':attachInstitute,'designation':designation,'dob':dob,'password':randNumber, 'mobile':mobile, 'doctors_category':doctorsCategory, 'status':status}
                            ...
                            error_data = row_data + '(Invalid DOB)\n'
                            error_str = error_str + error_data
                            count_error += 1
                            continue
                    else:
