...
#     return 'test'
    ...
    response.title = 'Farm Visit Detail'
    ...
    if (session.btn_filter_farm):
        ...
        if (session.search_type== 'FarmName'):
            ...
            search_value_id=str(session.search_value).split('|')[1]
            qset = qset(db.sm_farm_visit.farm_id == search_value_id)

        elif (session.search_type == 'RepName'):
            search_value=str(session.search_value).split('|')[0]
            qset = qset(db.sm_farm_visit.rep_name == search_value)
            search_value_id=str(session.search_value).split('|')[1]
            qset = qset(db.sm_farm_visit.rep_id == search_value_id)
                
    records = qset.select(db.sm_farm_visit.ALL, groupby=db.sm_farm_visit.id, orderby=~db.sm_farm_visit.farm_id, limitby=limitby)
    totalCount=qset.count()
#     records_update="update sm_farm  set  anniversary='1900-01-01' WHERE cid=cid and anniversary ='0000-00-00';"
#     records_update_run=db.executesql(records_update)
#     
...
def download_farm_visit():
    ...
    records = qset.select(db.sm_farm_visit.rep_id,db.sm_farm_visit.rep_name,db.sm_farm_visit.farm_id,db.sm_farm_visit.farm_name,db.sm_farm_visit.farm_type,db.sm_farm_visit.route,db.sm_farm_visit.meet_with,db.sm_farm_visit.visit_type,db.sm_farm_visit.doc_support, groupby=db.sm_farm_visit.id, orderby=~db.sm_farm_visit.farm_id)
    myString='Farm Visit Detail\n\n'
    myString+='RepID, Name, Farm ID, Farm Name, Farm Type, Route, Visit Type, Meeting with, Doc Support' \
              '\n'
    for rec in records:
        ...
        myString+=str(rep_id)+','+str(rep_name)+','+str(farm_id)+','+str(farm_name)+','+str(farm_type)+','+str(route)+','+str(visit_type)+','+str(meet_with)+','+str(doc_support)+'\n'

    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=download_farmVisit_detail.csv'
    return str(myString)
