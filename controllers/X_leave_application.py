...
def leave_application_approval():               
    ...
    record_id=str(request.vars.record_id)
    emp_id=''
    emp_name=''
    from_date=''
    to_date=''
    ...
    recordRows = db((db.sm_leave_application.cid == cid)  &  (db.sm_leave_application.emp_id==rep_id ) &  (db.sm_leave_application.id==record_id )).select(db.sm_leave_application.ALL, orderby=db.sm_leave_application.emp_id)
    # return recordRows
    ...
    return dict(depthNo=depthNo,cid=cid,uid=uid,u_name=u_name,ared_head_id=ared_head_id,ared_head_name=ared_head_name,u_pass=u_pass,rep_id=rep_id,rep_name=rep_name,emp_id=emp_id,emp_name=emp_name,from_date=from_date,to_date=to_date,total_leave=total_leave,leave_type=leave_type,record_id=record_id)
...
def leave_application_approval_p():               
    ...
    return dict(zonal_head_id=zonal_head_id,zonal_head_name=zonal_head_name,depthNo=depthNo,cid=cid,uid=uid,u_name=u_name,ared_head_id=ared_head_id,ared_head_name=ared_head_name,u_pass=u_pass,rep_id=rep_id,rep_name=rep_name,emp_id=emp_id,emp_name=emp_name,from_date=from_date,to_date=to_date,total_leave=total_leave,leave_type=leave_type,record_id=record_id)
...
def leave_application_confirm():              
    ...
    record_id=str(request.vars.record_id).strip()
    ...
    if checkRep: 
        ...
        if btn_approve_application:            
            ...
            if depth== '2'  :            
                ...
                statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == rep_id) & (db.sm_leave_application.id == record_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,ared_head_id=uid,ared_head_name=u_name,area_head_status='Approved')            
                session.flash='Approved Successfully.'          
    
            if depth== '1'  : 

                # return from_dt                   
                statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == rep_id) & (db.sm_leave_application.id == record_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,zonal_head_id=uid,zonal_head_name=u_name,zonal_head_status='Approved')            
                session.flash='Approved Successfully.' 

            if depth== '0' :                
                statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == rep_id) & (db.sm_leave_application.id == record_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,divisional_head_id=uid,divisional_head_name=u_name,divisional_head_status='Approved')            
                session.flash='Approved Successfully.'


        if btn_reject_application:            
            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == rep_id) & (db.sm_leave_application.id == record_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,ared_head_id=uid,ared_head_name=u_name,area_head_status='Rejected')              
...
def director_approveList():              
    ...
    if checkSettings:
        ...
        if checkRep: 
            ...
            leave_req = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.divisional_head_status == 'Approved') & (db.sm_leave_application.director_marktng_status == '')  ).select(db.sm_leave_application.emp_id, db.sm_leave_application.emp_name, db.sm_leave_application.from_date,db.sm_leave_application.to_date,db.sm_leave_application.total_leave, db.sm_leave_application.leave_type, db.sm_leave_application.id, orderby=db.sm_leave_application.emp_id)                                    
            # return leave_req
...
def leave_application_approval_d():               
    ...
    return dict(director_id=director_id,director_pass=director_pass,zonal_head_id=zonal_head_id,zonal_head_name=zonal_head_name,depthNo=depthNo,cid=cid,uid=uid,u_name=u_name,ared_head_id=ared_head_id,ared_head_name=ared_head_name,emp_id=emp_id,emp_name=emp_name,from_date=from_date,to_date=to_date,total_leave=total_leave,leave_type=leave_type,record_id=record_id)
...
def leave_application_dConfirm():              
    ...
    d_pass=str(request.vars.director_pass).strip().upper()
    ...
    if checkRep: 
        ...
        if btn_approve_application:            
            ...
            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == emp_id) & (db.sm_leave_application.id == record_id) & (db.sm_leave_application.director_marktng_status == '')).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,director_id=director_id,director_marktng_status='Approved')            
            session.flash='Approved Successfully.'
            redirect(URL(c='leave_application',f='leave_application_approval_d'))

            
        if btn_reject_application:            
            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == emp_id)& (db.sm_leave_application.id == record_id) ).update(director_id=director_id,director_marktng_status='Rejected')              
            session.flash='Rejected Successfully.'
            redirect(URL(c='leave_application',f='leave_application_approval_d'))
    else:
        session.flash='Invalid Authorization.'         
