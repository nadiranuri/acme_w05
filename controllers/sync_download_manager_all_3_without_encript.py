...
#        ====================if user type HO===============
    ...
    if(depot_mac == mac_get):
        ...
        sl_list = []
        update_string_list = update_string.split(',', update_string.count(',')) #Split main data by.  
        total_update_string_list = update_string.count(',') #count total .
        i = 0
        while i < total_update_string_list + 1: 
            sl_list.append(update_string_list[i])
            i=i+1
        
#        try:
        query = db((db.sm_order.cid == cid) & (db.sm_order.depot_id == user_id) & (db.sm_order.depot_status == 1) & (db.sm_order.sl.belongs(sl_list))).select(db.sm_order.depot_status)
#        return db._lastsql
#        return query
        result="all_zero"
        if query:
...
#        success = "success";
        return "<STARTSTART>" + result + "<ENDEND>"
