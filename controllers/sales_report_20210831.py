    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            ...
            if not(session.from_dt=='' or session.from_dt==None or session.to_date=='' or session.to_date==None):
                ...
                sql_str="SELECT  level3_id,  level3_name,area_id,  area_name,order_date FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND order_date >= '"+ str(session.from_dt) +"' AND order_date <= '"+ str(session.to_date) +"' "+ str(condition) +" GROUP BY level3_id,order_date order by order_date desc;"
                # return sql_str
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass
            ...
            sql_str="SELECT level3_id,  level3_name,area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date  =  '"+ str(session.from_dt) +"' "+ str(condition) +" GROUP BY level3_id,order_date order by order_date desc;"
            records_ov=db.executesql(sql_str,as_dict = True)
        else:
            # return 'sd'
            sql_str="SELECT level3_id,  level3_name,area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date  =  '"+ str(order_date) +"' "+ str(condition) +" GROUP BY level3_id,order_date order by order_date desc;"
            records_ov=db.executesql(sql_str,as_dict = True)
