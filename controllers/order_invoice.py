...
def invoice_list():
    ...
    depot_id_value=str(request.vars.depot_id_value).strip()
    search_ord_date_from = request.vars.search_ord_date_from
    search_ord_date_to = request.vars.search_ord_date_to
    ...
    if btn_filter_invoice:
        ...
        if search_ord_date_from != '' and search_ord_date_to != '':
            try:
                from_dt2 = datetime.datetime.strptime(str(search_ord_date_from), '%Y-%m-%d')
                to_dt2 = datetime.datetime.strptime(str(search_ord_date_to), '%Y-%m-%d')
                if from_dt2 > to_dt2:
                    dateFlag = False
            except:
                dateFlag = False
            ...
            if dateFlag == False:
                ...
                if dateDiff > 7:
                    response.flash = "Maximum 7 days allowed between Date Range"
                    dateFlag = False

            if dateFlag == True:
                session.search_ord_date_from = search_ord_date_from
                session.search_ord_date_to = search_ord_date_to
            else:
                session.search_ord_date_from = None
                session.search_ord_date_to = None
...
#             qset=qset(db.sm_invoice_head.depot_id==session.depot_id)
    ...
    if session.btn_filter_invoice:
        ...
        if session.search_ord_date_from!=None and session.search_ord_date_to!=None:
            search_ord_date_from_v=datetime.datetime.strptime(str(session.search_ord_date_from),'%Y-%m-%d')
            search_ord_date_to_v = datetime.datetime.strptime(str(session.search_ord_date_to), '%Y-%m-%d')
            search_ord_date_to_v=search_ord_date_to_v+datetime.timedelta(days=1)

            qset=qset((db.sm_invoice_head.order_datetime>=search_ord_date_from_v)&(db.sm_invoice_head.order_datetime<search_ord_date_to_v))
    ...
    records=qset.select(db.sm_invoice_head.id,db.sm_invoice_head.depot_id,db.sm_invoice_head.depot_name,db.sm_invoice_head.sl,db.sm_invoice_head.order_sl,db.sm_invoice_head.order_datetime,db.sm_invoice_head.delivery_date,db.sm_invoice_head.client_id,db.sm_invoice_head.client_name,db.sm_invoice_head.area_id,db.sm_invoice_head.area_name,db.sm_invoice_head.d_man_id,db.sm_invoice_head.market_id,db.sm_invoice_head.market_name,db.sm_invoice_head.rep_id,db.sm_invoice_head.rep_name,db.sm_invoice_head.status,db.sm_invoice_head.field2,db.sm_invoice_head.note,db.sm_invoice_head.total_amount,db.sm_invoice_head.discount,db.sm_invoice_head.vat_total_amount,db.sm_invoice_head.sp_discount,db.sm_invoice_head.return_tp,db.sm_invoice_head.return_vat,db.sm_invoice_head.return_discount,db.sm_invoice_head.collection_amount,orderby=~db.sm_invoice_head.id,limitby=limitby)

    totalRecordRows=qset.select(db.sm_invoice_head.id.count(),groupby=db.sm_invoice_head.cid)
    totalRecords=0
    for row in totalRecordRows:
        totalRecords=row[db.sm_invoice_head.id.count()]
...
def download_invoice_head():
    ...
    if (session.btn_filter_invoice):
        ...
        if (session.search_type_invoice=='STATUS'):
            qset=qset(db.sm_invoice_head.status==session.search_value_invoice)
        #------------
        ...
        session.flash='Required filter by depot, date range and status'
        redirect (URL('invoice_list'))
        
    records=qset.select(db.sm_invoice_head.depot_id,db.sm_invoice_head.depot_name,db.sm_invoice_head.sl,db.sm_invoice_head.order_sl,db.sm_invoice_head.order_datetime,db.sm_invoice_head.delivery_date,db.sm_invoice_head.client_id,db.sm_invoice_head.client_name,db.sm_invoice_head.area_id,db.sm_invoice_head.market_id,db.sm_invoice_head.market_name,db.sm_invoice_head.rep_id,db.sm_invoice_head.rep_name,db.sm_invoice_head.status,orderby=~db.sm_invoice_head.id)
            ...
            if reqSl!=0 :
                ...
                if (return_date=='' or invoice_sl <= 0):
                    ...
                    if not reqRecords:
                        ...
                        # MR check
                        collection_amount = 0
                        collRecords = db((db.sm_payment_collection.cid == c_id) & (db.sm_payment_collection.depot_id == req_depot_id) & (db.sm_payment_collection.sl == invoice_sl) & (db.sm_payment_collection.status == 'Posted')).select(db.sm_payment_collection.sl,db.sm_payment_collection.collection_amount.sum(),groupby=db.sm_payment_collection.sl,limitby=(0, 1))
                        if collRecords:
                            collection_amount = collRecords[0][db.sm_payment_collection.collection_amount.sum()]

                        if collection_amount > 0:
                            session.flash = 'Already Money Received Exists'
                        else:
                    ...
                    if not rows:
                        ...
                        if flag=='True':
                            ...
                            invHRow = db((db.sm_invoice_head.cid == c_id) & (
                                ...
                                db.sm_invoice_head.id, db.sm_invoice_head.return_tp,
                                db.sm_invoice_head.return_vat,
                                db.sm_invoice_head.return_discount,
                                db.sm_invoice_head.return_sp_discount,
                                db.sm_invoice_head.discount,
                                db.sm_invoice_head.vat_total_amount,
                                db.sm_invoice_head.sp_discount, db.sm_invoice_head.return_count,
                                db.sm_invoice_head.actual_total_tp,
                                db.sm_invoice_head.regular_disc_tp,
                                db.sm_invoice_head.flat_disc_tp,
                                db.sm_invoice_head.approved_disc_tp,
                                db.sm_invoice_head.others_disc_tp, db.sm_invoice_head.sp_flat,
                                db.sm_invoice_head.sp_approved, db.sm_invoice_head.sp_others,
                                db.sm_invoice_head.ret_actual_total_tp,
                                db.sm_invoice_head.ret_regular_disc_tp,
                                db.sm_invoice_head.ret_flat_disc_tp,
                                db.sm_invoice_head.ret_approved_disc_tp,
                                db.sm_invoice_head.ret_others_disc_tp,
                                db.sm_invoice_head.ret_sp_flat,
                                db.sm_invoice_head.ret_sp_approved,
                                db.sm_invoice_head.ret_sp_others,
                            ...
                            insertRpt = db.sm_rpt_transaction.insert(cid=c_id, depot_id=req_depot,
                                                                     ...
                                                                     shipment_no=shipment_no, note=note)
