...
def prescription_details():
    ...
    pr_deletebtn = request.vars.pr_deletebtn
    if pr_deletebtn:
        rowid = request.vars.rowid 
        pr_delete = request.vars.pr_delete
        if pr_delete=='yes': 
             
            prHeadRow = db((db.sm_prescription_seen_head.cid == c_id) & (db.sm_prescription_seen_head.id == rowid)).select(db.sm_prescription_seen_head.sl,db.sm_prescription_seen_head.doctor_id,db.sm_prescription_seen_head.doctor_name,db.sm_prescription_seen_head.area_id,db.sm_prescription_seen_head.area_name,db.sm_prescription_seen_head.created_on,db.sm_prescription_seen_head.submit_by_name,db.sm_prescription_seen_head.submit_by_id,db.sm_prescription_seen_head.rx_type,db.sm_prescription_seen_head.image_name, limitby=(0, 1))
            # return prHeadRow
            if not prHeadRow:
                session.flash = 'Invalid request'
                redirect (URL('prescription_list'))
            else: 
                pr_hsl=prHeadRow[0].sl
            pr_headRow = db((db.sm_prescription_seen_head.cid == c_id) & (db.sm_prescription_seen_head.id == rowid)).delete()
            detailRow = db((db.sm_prescription_seen_details.cid == c_id) & (db.sm_prescription_seen_details.sl == pr_hsl)).delete()
    ...
    return dict(page=page, headRow=headRow,detailsRow=detailsRow,repRows=repRows,rowid=rowid)
