...
#                 return menuStr
                ...
                if checkSettings:
                    linkpathRows = db((db.sm_linkpath.cid == cid) & (db.sm_linkpath.user_type == 'sup') & (db.sm_linkpath.link_name != 'LEAVE APPLICATION') & (db.sm_linkpath.link_name != 'APPROVE LIST') & (db.sm_linkpath.link_name != 'DDM')).select(db.sm_linkpath.link_name, db.sm_linkpath.link_path, db.sm_linkpath.field1,orderby=db.sm_linkpath.link_name)
                elif checkSettings_dd:
                    linkpathRows = db((db.sm_linkpath.cid == cid) & (db.sm_linkpath.user_type == 'sup') & (db.sm_linkpath.link_name != 'LEAVE APPLICATION') & (db.sm_linkpath.link_name != 'APPROVE LIST') & (db.sm_linkpath.link_name != 'Director Sales​')).select(db.sm_linkpath.link_name, db.sm_linkpath.link_path, db.sm_linkpath.field1,orderby=db.sm_linkpath.link_name)
