...
def sales_report_zm_0():
    ...
    if date_from==date_to:
        date_to_invoice=date_to_next
    else:
        date_to_invoice=date_to
    session.date_to_invoice=date_to_invoice
    # return session.date_to_invoice
    ...
    if (user_type=='SUP'):
        ...
        qstInvcCount = qstInvcCount(db.sm_invoice_head.invoice_date <= session.date_to_invoice)
        ...
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.invoice_date <= session.date_to_invoice)
        ...
        if session.btn_filter:
            ...
            if not(session.from_dt=='' and session.from_dt==None and session.to_date=='' and session.to_date==None):
                ...
                # recordsInvcCount = qstInvcCount.select(db.sm_invoice_head.sl,db.sm_invoice_head.level0_id,db.sm_invoice_head.order_datetime, orderby=db.sm_invoice_head.level0_name, groupby=db.sm_invoice_head.level0_id)
                ...
                # recordsInvcAmount = qstInvcAmnt.select(((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum(),((db.sm_invoice.price)*(db.sm_invoice.return_qty)).sum(),db.sm_invoice.level0_id,db.sm_invoice.level0_name,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level0_name, groupby=db.sm_invoice.level0_id )                     
                # # return db._lastsql
                # invcAmntChecklist=[]
                # invcAmntList=[]
                # for recordsInvcAmount in recordsInvcAmount:
                #     # invcAmnt=recordsInvcAmount[((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum()]
                #     invcAmnt=(recordsInvcAmount[((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum()])-(recordsInvcAmount[((db.sm_invoice.price)*(db.sm_invoice.return_qty)).sum()])                
                ...
                recordsInvcAmount = qstInvcAmnt.select(( (db.sm_invoice.actual_tp)*((db.sm_invoice.quantity)+(db.sm_invoice.bonus_qty)-(db.sm_invoice.return_qty)-(db.sm_invoice.return_bonus_qty))).sum(),db.sm_invoice.level0_id,db.sm_invoice.level0_name,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level0_name, groupby=db.sm_invoice.level0_id )
                # return db._lastsql
                invcAmntChecklist=[]
                invcAmntList=[]
                for recordsInvcAmount in recordsInvcAmount:
...
def sales_report_zm():
    ...
    if (user_type=='SUP'):
        ...
        try:
            ...
            now = datetime.datetime.strptime(str(current_date), "%Y-%m-%d")
        date_ton=now + datetime.timedelta(days = 1)
        date_to_next=str(date_ton).split(' ')[0]
        ...
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.cid == cid)
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.invoice_date >= session.from_dt)
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.order_datetime <= session.date_to_invoice)
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.status == 'Invoiced')
        ...
        if session.btn_filter:   
            ...
            # recordsInvcCount = qstInvcCount.select(db.sm_invoice_head.sl,db.sm_invoice_head.level1_id,db.sm_invoice_head.order_datetime, orderby=db.sm_invoice_head.level1_name, groupby=db.sm_invoice_head.level1_id )
            # return db._lastsql
            invcChecklist=[]
            invcCountList=[]
            for recordsInvcCount in recordsInvcCount:
            ...
            recordsInvcAmount = qstInvcAmnt.select(( (db.sm_invoice.actual_tp)*((db.sm_invoice.quantity)+(db.sm_invoice.bonus_qty)-(db.sm_invoice.return_qty)-(db.sm_invoice.return_bonus_qty))).sum(),db.sm_invoice.level1_id,db.sm_invoice.level1_name,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level1_name, groupby=db.sm_invoice.level1_id )
...
def sales_report_rsm():
    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            ...
            recordsInvcAmount = qstInvcAmnt.select(( (db.sm_invoice.actual_tp)*((db.sm_invoice.quantity)+(db.sm_invoice.bonus_qty)-(db.sm_invoice.return_qty)-(db.sm_invoice.return_bonus_qty))).sum(),db.sm_invoice.level2_id,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level2_name, groupby=db.sm_invoice.level2_id)
...
def sales_report_fm():
    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            ...
            recordsInvcAmount = qstInvcAmnt.select(( (db.sm_invoice.actual_tp)*((db.sm_invoice.quantity)+(db.sm_invoice.bonus_qty)-(db.sm_invoice.return_qty)-(db.sm_invoice.return_bonus_qty))).sum(),db.sm_invoice.level3_id,db.sm_invoice.level3_name,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level3_name, groupby=db.sm_invoice.level3_id)
            ...
            recordsInvcAmount = qstInvcAmnt.select(( (db.sm_invoice.actual_tp)*((db.sm_invoice.quantity)+(db.sm_invoice.bonus_qty)-(db.sm_invoice.return_qty)-(db.sm_invoice.return_bonus_qty))).sum(),db.sm_invoice.level3_id,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level3_name, groupby=db.sm_invoice.level3_id)
    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            ...
            if not(session.from_dt!='' and session.from_dt!=None and session.to_date!='' and session.to_date!=None):
                ...
                # qstInvcCount = qstInvcCount(db.sm_invoice_head.invoice_date >= session.from_dt)
                # qstInvcCount = qstInvcCount(db.sm_invoice_head.invoice_date <= session.to_date)
            ...
            recordsInvcAmount = qstInvcAmnt.select(( (db.sm_invoice.actual_tp)*((db.sm_invoice.quantity)+(db.sm_invoice.bonus_qty)-(db.sm_invoice.return_qty)-(db.sm_invoice.return_bonus_qty))).sum(),db.sm_invoice.level2_id,db.sm_invoice.level2_name,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level2_name, groupby=db.sm_invoice.level2_id)
...
def sales_report_detail_url():
    ...
    if btn_filter:
        ...
        try:
            from_dt=datetime.datetime.strptime(str(from_dt),'%Y-%m-%d')
            to_date=datetime.datetime.strptime(str(to_date),'%Y-%m-%d')
            date_ton=to_date + datetime.timedelta(days = 1)
            date_to=str(date_ton).split(' ')[0]
            session.from_dt=str(from_dt).split(' ')[0]
    ...
    try:
        ...
        if (user_type=='REP'):
            ...
            if session.btn_filter:
                ...
                sql_str="SELECT  area_id, max(area_name),rep_id,max(rep_name),field1 FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND order_date >= '"+ str(session.from_dt).split(' ')[0] +"' AND order_date < '"+ str(session.to_date) +"' "+ condition + " GROUP BY area_id, area_name,rep_id,rep_name  order by area_name,rep_name asc;"
                # return sql_str
                records_ov=db.executesql(sql_str,as_dict = True)
                ...
                # recordsInvcAmount = qstInvcAmnt.select(( (db.sm_invoice.actual_tp)*((db.sm_invoice.quantity)+(db.sm_invoice.bonus_qty)-(db.sm_invoice.return_qty)-(db.sm_invoice.return_bonus_qty))).sum(),db.sm_invoice.area_id,db.sm_invoice.order_datetime, orderby=db.sm_invoice.area_name, groupby=db.sm_invoice.area_id)
                # return recordsInvcAmount     
                invcAmntChecklist=[]
                invcAmntList=[]
                # for recordsInvcAmount in recordsInvcAmount:
            ...
            for SuplevelRows in SuplevelRows:
                ...
                if Suplevel_id not in levelList:
                    levelList.append(Suplevel_id)
                    if level_str=='':
                        level_str="'"+str(Suplevel_id)+"'" 
                    else:
                        level_str=level_str+",'"+str(Suplevel_id)+"'" 
            ...
            # =============
            if int(depth)==0:
                c_sql_str="SELECT * FROM `sm_supervisor_level` WHERE level_id in ("+str(level_str)+")  and level_id in (SELECT level0 FROM `sm_level` WHERE `special_territory_code` IS NOT NULL AND `depth` = 3 and special_territory_code!='NULL')"
                records_c_team=db.executesql(c_sql_str,as_dict = True)
            if int(depth)==1:
                c_sql_str="SELECT * FROM `sm_supervisor_level` WHERE level_id in ("+str(level_str)+")  and level_id in (SELECT level1 FROM `sm_level` WHERE `special_territory_code` IS NOT NULL AND `depth` = 3 and special_territory_code!='NULL')"
                records_c_team=db.executesql(c_sql_str,as_dict = True)
            if int(depth)==2:
            ...
            for i in range(len(levelList)):
                ...
                for levelRow in levelRows:
                    ...
                    # else:
                    if level_id not in rp_areaList:
            ...
            qstOAmount = qstOAmount(db.sm_order.order_date >= session.from_dt)
            qstOAmount = qstOAmount(db.sm_order.order_date < session.to_date)
            qstOAmount = qstOAmount(db.sm_order.rep_id.belongs(rp_List))
...
def sales_report_area_wise_url():
    ...
    area_name = str(request.vars.area_name).strip()
    ...
    # session.order_date=order_date
    redirect(URL(c='sales_report_invoice',f='sales_report_area_wise',vars=dict(invoice_count=invoice_count,order_count=order_count)))
...
def sales_report_area_wise():
    ...
    invoice_count=str(request.vars.invoice_count).strip()
    order_count_show=session.order_count
    ...
    if (user_type=='REP'):
        ...
        repAreaRow = db((db.sm_rep_area.cid == cid) & (db.sm_rep_area.rep_id == s_id)).select(db.sm_rep_area.area_id,orderby=db.sm_rep_area.area_id,groupby=db.sm_rep_area.area_id)
        if not repAreaRow:
           retStatus = 'FAILED<SYNCDATA>Invalid Area'
           return retStatus
        else:
...
def sales_report_slWise():
    ...
    return dict(records_ov=records_ov,vsl=vsl,order_date=order_date,area_id=area_id,area_name=area_name,client_id=client_id,rep_name=rep_name,rep_id=rep_id)
