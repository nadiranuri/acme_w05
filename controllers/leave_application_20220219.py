...
def get_available_leave():
    ...
    totalLeave=''
    checkLtype=  db((db.sm_leave_type.cid == c_id) & (db.sm_leave_type.leave_type == leave_type)).select(db.sm_leave_type.total_leave,limitby=(0, 1))
    # return checkLtype
    ...
    checkLeave=  db ((db.sm_leave_application.cid == c_id) & (db.sm_leave_application.leave_type == leave_type) & (db.sm_leave_application.first_date == currentMonth) & (db.sm_leave_application.emp_id == rep_id) ).select(db.sm_leave_application.ALL, orderby=db.sm_leave_application.emp_id)
    # return checkLeave
    if checkLeave:
        checkLeave=int(checkLeave[0].total_leave)
