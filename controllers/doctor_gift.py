...
def validation_gift_add(form):
    ...
    #------- check duplicate
    existRows = db((db.sm_doctor_gift.cid == c_id) & (db.sm_doctor_gift.gift_id == gift_id)).select(db.sm_doctor_gift.gift_id, limitby=(0, 1))
    if existRows:
...
def gift_add():
    ...
    response.title = 'Gift Item'
    
    c_id = session.cid
    
    #   ---------------------
    form = SQLFORM(db.sm_doctor_gift,
    ...
    qset = db()
    qset = qset(db.sm_doctor_gift.cid == c_id)
    if (session.btn_filter_gift):

        if (session.searchType_gift == 'GiftID'):
            qset = qset(db.sm_doctor_gift.gift_id == session.searchValue_gift)

        elif (session.searchType_gift == 'Status'):
            qset = qset(db.sm_doctor_gift.status == session.searchValue_gift)

    records = qset.select(db.sm_doctor_gift.ALL, orderby=db.sm_doctor_gift.gift_name, limitby=limitby)
...
def gift_edit():
    ...
    record = db.sm_doctor_gift(rowID) or redirect(URL('gift_add'))
...
def download_gift():
    ...
    records = qset.select(db.sm_doctor_gift.ALL, orderby=db.sm_doctor_gift.gift_name)

    #---------
    myString = 'Gift List\n\n'
    myString += 'GiftID,Name,Description,Status\n'
    for rec in records:
    ...
    response.headers['Content-disposition'] = 'attachment; filename=download_gift.csv'
    return str(myString)
...
def batch_upload_gift():
    ...
    #----------
    response.title = 'Gift Batch Upload'
    ...
    if btn_upload == 'Upload':
        ...
        #        create client list
        existClientRows = db((db.sm_doctor_gift.cid == c_id) & (db.sm_doctor_gift.gift_id.belongs(gift_list_excel))).select(db.sm_doctor_gift.gift_id, orderby=db.sm_doctor_gift.gift_id)
                ...
                if not(gift_id == '' or gift_name == ''):
                    try:
                        ...
                        if(duplicate_gift == False):
                            ...
                            error_data = row_data + '(duplicate gifID)\n'
                            error_str = error_str + error_data
                            count_error += 1
                            continue
                    ...
                    error_data = row_data + '(giftID and name needed)\n'
                    error_str = error_str + error_data
                    count_error += 1
                    continue
            else:
        ...
        if len(ins_list) > 0:
            inCountList = db.sm_doctor_gift.bulk_insert(ins_list)
