...
# ===================== 20220209 sales order report  START=================
...
def salsCall_order_smry():
    ...
    if (user_type=='REP'):
        ...
        for i in range(len(records_ov)): 
            ...
            gtotalprice=float(gtotalprice)+float(totalprice)
            gintotalprice=float(gintotalprice)+float(invoice_totalprice)
...
# ===================== 20220209 sales order report  END=================
...
def salesRprt_doctor():
    ...
    if (user_type=='SUP'):   
        ...
        if (level=='level2'):
            linkPath="window.open('"+sals_rprt_dcr_more_path+"salesDcr_report_detail_url?"+"cid="+cid+"&rep_id="+rep_id+"&password="+password+"&synccode="+synccode+"&se_item_report="+se_item_report+"&se_market_report="+se_market_report+" ', '_system');"
            pass
...
#                 return menuStr
                ...
                if checkSettings:
                    linkpathRows = db((db.sm_linkpath.cid == cid) & (db.sm_linkpath.user_type == 'sup') & (db.sm_linkpath.link_name != 'LEAVE APPLICATION') & (db.sm_linkpath.link_name != 'APPROVE LIST')).select(db.sm_linkpath.link_name, db.sm_linkpath.link_path, db.sm_linkpath.field1,orderby=db.sm_linkpath.link_name)

                else:
                    linkpathRows = db((db.sm_linkpath.cid == cid) & (db.sm_linkpath.user_type == 'sup') & (db.sm_linkpath.link_name != 'Director Sales​')).select(db.sm_linkpath.link_name, db.sm_linkpath.link_path, db.sm_linkpath.field1,orderby=db.sm_linkpath.link_name)
#                 return db._lastsql
                linkStr=''
                for linkpathRows in linkpathRows:
