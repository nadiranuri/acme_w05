        ...
        if session.btn_filter:
            ...
            if not(session.from_dt=='' and session.from_dt==None and session.to_date=='' and session.to_date==None):
                ...
                for recordsV_Count in recordsV_Count:
                    ...
                    vCheck=str(recordsV_Count[db.sm_order_head.level0_id]) +"|"+str(recordsV_Count[db.sm_order_head.order_date])
                ...
                for recordsO_Count in recordsO_Count:
                    ...
                    oCheck=str(recordsO_Count[db.sm_order_head.level0_id]) +"|"+str(recordsO_Count[db.sm_order_head.order_date])
                ...
                for recordsO_amount in recordsO_amount:
                    ...
                    oaCheck=str(recordsO_amount[db.sm_order.level0_id]) +"|"+str(recordsO_amount[db.sm_order.order_date])
                ...
                for recordsInvcCount in recordsInvcCount:
                    ...
                    invcCheck=str(recordsInvcCount[db.sm_invoice_head.level0_id]) +"|"+str(invcDateTime).split(' ')[0]
                ...
                for recordsInvcAmount in recordsInvcAmount:
                    ...
                    invcAmountCheck=str(recordsInvcAmount[db.sm_invoice.level0_id])+"|"+str(invcDateTime).split(' ')[0]
...
def sales_report_zm():
    ...
    if (user_type=='SUP'):
        ...
        qsetOCount = qsetOCount(db.sm_order_head.cid == cid) 
        qsetOCount = qsetOCount(db.sm_order_head.order_date >= date_from)
        qsetOCount = qsetOCount(db.sm_order_head.order_date <= date_to)
        if (level=='level0'): 
        ...
        qstOAmount = qstOAmount(db.sm_order.cid == cid) 
        qstOAmount = qstOAmount(db.sm_order.order_date >= date_from)
        qstOAmount = qstOAmount(db.sm_order.order_date <= date_to)
        if (level=='level0'): 
        ...
        qstInvcCount = db()
        qstInvcCount = qstInvcCount(db.sm_invoice_head.cid == cid)
        qstInvcCount = qstInvcCount(db.sm_invoice_head.order_datetime >= date_from)
        qstInvcCount = qstInvcCount(db.sm_invoice_head.order_datetime < date_to_next)
        qstInvcCount = qstInvcCount(db.sm_invoice_head.status == 'Invoiced')
        if (level=='level0'): 
        ...
        qstInvcAmnt = db()
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.cid == cid)
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.order_datetime >= date_from)
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.order_datetime < date_to_next)
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.status == 'Invoiced')
        if (level=='level0'): 
        ...
        if session.btn_filter: 
            ...
            for recordsV_Count in recordsV_Count:
                vCount=recordsV_Count[db.sm_order_head.sl.count()]
                vCheck=str(recordsV_Count[db.sm_order_head.level1_id])+"|"+str(recordsV_Count[db.sm_order_head.order_date])
            ...
            for recordsO_Count in recordsO_Count:
                oCount=recordsO_Count[db.sm_order_head.sl.count()]
                oCheck=str(recordsO_Count[db.sm_order_head.level1_id]) +"|"+str(recordsO_Count[db.sm_order_head.order_date])
                OChecklist.append(oCheck)
                OCountList.append(oCount) 
            ...
            for recordsO_amount in recordsO_amount:
                ...
                oaCheck=str(recordsO_amount[db.sm_order.level1_id])+"|"+str(recordsO_amount[db.sm_order.order_date])
                OAmountChecklist.append(oaCheck)
            ...
            for recordsInvcCount in recordsInvcCount:
                invcCount=recordsInvcCount[db.sm_invoice_head.sl.count()]
                invcDateTime=recordsInvcCount[db.sm_invoice_head.order_datetime]
                invcCheck=str(recordsInvcCount[db.sm_invoice_head.level1_id]) +"|"+str(invcDateTime).split(' ')[0]
                invcChecklist.append(invcCheck)
                invcCountList.append(invcCount)
            ...
            for recordsInvcAmount in recordsInvcAmount:
                invcAmnt=recordsInvcAmount[((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum()]
                invcDateTime=recordsInvcAmount[db.sm_invoice.order_datetime]
                invcAmountCheck=str(recordsInvcAmount[db.sm_invoice.level1_id]) +"|"+str(invcDateTime).split(' ')[0]
                invcAmntChecklist.append(invcAmountCheck)
                invcAmntList.append(invcAmnt)
            ...
            recordsO_amount = qstOAmount.select(((db.sm_order.price) * ( db.sm_order.quantity )).sum(),db.sm_order.level1_id,db.sm_order.order_date, orderby=db.sm_order.level1_name, groupby=db.sm_order.level1_id | db.sm_order.order_date)
            ...
            recordsInvcCount = qstInvcCount.select(db.sm_invoice_head.sl.count(),db.sm_invoice_head.level1_id,db.sm_invoice_head.order_datetime, orderby=db.sm_invoice_head.level1_name, groupby=db.sm_invoice_head.level1_id | db.sm_invoice_head.order_datetime)
            ...
            recordsInvcAmount = qstInvcAmnt.select(((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum(),db.sm_invoice.level1_id,db.sm_invoice.level1_name,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level1_name, groupby=db.sm_invoice.level1_id | db.sm_invoice.order_datetime)
    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            ...
            if not(session.from_dt=='' or session.from_dt==None or session.to_date=='' or session.to_date==None):
                ...
                # qstInvcCount = qstInvcCount(db.sm_invoice_head.order_datetime >= session.from_dt)
                # qstInvcCount = qstInvcCount(db.sm_invoice_head.order_datetime < session.to_date)

                # qstInvcAmnt = qstInvcAmnt(db.sm_invoice.order_datetime >= session.from_dt)
                # qstInvcAmnt = qstInvcAmnt(db.sm_invoice.order_datetime < session.to_date)

                recordsV_Count = qsetVCount.select(db.sm_order_head.sl.count(),db.sm_order_head.level2_id,db.sm_order_head.level2_name,db.sm_order_head.order_date, orderby=db.sm_order_head.level2_name, groupby=db.sm_order_head.level2_id|db.sm_order_head.order_date)

                vChecklist=[]
                vCountList=[]
                for recordsV_Count in recordsV_Count:
                    vCount=recordsV_Count[db.sm_order_head.sl.count()]
                    vCheck=str(recordsV_Count[db.sm_order_head.level2_id])+'|'+str(recordsV_Count[db.sm_order_head.order_date])
                    vChecklist.append(vCheck)
                    vCountList.append(vCount) 



                recordsO_Count = qsetOCount.select(db.sm_order_head.sl.count(),db.sm_order_head.level2_id,db.sm_order_head.order_date, orderby=db.sm_order_head.level2_name, groupby=db.sm_order_head.level2_id|db.sm_order_head.order_date)
                OChecklist=[]
                OCountList=[]
                for recordsO_Count in recordsO_Count:
                    oCount=recordsO_Count[db.sm_order_head.sl.count()]
                    oCheck=str(recordsO_Count[db.sm_order_head.level2_id])+'|'+str(recordsO_Count[db.sm_order_head.order_date])
                    OChecklist.append(oCheck)
                    OCountList.append(oCount)



                recordsO_amount = qstOAmount.select(((db.sm_order.price) * ( db.sm_order.quantity )).sum(),db.sm_order.level2_id,db.sm_order.order_date, orderby=db.sm_order.level2_name, groupby=db.sm_order.level2_id|db.sm_order.order_date)
                 
                OAmountChecklist=[]
                OamountList=[]
                for recordsO_amount in recordsO_amount:
                    oaCount=recordsO_amount[((db.sm_order.price)*(db.sm_order.quantity)).sum()]
                    oaCheck=str(recordsO_amount[db.sm_order.level2_id])+'|'+str(recordsO_amount[db.sm_order.order_date])
                    OAmountChecklist.append(oaCheck)
                    OamountList.append(oaCount)



                recordsInvcCount = qstInvcCount.select(db.sm_invoice_head.sl.count(),db.sm_invoice_head.level2_id,db.sm_invoice_head.order_datetime, orderby=db.sm_invoice_head.level2_name, groupby=db.sm_invoice_head.level2_id|db.sm_invoice_head.order_datetime)
                ...
                for recordsInvcCount in recordsInvcCount:
                    invcCount=recordsInvcCount[db.sm_invoice_head.sl.count()]
                    invcDateTime=recordsInvcCount[db.sm_invoice_head.order_datetime]
                    invcCheck=str(recordsInvcCount[db.sm_invoice_head.level2_id])+"|"+str(invcDateTime).split(' ')[0]
                    invcChecklist.append(invcCheck)
                    invcCountList.append(invcCount)

                recordsInvcAmount = qstInvcAmnt.select(((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum(),db.sm_invoice.level2_id,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level0_name, groupby=db.sm_invoice.level2_id|db.sm_invoice.order_datetime)
                ...
                for recordsInvcAmount in recordsInvcAmount:
                    invcAmnt=recordsInvcAmount[((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum()]
                    invcDateTime=recordsInvcCount[db.sm_invoice_head.order_datetime]
                    invcAmountCheck=str(recordsInvcAmount[db.sm_invoice.level2_id])+"|"+str(invcDateTime).split(' ')[0]
                    invcAmntChecklist.append(invcAmountCheck)
                    invcAmntList.append(invcAmnt)
...
def sales_report_fm_url():
    ...
    redirect(URL(c='sales_report_invoice',f='sales_report_fm'))
...
def sales_report_fm():
    ...
    if (user_type=='SUP'):
        ...
        # return level2List
        # return str(date_from)+'.......'+str(date_to_next)
        qstInvcCount = db()
        qstInvcCount = qstInvcCount(db.sm_invoice_head.cid == cid)
        qstInvcCount = qstInvcCount(db.sm_invoice_head.order_datetime >= session.from_dt)
        qstInvcCount = qstInvcCount(db.sm_invoice_head.order_datetime < date_to_next)
        ...
        if session.btn_filter: 
            ...
            for recordsV_Count in recordsV_Count:
                vCount=recordsV_Count[db.sm_order_head.sl.count()]
                vCheck=str(recordsV_Count[db.sm_order_head.level3_id])+"|"+str(recordsV_Count[db.sm_order_head.order_date])
                vChecklist.append(vCheck)
                vCountList.append(vCount)  
            ...
            for recordsO_Count in recordsO_Count:
                oCount=recordsO_Count[db.sm_order_head.sl.count()]
                oCheck=str(recordsO_Count[db.sm_order_head.level3_id])+"|"+str(recordsO_Count[db.sm_order_head.order_date])
                OChecklist.append(oCheck)
                OCountList.append(oCount)
            ...
            for recordsO_amount in recordsO_amount:
                oaCount=recordsO_amount[((db.sm_order.price)*(db.sm_order.quantity)).sum()]
                oaCheck=str(recordsO_amount[db.sm_order.level3_id])+"|"+str(recordsO_amount[db.sm_order.order_date])
                OAmountChecklist.append(oaCheck)
                OamountList.append(oaCount)
            ...
            for recordsInvcCount in recordsInvcCount:
                invcCount=recordsInvcCount[db.sm_invoice_head.sl.count()]
                invcDateTime=recordsInvcCount[db.sm_invoice_head.order_datetime]
                invcCheck=str(recordsInvcCount[db.sm_invoice_head.level3_id]) +"|"+str(invcDateTime).split(' ')[0]
                invcChecklist.append(invcCheck)
                invcCountList.append(invcCount)
            ...
            for recordsInvcAmount in recordsInvcAmount:
                invcAmnt=recordsInvcAmount[((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum()]
                invcDateTime=recordsInvcAmount[db.sm_invoice.order_datetime]
                invcAmountCheck=str(recordsInvcAmount[db.sm_invoice.level3_id]) +"|"+str(invcDateTime).split(' ')[0]
                invcAmntChecklist.append(invcAmountCheck)
                invcAmntList.append(invcAmnt)

        elif session.btn_all:

            sql_str="SELECT level3_id,  level3_name,area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date  =  '"+ str(order_date) +"' "+ str(condition) +" GROUP BY level3_id order by level3_name asc;"
            records_ov=db.executesql(sql_str,as_dict = True)
            ...
            sql_str="SELECT level3_id,  level3_name,area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date  =  '"+ str(order_date) +"' "+ str(condition) +" GROUP BY level3_id,order_date order by level3_name asc;"
            records_ov=db.executesql(sql_str,as_dict = True)

            recordsV_Count = qsetVCount.select(db.sm_order_head.sl.count(),db.sm_order_head.level3_id,db.sm_order_head.level3_name,db.sm_order_head.order_date, orderby=db.sm_order_head.level3_name, groupby=db.sm_order_head.level3_id|db.sm_order_head.order_date)
            vChecklist=[]
            vCountList=[]
            for recordsV_Count in recordsV_Count:
                vCount=recordsV_Count[db.sm_order_head.sl.count()]
            ...
            recordsO_amount = qstOAmount.select(((db.sm_order.price) * (db.sm_order.quantity)).sum(),db.sm_order.level3_id,db.sm_order.order_date, orderby=db.sm_order.level3_name, groupby=db.sm_order.level3_id|db.sm_order.order_date)
            OAmountChecklist=[]
            OamountList=[]
            for recordsO_amount in recordsO_amount:
                oaCount=recordsO_amount[((db.sm_order.price)*(db.sm_order.quantity)).sum()]
...
def sales_report_detail(): 
    ...
    if (user_type=='REP'):
        ...
        qsetVCount = qsetVCount(db.sm_order_head.area_id.belongs(areaList))

        qsetOCount = db()
        qsetOCount = qsetOCount(db.sm_order_head.cid == cid) 
        qsetOCount = qsetOCount(db.sm_order_head.rep_id == rep_id) 
        qsetOCount = qsetOCount(db.sm_order_head.area_id.belongs(areaList))
        qsetOCount = qsetOCount(db.sm_order_head.field1=='ORDER')

        qstOAmount = db()
        qstOAmount = qstOAmount(db.sm_order.cid == cid) 
        qstOAmount = qstOAmount(db.sm_order.rep_id == rep_id) 
        qstOAmount = qstOAmount(db.sm_order.area_id.belongs(areaList))
        ...
        # qstInvcCount = qstInvcCount(db.sm_invoice_head.order_datetime < date_to_next)
        qstInvcCount = qstInvcCount(db.sm_invoice_head.status == 'Invoiced')
        qstInvcCount = qstInvcCount(db.sm_invoice_head.area_id.belongs(areaList))
        ...
        # qstInvcAmnt = qstInvcAmnt(db.sm_invoice.order_datetime >= date_from)
        # qstInvcAmnt = qstInvcAmnt(db.sm_invoice.order_datetime < date_to)
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.status == 'Invoiced')
        qstInvcAmnt = qstInvcAmnt(db.sm_invoice.area_id.belongs(areaList))
        ...
        if session.btn_filter:
            ...
            if not(session.from_dt=='' or session.from_dt==None or session.to_date=='' or session.to_date==None):
                ...
                for recordsV_Count in recordsV_Count:
                    vCount=recordsV_Count[db.sm_order_head.sl.count()]
                    vCheck=str(recordsV_Count[db.sm_order_head.area_id])+"|"+str(recordsV_Count[db.sm_order_head.order_date])
                    vChecklist.append(vCheck)
                    vCountList.append(vCount) 
                ...
                for recordsO_Count in recordsO_Count:
                    ...
                    oCheck=str(recordsO_Count[db.sm_order_head.area_id])+"|"+str(recordsO_Count[db.sm_order_head.order_date])
                    OChecklist.append(oCheck)
                    OCountList.append(oCount)
                ...
                for recordsO_amount in recordsO_amount:
                    oaCount=recordsO_amount[((db.sm_order.price)*(db.sm_order.quantity)).sum()]
                    oaCheck=str(recordsO_amount[db.sm_order.area_id])+"|"+str(recordsO_amount[db.sm_order.order_date])
                    OAmountChecklist.append(oaCheck)
                    OamountList.append(oaCount)



                recordsInvcCount = qstInvcCount.select(db.sm_invoice_head.sl.count(),db.sm_invoice_head.area_id,db.sm_invoice_head.order_datetime, orderby=db.sm_invoice_head.area_name, groupby=db.sm_invoice_head.area_id|db.sm_invoice_head.order_datetime)
                ...
                for recordsInvcCount in recordsInvcCount:
                    invcCount=recordsInvcCount[db.sm_invoice_head.sl.count()]
                    invcDateTime=recordsInvcCount[db.sm_invoice_head.order_datetime]
                    invcCheck=str(recordsInvcCount[db.sm_invoice_head.area_id])+"|"+str(invcDateTime).split(' ')[0]
                    invcChecklist.append(invcCheck)
                    invcCountList.append(invcCount)
                ...
                for recordsInvcAmount in recordsInvcAmount:
                    invcAmnt=recordsInvcAmount[((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum()]
                    invcDateTime=recordsInvcAmount[db.sm_invoice.order_datetime]
                    invcAmountCheck=str(recordsInvcAmount[db.sm_invoice.area_id])+"|"+str(invcDateTime).split(' ')[0]
                    invcAmntChecklist.append(invcAmountCheck)
                    invcAmntList.append(invcAmnt)
            ...
            sql_str="SELECT area_id, area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date =  '"+ str(date_from) +"'  "+ condition + " GROUP BY area_id,order_date order by  area_name asc;"
            # return sql_str
            records_ov=db.executesql(sql_str,as_dict = True)
            ...
            recordsV_Count = qsetVCount.select(db.sm_order_head.sl.count(),db.sm_order_head.area_id,db.sm_order_head.area_name,db.sm_order_head.order_date, orderby=db.sm_order_head.area_name, groupby=db.sm_order_head.area_id|db.sm_order_head.order_date)
            ...
            recordsO_Count = qsetOCount.select(db.sm_order_head.sl.count(),db.sm_order_head.area_id,db.sm_order_head.order_date, orderby=db.sm_order_head.level0_name, groupby=db.sm_order_head.area_id|db.sm_order.order_date)
            OChecklist=[]
            OCountList=[]
            for recordsO_Count in recordsO_Count:
                oCount=recordsO_Count[db.sm_order_head.sl.count()]
            ...
            recordsO_amount = qstOAmount.select(((db.sm_order.price) * ( db.sm_order.quantity )).sum(),db.sm_order.area_id,db.sm_order.order_date, orderby=db.sm_order.level0_name, groupby=db.sm_order.area_id|db.sm_order.order_date)
            ...
            recordsInvcCount = qstInvcCount.select(db.sm_invoice_head.sl.count(),db.sm_invoice_head.area_id,db.sm_invoice_head.order_datetime, orderby=db.sm_invoice_head.level0_name, groupby=db.sm_invoice_head.area_id|db.sm_invoice_head.order_date)
            ...
            recordsInvcAmount = qstInvcAmnt.select(((db.sm_invoice.price)*(db.sm_invoice.quantity)).sum(),db.sm_invoice.area_id,db.sm_invoice.level1_name,db.sm_invoice.order_datetime, orderby=db.sm_invoice.level0_name, groupby=db.sm_invoice.area_id|db.sm_invoice.order_date)
...
def sales_report_area_wise_url():
    ...
    session.se_market_report=se_market_report
    # session.order_date=order_date
    redirect(URL(c='sales_report_invoice',f='sales_report_area_wise'))
