    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            if not(session.from_dt=='' or session.from_dt==None or session.to_date=='' or session.to_date==None):
                ...
                sql_str="SELECT  level2_id,  level2_name,area_id,  area_name,order_date,order_datetime FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND order_date >= '"+ str(session.from_dt) +"' AND order_date <= '"+ str(session.to_date) +"' "+ condition + " GROUP BY level2_id,order_date order by order_date desc;"
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass
            if not(session.levelIdstr=='' or session.levelIdstr==None):
            
                sql_str="SELECT  level2_id,  level2_name,area_id,  area_name,order_date,order_datetime FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND  level2_id='"+ str(levelIdstr) +"' AND order_date >= '"+ str(session.from_dt) +"' AND order_date <= '"+ str(session.to_date) +"' GROUP BY area_id,order_date order by order_date desc;"
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass 
        else:
            sql_str="SELECT level2_id,  level2_name,area_id,  area_name, order_date ,order_datetime FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date  =  '"+ str(session.from_dt) +"' "+ condition + " GROUP BY level2_id,order_date order by order_date desc;"
            records_ov=db.executesql(sql_str,as_dict = True)
