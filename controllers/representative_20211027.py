...
def rep():
    ...
    if (session.btn_filter_rep):
        ...
        if (session.search_type_rep=='UserType'):
            if session.search_value_rep=='SURVEY': 
                session.search_value_rep='survey'
            qset=qset(db.sm_rep.user_type==session.search_value_rep)
        else:
...
#======================
    ...
    form =SQLFORM(db.sm_rep,
                  record=record,
                  deletable=True,
                  fields=['name','mobile_no','password','status'],
