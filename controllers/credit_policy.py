...
def approved_credit_validation(form):    
    ...
    clientId_rows_check=db((db.sm_client.cid==cid) & (db.sm_client.client_id==client_id)).select(db.sm_client.client_id,db.sm_client.depot_id,limitby=(0,1))
    if not clientId_rows_check:
        form.errors.client_id=''
        response.flash = 'Invalid Client Id '
    else:
        depot_id=clientId_rows_check[0].depot_id
...
def approved_credit_edit_validation(form):    
    ...
    clientId_rows_check=db((db.sm_client.cid==session.cid) & (db.sm_client.client_id==client_id)).select(db.sm_client.depot_id,limitby=(0,1))
    if not clientId_rows_check:
        form.errors.client_id=''
        response.flash = 'Invalid Client Id'
    else:
...
#    access_permission_view=check_role(task_id_view)
    ...
    if btn_upload=='Upload':        
        ...
        clientRows=db((db.sm_client.cid==c_id)&(db.sm_client.client_id.belongs(client_id_list_excel))&(db.sm_client.status=='ACTIVE')).select(db.sm_client.client_id,db.sm_client.name,db.sm_client.depot_id,orderby=db.sm_client.client_id)
                            ...
                            if dateFlag==False:
                                ...
                                if existCheckRows:
                                    ...
                                    if creditAmountExcel<=0:
                                        ...
                                        db.sm_cp_approved.insert(cid=c_id,approved_date=approved_dateExcel,branch_id=depot_id,client_id=clientIdExcel,credit_amount=creditAmountExcel,status=status)
                                        count_inserted+=1                                    
                    except:
