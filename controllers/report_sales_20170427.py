...
#           return btn_item_wise_sales_sheet
          ...
          if dateFlag==False:
              response.flash="Invalid Date "
          elif (teritory_id=='' and  mso_id=='' and customer_id=='' and fm_order=='' and btn_item_wise_sales_sheet!='Order Sheet' ):
...
#     return len(session.s_text)
    ...
    if len(session.s_textCteam) < 10:
        session.flash='Please Processed First'
        redirect (URL('report_sales','home'))
...
#     ================FmList
    ...
    fmInvStr="""select area_id,rep_id,rep_category from  z_mso_area where first_date ='"""+str(fDate)+"""' and cid <='"""+str(c_id)+"""' """+""" and area_id in ("""+str(sptrList)+""") GROUP BY area_id,rep_id,rep_category  ORDER BY  area_id,rep_id,rep_category;"""
#     return fmInvStr
...
# ===============================
    ...
    for inv in range(len(invRecords)):
        ...
        if (tr!=trPast):
            invStr=invStr+'<'+str(tr)+'>'
        invStr=invStr+str(recordInv['item_id'])+'fdfd'+str(recordInv['category_id'])+'fdfd'+str(recordInv['qty'])+'rdrd'
        trPast=str(recordInv['special_territory_code'])    
...
#     return fDate
#     fDate=str(first_currentDate)[0:10]
    fmInvStr="""select area_id,rep_id,rep_category from  z_mso_area where first_date ='"""+str(fDate)+"""' and cid <='"""+str(c_id)+"""' """+""" GROUP BY area_id,rep_id,rep_category  ORDER BY  area_id,rep_id,rep_category;"""
#     return fmInvStr
    fmInvRecords=db.executesql(fmInvStr,as_dict = True) 
