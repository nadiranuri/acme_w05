...
def salsCall_order_smry():
    ...
    if (user_type=='REP'):
        ...
        for i in range(len(records_ov)): 
            ...
            qset_invoice_str="SELECT count(sl) as inv_count FROM sm_invoice_head WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced' AND area_id = '"+str(area_id)+"' AND  order_datetime>='"+str(session.from_dt)+"' AND  order_datetime<='"+str(session.to_date)+"'"
            ...
            qset_inv_amount="SELECT SUM((sm_invoice.price) * ( sm_invoice.quantity ) ) AS inv_totalprice FROM sm_invoice WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced'  AND area_id = '"+str(area_id)+"'  AND  order_datetime>='"+str(session.from_dt)+"' AND  order_datetime<='"+str(session.to_date)+"'" 
        ...
        report_str=report_str+'<tr><td colspan="5" align="right">Total Amount</td><td colspan="1" align="right">'+str(gtotalprice).split('.')[0]+'</td></tr>'
