...
#     db.sm_client.thana_id.requires=IS_EMPTY_OR(IS_IN_DB(db(db.thana.cid == session.cid),db.thana.id,'%(district)s - %(name)s',orderby=db.thana.district|db.thana.name))
    ...
    form.vars.cid=cid
    depot_id=str(request.vars.depot_id)
    form.vars.depot_id=depot_id.split('|')[0]
    if session.user_type=='Depot':
...
def download_client():
    ...
    for rec in records:
        ...
        name=str(rec.sm_client.name).replace(',', ' ').replace('\n', ' ')
        area_id=str(rec.sm_client.area_id)
        depot_id=str(rec.sm_client.depot_id)
        depot_name=str(rec.sm_client.depot_name).replace(',', ' ').replace('\n', ' ')
        level_name=str(rec.sm_level.level_name).replace(',', ' ').replace('\n', ' ')
        level0=str(rec.sm_level.level0)  
        
        store_id=str(rec.sm_client.store_id)
        store_name=str(rec.sm_client.store_name).replace(',', ' ').replace('\n', ' ')
        ...
        depot_belt_name=str(rec.sm_client.depot_belt_name).replace(',', ' ').replace('\n', ' ')
        owner_name=str(rec.sm_client.owner_name).replace(',', ' ').replace('\n', ' ')
        contact_no1=str(rec.sm_client.contact_no1)
        contact_no2=str(rec.sm_client.contact_no2)
        address=str(rec.sm_client.address).replace(',', ' ').replace('\n', ' ')
        
        market_id=str(rec.sm_client.market_id)
        market_name=str(rec.sm_client.market_name).replace(',', ' ').replace('\n', ' ')
