...
def index():
    ...
    btn_tourVSummary=request.vars.btn_tourVSummary
    btn_tourVSummaryD=request.vars.btn_tourVSummaryD
    
    btn_tourDateVSummary=request.vars.btn_tourDateVSummary
    btn_tourDateVSummaryD=request.vars.btn_tourDateVSummaryD
    
    dcr_day_count=request.vars.dcr_day_count
#     return btn_dcrMsoD
    if (btn_dcrRegion or btn_dcrRegionD or btn_dcrFm or btn_dcrFmD or btn_dcrTeritory or btn_dcrMso or btn_dcrMsoD or btn_dcrVisit or btn_dcrTeritoryD or btn_dcrDoc or btn_dcrDocD or btn_dcrSummary or btn_dcrSummaryD or btn_dcrVSummary or btn_dcrVSummaryD or btn_dcrVSummarProduct or btn_dcrVSummarProductD or btn_prSummary or btn_prSummaryD or btn_tourSummary or btn_tourSummaryD or btn_tourVSummary or btn_tourVSummaryD or btn_tourDateVSummary or btn_tourDateVSummaryD or dcr_day_count):
...
#         return    dateFlag  
        if dateFlag!=False:
            ...
            if btn_tourVSummary:
                redirect (URL('tourVSummary',vars=dict(date_from=date_from,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,product_id=product_id,product_name=product_name,brand=brand,category=category,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)))
            if btn_tourVSummaryD:
                redirect (URL('tourVSummaryD',vars=dict(date_from=date_from,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,product_id=product_id,product_name=product_name,brand=brand,category=category,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)))
            if btn_tourDateVSummary:     
                redirect (URL('tourDateVSummary',vars=dict(date_from=date_from,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,product_id=product_id,product_name=product_name,brand=brand,category=category,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)))
            if btn_tourDateVSummaryD:     
                redirect (URL('tourDateVSummaryD',vars=dict(date_from=date_from,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,product_id=product_id,product_name=product_name,brand=brand,category=category,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)))
            if dcr_day_count:
...
#     return mso_id
    date_to_m=datetime.datetime.strptime(str(date_to),'%Y-%m-%d') 
    date_first = datetime.datetime.strptime(str(date_to_m)[0:7] + '-01', '%Y-%m-%d')
    date_first_month= str(date_first)[0:10]
#     return date_first_month
#     date_to_m=date_to_m + datetime.timedelta(days = 1) 
...
#     return date_from
    ...
    qset = qset(db.sm_doctor_visit_plan.status != 'CReq')
    qset = qset(db.sm_doctor_visit_plan.route_id == db.sm_microunion.microunion_id)
    qset = qset(db.sm_microunion.level0 != '')
    qset = qset(db.sm_doctor_visit_plan.first_date  == date_first_month)
#     qset = qset(db.sm_doctor_visit_plan.schedule_date     <= date_to)
    ...
    records = qset.select(db.sm_microunion.level0,db.sm_microunion.level0_name,db.sm_microunion.level1,db.sm_microunion.level1_name,db.sm_microunion.level2,db.sm_microunion.level2_name,db.sm_microunion.area_id,db.sm_microunion.area_name,db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_id,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.status,db.sm_doctor_visit_plan.schedule_date, orderby=db.sm_microunion.level0_name|db.sm_microunion.level1_name|db.sm_microunion.level2_name|db.sm_microunion.area_name|db.sm_doctor_visit_plan.rep_name,groupby= db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.first_date)
#     return records
    return dict(records=records,date_from=date_first_month,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,brand=brand,category=category,product_id=product_id,product_name=product_name,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)
...
def tourVSummary():
    ...
    c_id = session.cid
    response.title='Tour Visit Summary'    
    date_from=request.vars.date_from
    date_to=request.vars.date_to
    depot_id=str(request.vars.depot_id).strip()
    depot_name=str(request.vars.depot_name).strip()    
...
#     return mso_id
    ...
    date_first = datetime.datetime.strptime(str(date_to_m)[0:7] + '-01', '%Y-%m-%d')
    date_first_month= str(date_first)[0:10]
    getMonth=str(date_first_month)[5:7]
    nextMonthG=int(getMonth)+1
    nextMonth=str(nextMonthG)
    if int(nextMonthG)<10:
        nextMonth='0'+str(nextMonthG)
#     return nextMonth
    date_to_m=str(date_first_month)[0:5]+str(nextMonth)+ '-01'
#     return date_to_m
#     return date_to_m
#     return len(mso_id)
#     if ((len(rsm_id)<1) and (fm_id<1) and (tr_id<1) and (doc<1) and (product_id<1) and (mso_id<1)):
...
#     if (tr_id!=''):
#         qset = qset(db.sm_microunion.area_id == tr_id)
#     records = qset.select(db.sm_microunion.level0,db.sm_microunion.level0_name,db.sm_microunion.level1,db.sm_microunion.level1_name,db.sm_microunion.level2,db.sm_microunion.level2_name,db.sm_microunion.area_id,db.sm_microunion.area_name,db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_id,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.status, orderby=db.sm_microunion.level0_name|db.sm_microunion.level1_name|db.sm_microunion.level2_name|db.sm_microunion.area_name|db.sm_doctor_visit_plan.rep_name|db.sm_doctor_visit_plan.schedule_date,groupby= db.sm_microunion.level0|db.sm_microunion.level1|db.sm_microunion.level2|db.sm_microunion.area_id|db.sm_doctor_visit_plan.rep_id)

    qset=db()
    qset = qset(db.sm_rep_area.cid == c_id)
    qset = qset(db.sm_level.cid == db.sm_rep_area.cid)
    qset = qset(db.sm_rep_area.area_id == db.sm_level.level_id)
    qset = qset(db.sm_level.is_leaf == '1')

    if (rsm_id!=''):
        qset = qset(db.sm_level.level1 == rsm_id)
#     if (fm_id!=''):
#         qset = qset(db.sm_level.level2 == fm_id)
#     if (tr_id!=''):
#         qset = qset(db.sm_level.level_id == tr_id)
    records = qset.select(db.sm_level.level0,db.sm_level.level0_name,db.sm_level.level1,db.sm_level.level1_name,db.sm_level.level2,db.sm_level.level2_name,db.sm_rep_area.area_id,db.sm_rep_area.area_name,db.sm_rep_area.rep_id,db.sm_rep_area.rep_name, orderby=db.sm_level.level0|db.sm_level.level0_name|db.sm_level.level1|db.sm_level.level1_name|db.sm_level.level2|db.sm_level.level2_name|db.sm_rep_area.area_id|db.sm_rep_area.area_name|db.sm_rep_area.rep_name|db.sm_rep_area.rep_id,groupby= db.sm_level.level0|db.sm_level.level0_name|db.sm_level.level1|db.sm_level.level1_name|db.sm_level.level2|db.sm_level.level2_name|db.sm_rep_area.area_id|db.sm_rep_area.area_name|db.sm_rep_area.rep_name|db.sm_rep_area.rep_id)
#     return records
#     ===============================visit Count=========
    qsetCount=db()

    qsetCount = qsetCount(db.sm_doctor_visit.cid == c_id)

    qsetCount = qsetCount(db.sm_doctor_visit.visit_date >= date_first_month)
    qsetCount = qsetCount(db.sm_doctor_visit.visit_date < date_to_m)

    if (rsm_id!=''):
        qsetCount = qsetCount(db.sm_doctor_visit.level1_id == rsm_id)
#     if (fm_id!=''):
#         qsetCount = qsetCount(db.sm_doctor_visit.level2_id == fm_id)
#     if (tr_id!=''):
#         qsetCount = qsetCount(db.sm_doctor_visit.route_id == tr_id)
    recordsCount = qsetCount.select(db.sm_doctor_visit.id.count(),db.sm_doctor_visit.rep_id,db.sm_doctor_visit.visit_date,db.sm_doctor_visit.level0_id,db.sm_doctor_visit.level0_name,db.sm_doctor_visit.level1_id,db.sm_doctor_visit.level1_name,db.sm_doctor_visit.level2_id,db.sm_doctor_visit.level2_name,db.sm_doctor_visit.level3_id,db.sm_doctor_visit.level3_name, orderby=db.sm_doctor_visit.level0_id|db.sm_doctor_visit.level1_id|db.sm_doctor_visit.level2_id|db.sm_doctor_visit.level3_id|db.sm_doctor_visit.rep_id,groupby= db.sm_doctor_visit.rep_id)
#     return db._lastsql
    vChecklist=[]
    vCountList=[]
    for recordsCount in recordsCount:
        vCount=recordsCount[db.sm_doctor_visit.id.count()]
        vCheck=str(recordsCount[db.sm_doctor_visit.rep_id])
        vChecklist.append(vCheck)
        vCountList.append(vCount)
# ==========Prescription=======
    qsetCountP=db()
 
    qsetCountP = qsetCountP(db.sm_prescription_head.cid == c_id)
    qsetCountP = qsetCountP(db.sm_prescription_head.submit_date >= date_first_month)
    qsetCountP = qsetCountP(db.sm_prescription_head.submit_date < date_to_m)
 
    if (rsm_id!=''):
        qsetCountP = qsetCountP(db.sm_prescription_head.reg_id == rsm_id)
#     if (fm_id!=''):
#         qsetCountP = qsetCountP(db.sm_prescription_head.tl_id == fm_id)
#     if (tr_id!=''):
#         qsetCountP = qsetCountP(db.sm_prescription_head.area_id == tr_id)
    recordsCountP = qsetCountP.select(db.sm_prescription_head.id.count(),db.sm_prescription_head.submit_by_id,db.sm_prescription_head.reg_id,db.sm_prescription_head.tl_id,db.sm_prescription_head.area_id, orderby=db.sm_prescription_head.reg_id|db.sm_prescription_head.tl_id|db.sm_prescription_head.area_id|db.sm_prescription_head.submit_by_id,groupby= db.sm_prescription_head.submit_by_id)
#     return db._lastsql
    vChecklistP=[]
    vCountListP=[]
    for recordsCountP in recordsCountP:
        vCountP=recordsCountP[db.sm_prescription_head.id.count()]
        vCheckP=str(recordsCountP[db.sm_prescription_head.submit_by_id])
#         vCheckP=str(recordsCountP[db.sm_prescription_head.area_id])+'|'+str(recordsCountP[db.sm_prescription_head.submit_by_id])
        vChecklistP.append(vCheckP)
        vCountListP.append(vCountP)
#         
# # ========Tour
    qsetT=db()
    qset = qsetT(db.sm_doctor_visit_plan.cid == c_id)
    qsetT = qsetT(db.sm_microunion.cid == c_id)
    qsetT = qsetT(db.sm_doctor_visit_plan.status != 'CReq')
#     qsetT = qsetT(db.sm_doctor_visit_plan.route_id == db.sm_microunion.microunion_id)
    qsetT = qsetT(db.sm_doctor_visit_plan.first_date  == date_first_month)
 
    if (rsm_id!=''):
        qsetT = qsetT(db.sm_microunion.level1 == rsm_id)
#     if (fm_id!=''):
#         qsetT = qsetT(db.sm_microunion.level2 == fm_id)
#     if (tr_id!=''):
#         qsetT = qsetT(db.sm_microunion.area_id == tr_id)
    recordsT = qsetT.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.status, orderby=db.sm_microunion.level0_name|db.sm_microunion.level1_name|db.sm_microunion.level2_name|db.sm_microunion.area_name|db.sm_doctor_visit_plan.rep_name,groupby= db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.first_date)
    vChecklistT=[]
    vCountListT=[]
    for recordsT in recordsT:
        vCountT=recordsT[db.sm_doctor_visit_plan.status]
        vCheckT=str(recordsT[db.sm_doctor_visit_plan.rep_id])
        if recordsT[db.sm_doctor_visit_plan.status]=='Confirmed': 
            vCountT='Approved'  
        if recordsT[db.sm_doctor_visit_plan.status]!='Confirmed' and recordsT[db.sm_doctor_visit_plan.status]!='Cancelled': 
            vCountT='Submitted'
            
        vChecklistT.append(vCheckT)
        vCountListT.append(vCountT)
#     return dict(records=records,vChecklistT=vChecklistT,vCountListT=vCountListT,vChecklistP=vChecklistP,vCountListP=vCountListP,vChecklist=vChecklist,vCountList=vCountList,date_from=date_first_month,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,brand=brand,category=category,product_id=product_id,product_name=product_name,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)
    return dict(records=records,vChecklistT=vChecklistT,vCountListT=vCountListT,vChecklistP=vChecklistP,vCountListP=vCountListP,vChecklist=vChecklist,vCountList=vCountList,date_from=date_first_month,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,brand=brand,category=category,product_id=product_id,product_name=product_name,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)
...
#         qsetT = qsetT(db.sm_microunion.area_id == tr_id)
    ...
    month=''
    if int(date_from.split('-')[1])==1:
       month='January'
    elif int(date_from.split('-')[1])==2:
       month='February'
    elif int(date_from.split('-')[1])==3:
       month='March'
    elif int(date_from.split('-')[1])==4:
       month='April'
    elif int(date_from.split('-')[1])==5:
       month='May'
    elif int(date_from.split('-')[1])==6:
       month='June'
    elif int(date_from.split('-')[1])==7:
       month='June'
    elif int(date_from.split('-')[1])==8:
       month='June'
    elif int(date_from.split('-')[1])==9:
       month='June'
    elif int(date_from.split('-')[1])==10:
       month='June'
    elif int(date_from.split('-')[1])==11:
       month='June'
    elif int(date_from.split('-')[1])==12:
       month='December'
    
    myString='Month,'+month+' '+date_from.split('-')[0]+'\n'
    myString+='RSM,'+rsm_id+'|'+rsm_name+'\n'
    

        
    myString+='ASMBS_CD  ,   ASBS_NM  ,  RMBS_CD   , RMBS_NM ,   FMBS_CD  ,  FMBS_NM   , FPBS_CD ,   FPBS_NM ,   FP_ID  ,  FP_NM  ,  DCRCount ,   RXCount  ,  TouRSubmit\n'
    
    
    for record in records:
        vCount=0
        Vcheck=str(record[db.sm_rep_area.rep_id])
        if [s for s in vChecklist if Vcheck in s]:
           index_element = vChecklist.index(Vcheck)           
           vCount=vCountList[index_element]
        
        vCountP=0
        
        if [s for s in vChecklistP if Vcheck in s]:
           index_element = vChecklistP.index(Vcheck)           
           vCountP=vCountListP[index_element]

        status='No'
        statusCheck=record[db.sm_rep_area.rep_id]
        if [s for s in vChecklistT if statusCheck in s]:
           index_element = vChecklistT.index(statusCheck)           
           status=vCountListT[index_element]
        
        
        
        myString+=str(record[db.sm_level.level0])+','+str(record[db.sm_level.level0_name])+','+str(record[db.sm_level.level1])+','+str(record[db.sm_level.level1_name])+','+str(record[db.sm_level.level2])+','+str(record[db.sm_level.level2_name])+','+str(record[db.sm_rep_area.area_id])+','+str(record[db.sm_rep_area.area_name])+','+str(record[db.sm_rep_area.rep_id])+','+str(record[db.sm_rep_area.rep_name])+','+str(vCount)+','+str(vCountP)+','+str(status)+'\n'
         
    #Save as csv
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=Compliance.csv'   
    return str(myString)


# ===========Date Range================

def tourDateVSummary():
    task_id = 'rm_doctor_visit_manage'
    task_id_view = 'rm_doctor_visit_view'
    access_permission = check_role(task_id)
    access_permission_view = check_role(task_id_view)
...
#     if (tr_id!=''):
#         qset = qset(db.sm_level.level_id == tr_id)
    records = qset.select(db.sm_level.level0,db.sm_level.level0_name,db.sm_level.level1,db.sm_level.level1_name,db.sm_level.level2,db.sm_level.level2_name,db.sm_rep_area.area_id,db.sm_rep_area.area_name,db.sm_rep_area.rep_id,db.sm_rep_area.rep_name,db.sm_rep_area.rep_category, orderby=db.sm_level.level0|db.sm_level.level0_name|db.sm_level.level1|db.sm_level.level1_name|db.sm_level.level2|db.sm_level.level2_name|db.sm_rep_area.area_id|db.sm_rep_area.area_name|db.sm_rep_area.rep_name|db.sm_rep_area.rep_id,groupby= db.sm_level.level0|db.sm_level.level0_name|db.sm_level.level1|db.sm_level.level1_name|db.sm_level.level2|db.sm_level.level2_name|db.sm_rep_area.area_id|db.sm_rep_area.area_name|db.sm_rep_area.rep_name|db.sm_rep_area.rep_id)
#     return records
#     ===============================visit Count=========
    qsetCount=db()
...
#     if (tr_id!=''):
#         qsetCount = qsetCount(db.sm_doctor_visit.route_id == tr_id)
    recordsCount = qsetCount.select(db.sm_doctor_visit.id.count(),db.sm_doctor_visit.rep_id,db.sm_doctor_visit.visit_date,db.sm_doctor_visit.level0_id,db.sm_doctor_visit.level0_name,db.sm_doctor_visit.level1_id,db.sm_doctor_visit.level1_name,db.sm_doctor_visit.level2_id,db.sm_doctor_visit.level2_name,db.sm_doctor_visit.level3_id,db.sm_doctor_visit.level3_name, orderby=db.sm_doctor_visit.level0_id|db.sm_doctor_visit.level1_id|db.sm_doctor_visit.level2_id|db.sm_doctor_visit.level3_id|db.sm_doctor_visit.rep_id,groupby= db.sm_doctor_visit.level0_id|db.sm_doctor_visit.level1_id|db.sm_doctor_visit.level2_id|db.sm_doctor_visit.level3_id|db.sm_doctor_visit.rep_id)
#     return recordsCount
    vChecklist=[]
    vCountList=[]
    for recordsCount in recordsCount:
        vCount=recordsCount[db.sm_doctor_visit.id.count()]
        vCheck=str(recordsCount[db.sm_doctor_visit.level3_id])+'|'+str(recordsCount[db.sm_doctor_visit.rep_id])
        vChecklist.append(vCheck)
        vCountList.append(vCount)
# ==========Prescription=======
    qsetCountP=db()
    ...
    qsetCountP = qsetCountP(db.sm_prescription_head.cid == c_id)
    qsetCountP = qsetCountP(db.sm_prescription_head.submit_date >= date_from)
    qsetCountP = qsetCountP(db.sm_prescription_head.submit_date < date_to_m)
...
#         
    ...
    return dict(records=records,vChecklistP=vChecklistP,vCountListP=vCountListP,vChecklist=vChecklist,vCountList=vCountList,date_from=date_from,date_to=date_to,depot_id=depot_id,depot_name=depot_name,rsm_id=rsm_id,rsm_name=rsm_name,fm_id=fm_id,fm_name=fm_name,tr_id=tr_id,tr_name=tr_name,brand=brand,category=category,product_id=product_id,product_name=product_name,mso_id=mso_id,mso_name=mso_name,doc=doc,doc_name=doc_name)


def tourDateVSummaryD():
    task_id = 'rm_doctor_visit_manage'
    task_id_view = 'rm_doctor_visit_view'
    access_permission = check_role(task_id)
    access_permission_view = check_role(task_id_view)
...
#     ===============================visit Count=========
    ...
    qsetCount = qsetCount(db.sm_doctor_visit.visit_date >= date_to)
    qsetCount = qsetCount(db.sm_doctor_visit.visit_date < date_to_m)
    if (rsm_id!=''):
        qsetCount = qsetCount(db.sm_doctor_visit.level1_id == rsm_id)
#     if (fm_id!=''):
...
# ==========Prescription=======
    ...
    qsetCountP = qsetCountP(db.sm_prescription_head.cid == c_id)
    qsetCountP = qsetCountP(db.sm_prescription_head.submit_date >= date_from)    
    qsetCountP = qsetCountP(db.sm_prescription_head.submit_date <= date_to)
...
#         
    ...
    myString='Date From,'+str(date_from)+' To '+str(date_to)+'\n'
    myString+='RSM,'+rsm_id+'|'+rsm_name+'\n'
    

        
    myString+='ASMBS_CD  ,   ASBS_NM  ,  RMBS_CD   , RMBS_NM ,   FMBS_CD  ,  FMBS_NM   , FPBS_CD ,   FPBS_NM ,   FP_ID  ,  FP_NM  ,  FP_CATEGORY  ,  DCRCount ,   RXCount  \n'
    
    
    for record in records:
        vCount=0
        Vcheck=str(record[db.sm_rep_area.area_id])+'|'+str(record[db.sm_rep_area.rep_id])
        if [s for s in vChecklist if Vcheck in s]:
           index_element = vChecklist.index(Vcheck)           
           vCount=vCountList[index_element]
        ...
        myString+=str(record[db.sm_level.level0])+','+str(record[db.sm_level.level0_name])+','+str(record[db.sm_level.level1])+','+str(record[db.sm_level.level1_name])+','+str(record[db.sm_level.level2])+','+str(record[db.sm_level.level2_name])+','+str(record[db.sm_rep_area.area_id])+','+str(record[db.sm_rep_area.area_name])+',`'+str(record[db.sm_rep_area.rep_id])+','+str(record[db.sm_rep_area.rep_name])+','+str(record[db.sm_rep_area.rep_category])+','+str(vCount)+','+str(vCountP)+'\n'
         
    #Save as csv
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=ComplianceDateRanges.csv'   
    return str(myString)
...
#         qset = qset(db.sm_prescription_details.medicine_id == product_id)
    ...
    records = qset.select(db.sm_doctor_visit.doc_id,db.sm_doctor_visit.doc_name,db.sm_doctor_visit.visit_date, orderby=db.sm_doctor_visit.visit_date|db.sm_doctor_visit.doc_id,groupby= db.sm_doctor_visit.visit_date|db.sm_doctor_visit.doc_id)
    
    
    myString='DateRange,'+date_from+','+date_to+'\n'
    myString+='MSO,'+mso_id+'|'+mso_name+'\n\n'
    
    myString+='doctorID  ,  doctorName  ,  Date  ,  Visit\n'
    for record in records:
