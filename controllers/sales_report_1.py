...
def salesDcr_report_detail():
    ...
    if (user_type=='REP'):
        ...
        records = qset.select(db.sm_doctor_visit.doc_id.count(),db.sm_doctor_visit.doc_id,db.sm_doctor_visit.route_id,db.sm_doctor_visit.route_name,db.sm_doctor_visit.visit_date, orderby=db.sm_doctor_visit.visit_date, groupby=db.sm_doctor_visit.route_id|db.sm_doctor_visit.visit_date)
        doc_id_list=[]
        visit_date_list=[]
        if records:
        ...
        recordsCount = qsetCount.select(db.sm_doctor_visit.doc_id.count(),db.sm_doctor_visit.doc_id,db.sm_doctor_visit.route_id,db.sm_doctor_visit.route_name,db.sm_doctor_visit.visit_date, orderby=db.sm_doctor_visit.visit_date, groupby=db.sm_doctor_visit.route_id|db.sm_doctor_visit.visit_date)
        vChecklist=[]
        vCountList=[]
        for recordsCount in recordsCount:
            vCount=recordsCount[db.sm_doctor_visit.doc_id.count()]
            vCheck=str(recordsCount[db.sm_doctor_visit.doc_id])+'|'+str(recordsCount[db.sm_doctor_visit.visit_date])
        ...
        #         records_ov=[]
        #         sql_str="SELECT  COUNT(s1.doc_id)as doc_count,s1.route_id as route_id, s1.route_name as route_name,s1.visit_date as visit_date,(SELECT  COUNT(s2.doc_id) FROM sm_doctor_visit as s2 WHERE s2.cid = '"+ str(cid) +"' AND s2.rep_id = '"+ str(rep_id) +"' AND s2.giftnsample!='' AND s2.visit_date=s1.visit_date "+ condition2 + " GROUP BY s2.route_id) as scsdoc_count FROM sm_doctor_visit as s1 WHERE s1.cid = '"+ str(cid) +"' AND s1.rep_id = '"+ str(rep_id) +"' AND s1.visit_date > '"+ str(from_dt) +"' AND s1.visit_date <= '"+ str(to_date) +"' "+ condition + " GROUP BY s1.route_id,s1.visit_date;"
        #         records_ov=db.executesql(sql_str,as_dict = True)
        ...
        #     sql_str="SELECT  COUNT(s1.doc_id)as doc_count,s1.route_id as route_id, s1.route_name as route_name,s1.visit_date as visit_date,(SELECT  COUNT(s2.doc_id) FROM sm_doctor_visit as s2 WHERE s2.cid = '"+ str(cid) +"' AND s2.rep_id = '"+ str(rep_id) +"' AND s2.giftnsample!='' AND s2.visit_date=s1.visit_date "+ condition2 + " GROUP BY s2.route_id) as scsdoc_count FROM sm_doctor_visit as s1  WHERE s1.cid = '"+ str(cid) +"' AND s1.rep_id = '"+ str(rep_id) +"' AND s1.visit_date <= '"+ str(date_from) +"' AND s1.visit_date > '"+ str(date_to) +"' "+ condition + " GROUP BY s1.route_id,s1.visit_date;"
        #     records_ov=db.executesql(sql_str,as_dict = True)
...
#             return level
        ...
        condition=condition+" AND s1."+str(level)+" IN ("+str(levelStr)+")"  
