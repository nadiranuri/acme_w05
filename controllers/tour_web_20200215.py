...
#         if ((status=='Done') or (status=='Submitted')):
        docStr='Doctor'
        clStr='Client'
...
#                 sDate=str(draft_infoAll.schedule_date)
        ...
        strValueMorning=strValueMorning+'<'+sDate+'>'+route_name+' '+docStr+' '+clStr
#         else:     
...
#                 sDate=str(draft_infoAll.schedule_date)
#             return sDate        
        strValueMorningE=strValueMorningE+'<'+sDate+'>'+route_name+' '+docStrE+' '+clStrE
...
#                     route_id=route_nameG.split('|')[0]
                    ...
                    ins_dict = {'cid':cid, 'rep_id':rep_id,'rep_name':rep_name, 'first_date':first_date, 'schedule_date':schedule_date, 'route_id':route_id, 'route_name':route_name, 'note':'Morning','status':'Done','field2':session.userdepth}
...
#                     route_id=route_nameG.split('|')[0]
                    ...
                    ins_dict = {'cid':cid, 'rep_id':rep_id,'rep_name':rep_name, 'first_date':first_date, 'schedule_date':schedule_date, 'route_id':route_id, 'route_name':route_name, 'note':'Evening','status':'Done','field2':session.userdepth}
                    tourArrayList.append(ins_dict)
                m=m+1
...
#         return rowDoctor
        ...
        for rowClient in rowClient:
            clInfo = str(rowClient.client_name)+'-'+str(rowClient.client_id) 
            clStr=clStr+clInfo+','
        routeShow=routeName+'|'+route_id+' <d>'+docStr+' '+clStr
...
#     return strThisMorning
    ...
    for tourThisEvening in tourThisEvening:
        ...
        docStrE='Doctor'
        clStrE='Client'
        rowDoctor = db((db.sm_doctor_day_plan.cid==cid)  &(db.sm_doctor_day_plan.rep_id==rep_id) & (db.sm_doctor_day_plan.area_id==route_id) & (db.sm_doctor_day_plan.plan_date==sDate)  &(db.sm_doctor_day_plan.visit_time=='Evening') ).select(db.sm_doctor_day_plan.doc_name,db.sm_doctor_day_plan.doc_id, orderby=db.sm_doctor_day_plan.doc_name)
#             return db._lastsql
        for rowDoctor in rowDoctor:
            docInfo = str(rowDoctor.doc_name)+'-'+str(rowDoctor.doc_id) 
...
#                 sDate=str(draft_infoAll.schedule_date)
        ...
        rowClient = db((db.sm_client_day_plan.cid==cid)  &(db.sm_client_day_plan.rep_id==rep_id) & (db.sm_client_day_plan.area_id==route_id) & (db.sm_client_day_plan.plan_date==sDate)  &(db.sm_client_day_plan.visit_time=='Evening') ).select(db.sm_client_day_plan.client_name,db.sm_client_day_plan.client_id, orderby=db.sm_client_day_plan.client_name)
        for rowClient in rowClient:
            clInfo = str(rowClient.client_name)+'-'+str(rowClient.client_id) 
            clStrE=clStrE+clInfo+','
        
        routeShow=routeName+'|'+route_id+' -'+docStr+' '+clStr
            
        if s_date!=s_datePast:
            strThisEvening=strThisEvening+'<rd>'+s_date+' -'+docStr+' '+clStr
...
#     return cid
    tourThisMorning = db( (db.sm_doctor_visit_plan.rep_id == rep_id) & (db.sm_doctor_visit_plan.first_date == DateTour)& (db.sm_doctor_visit_plan.note == 'Morning') & ((db.sm_doctor_visit_plan.status == 'Confirmed') | (db.sm_doctor_visit_plan.status == 'Submitted'))).select(db.sm_doctor_visit_plan.ALL,orderby=db.sm_doctor_visit_plan.schedule_date)
    tourThisEvening = db( (db.sm_doctor_visit_plan.rep_id == rep_id) & (db.sm_doctor_visit_plan.first_date == DateTour)& (db.sm_doctor_visit_plan.note == 'Evening') & ((db.sm_doctor_visit_plan.status == 'Confirmed') | (db.sm_doctor_visit_plan.status == 'Submitted'))).select(db.sm_doctor_visit_plan.ALL,orderby=db.sm_doctor_visit_plan.schedule_date)
#     return tourThisMorning
#     return tourThisMorning
#     return DateTour
#     tourNext = db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan.rep_id == rep_id) & (db.sm_doctor_visit_plan.first_date == DateTourNext) ).select(db.sm_doctor_visit_plan.ALL,orderby=db.sm_doctor_visit_plan.schedule_date)
...
#     return 'assf'
    ...
    submitStrMorning=request.vars.submitStrMorning
    submitStrEvening=request.vars.submitStrEvening
