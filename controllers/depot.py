...
def validation_requisition(form):#Validation for depot_stock_requisition
    ...
    if depot_id=='' or depot_id==None:
        ...
        if depot_id == requisition_to:
            form.errors.quantity = ''
            response.flash = 'Select Valid Requisition To'
        else:
...
#========================= Date filter start ============
        ...
        if not(from_dt=='' or to_dt==''):
            try:
                ...
                date_to_m=datetime.datetime.strptime(str(to_dt),'%Y-%m-%d')
    ...
    if (session.btn_filter_issue):
        if not (session.fromDate_v==None or session.date_to_m==None):  
            qset=qset((db.sm_issue_head.issue_date >=session.fromDate_v)&(db.sm_issue_head.issue_date <= session.date_to_m))
    ...
    #-------------
    if not(session.fromDate_v==None or session.date_to_m==None):
        search_form.vars.from_dt=session.fromDate_v
        search_form.vars.to_dt=session.date_to_m
    else:
                    ...
                    if (req_to!='' and issue_date!='' and ref_sl > 0):
                        ...
                        if reqRecords:
                            ...
                            for row in reqRecords:
                                ...
                                #=======
                                batch_id=''
                                itemBatchRows = db((db.sm_item_batch.cid == c_id) & (db.sm_item_batch.item_id == item_id) & (db.sm_item_batch.expiary_date >= current_date)).select(db.sm_item_batch.item_id,db.sm_item_batch.batch_id, db.sm_item_batch.expiary_date, limitby=(0, 1))
                                if itemBatchRows:
                                    batch_id=itemBatchRows[0].batch_id
                                    expiary_date = itemBatchRows[0].expiary_date
                                #------------
                                reqDict={'cid':c_id,'depot_id':requisition_to,'depot_name':depot_name,'sl':maxSl,'store_id':store_id,'store_name':store_name,'issued_to':from_depot,'depot_to_name':depot_to_name,'issue_date':issue_date,'total_discount':total_discount,'req_sl':ref_sl,'transaction_cause':transaction_cause,
                                         'item_id':item_id,'item_name':item_name,'batch_id':batch_id,'quantity':quantity,'dist_rate':dist_rate,'item_unit':item_unit,'item_carton':item_carton,'expiary_date':expiary_date,'short_note':short_note,'ym_date':ym_date}
...
#             break
    ...
    issue_item_id_list=[]
    records1=db((db.sm_issue.cid==c_id)& (db.sm_issue.depot_id==depot_id) & (db.sm_issue.sl==req_sl)).select(db.sm_issue.item_id,orderby=db.sm_issue.item_id)
    for rec1 in records1:
        item_id1=str(rec1.item_id).strip()
        issue_item_id_list.append(item_id1)
    
    stockRecords = db((db.sm_depot_stock_balance.cid == c_id) & (db.sm_depot_stock_balance.depot_id == depot_id) & (db.sm_depot_stock_balance.store_id == store_id) & (db.sm_depot_stock_balance.item_id.belongs(issue_item_id_list))).select(db.sm_depot_stock_balance.item_id,db.sm_depot_stock_balance.quantity,db.sm_depot_stock_balance.block_qty, orderby=db.sm_depot_stock_balance.item_id)
    ...
    #--------------------
    return dict(form=form,reqRecords=reqRecords,records=records,stockRecords=stockRecords,storeRecords=storeRecords,depot_id=depot_id,depot_name=depot_name,store_id=store_id,store_name=store_name,sl=sl,ref_sl=ref_sl,issued_to=issued_to,depot_to_name=depot_to_name,issue_date=issue_date,status=status,issue_process_status=issue_process_status,note=note,field2=field2,
...
def show_pending_requisition():
    ...
    # Set text for filter
    btn_filter_rq_pending = request.vars.btn_filter
    btn_all = request.vars.btn_all
    ...
    if btn_filter_rq_pending:
        session.btn_filter_rq_pending = btn_filter_rq_pending
        session.date_from = date_from
        session.date_to = date_to
        session.depot_id_value = depot_id_value

    elif btn_all:
        session.btn_filter_rq_pending = None
        session.date_from = None
        session.date_to = None
        session.depot_id_value = None

    if session.date_from == 'None' or session.date_from == None:
        session.date_from=''

    if session.date_to == 'None' or session.date_to == None:
        session.date_to=''

    if session.depot_id_value == 'None' or session.depot_id_value == None:
        session.depot_id_value=''

    qset=db()
    qset=qset(db.sm_requisition_head.cid==session.cid)
    qset = qset(db.sm_requisition_head.requisition_to==depotId)
    qset = qset(db.sm_requisition_head.status=='Posted')
    qset = qset(db.sm_requisition_head.req_process_status=='Requisition')

    if session.btn_filter_rq_pending:
        if session.date_from!='' and session.date_to!='':
            qset = qset((db.sm_requisition_head.req_date >= session.date_from)&(db.sm_requisition_head.req_date <= session.date_to))

        if session.depot_id_value!='':
            qset = qset(db.sm_requisition_head.depot_id == str(session.depot_id_value).split('|')[0])

    records=qset.select(db.sm_requisition_head.ALL,orderby=db.sm_requisition_head.depot_id|~db.sm_requisition_head.sl)
    #return db._lastsql
    return dict(records=records)
...
#        try:
    ...
    #            if (not(receive_from=='' or depot_category=='EXTERNAL') and receive_date!='' and ref_sl > 0):
                if (not(receive_from=='') and receive_date!='' and ref_sl > 0):
                    ...
                    recRows=db((db.sm_receive.cid==c_id)& (db.sm_receive.depot_id==req_depot_id) & (db.sm_receive.receive_from==receive_from)& (db.sm_receive.ref_sl==ref_sl) & (db.sm_receive.status!='Cancelled')).select(db.sm_receive.ref_sl,limitby=(0,1))
                    if not recRows:
                        reqRecords=db((db.sm_issue.cid==c_id) & (db.sm_issue.issued_to==req_depot_id) & (db.sm_issue.depot_id==receive_from)&(db.sm_issue.sl==ref_sl)&(db.sm_issue.status=='Posted') & ((db.sm_issue.issue_process_status=='Issued')|(db.sm_issue.issue_process_status=='Received'))).select(db.sm_issue.ALL,orderby=db.sm_issue.item_name)
