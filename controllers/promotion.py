...
def flat_rate():
    ...
    form =SQLFORM(db.sm_promo_flat_rate,
                  fields=['from_date','to_date','campaign_ref','product_id','product_name','min_qty','flat_rate','allowed_credit_inv','status'],       #'regular_discount_apply','allow_bundle',
                  submit_button='Save'
                  )
     
    form.vars.cid=cid
    form.vars.regular_discount_apply='NO'
    form.vars.allow_bundle='NO'
    #all qty will be flat rate, regular discount not applied, all bundle not used because of after budle regular qty will get regular discount(that not used)
