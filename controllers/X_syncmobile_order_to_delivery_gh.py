...
#            return user_type
            ...
            settingsRows = db((db.sm_settings.cid == cid) &(db.sm_settings.s_key.belongs(s_key_list)) ).select(db.sm_settings.s_key,db.sm_settings.s_value)
#            return db._lastsql
            visit_save_limit='0'
            visit_location=''
            delivery_date=''
...
#            if visit_saveRow:
#                visit_save_limit=visit_saveRow[0].s_value
#        #   Set order location must or not 
#            visit_locationRow = db((db.sm_settings.cid == cid) & (db.sm_settings.s_key == 'ORDER_LOCATION')).select(db.sm_settings.s_value, limitby=(0, 1))
#            visit_location='0'
#            if visit_locationRow:
#                visit_location=visit_locationRow[0].s_value
...
#                return 'FAILED<SYNCDATA>Invalid Date'
    ...
    day_name=str(visit_datetime.strftime('%A'))
...
#            return db._lastsql
            ...
            if not clientRecords:
                ...
                marketid = ''
                #-----
                levelRecords = db((db.sm_level.cid == cid) & (db.sm_level.level_id == route_id) & (db.sm_level.is_leaf == '1')).select(db.sm_level.level_id, db.sm_level.level_name, db.sm_level.depth, db.sm_level.level0, db.sm_level.level1, db.sm_level.level2, limitby=(0, 1))
                if levelRecords:
                ...
                #================ Schedule/Unschedule Check
                marketDayRow=db((db.sm_market_day.cid==cid) & (db.sm_market_day.area_id==route_id) & (db.sm_market_day.day_name==day_name)).select(db.sm_market_day.day_name,limitby=(0,1))
                if marketDayRow:
...
#                Insert in tracking table====================
                ...
                for i in range(len(order_infoList)):
                    ...
                    if len(orderDataList) == 2:
                        ...
                        orderTotal+=itemQty*float(itemPrice)
...
#                    db((db.sm_visit_plan.cid == cid) & (db.sm_visit_plan.schedule_date == schedule_date) & (db.sm_visit_plan.client_id == client_id)).update(visited_flag=1, visit_sl=vsl, visit_date=visit_date, status='Visited')
                ...
                # =============== call function to create INVOICE from ORDER
                settCompRows=db((db.sm_settings.cid == cid)&(db.sm_settings.s_key=='AUTO_DELIVERY')&(db.sm_settings.s_value=='YES')).select(db.sm_settings.cid,limitby=(0,1))
                if settCompRows:                    
                    getMsgSl=get_order_to_delivery(cid,depot_id,ordSl,client_id,visit_date)
                    try:
                        getMsgSlList=str(getMsgSl).split(',')
                        if len(getMsgSlList)==2:
                            invoiceRes=getMsgSlList[0]
                            invoiceSl=int(getMsgSlList[1])
                        else:
                            return 'FAILED<SYNCDATA>Error in invoice process'
                            
                    except:
                        return 'FAILED<SYNCDATA>Error in invoice process'
                
                # =============== Rep Achievement Update
                rows_check=db((db.target_vs_achievement_field_force.cid==cid) & (db.target_vs_achievement_field_force.first_date==firstDate)& (db.target_vs_achievement_field_force.rep_id==rep_id)).select(db.target_vs_achievement_field_force.id,db.target_vs_achievement_field_force.achievement_amount,limitby=(0,1))
                if rows_check:
                    achievement_amount=float(rows_check[0].achievement_amount)
                    netAchievement=achievement_amount+orderTotal
                    
                    rows_check[0].update_record(achievement_amount=netAchievement)
                                        
                else:   
                    target_year=int(str(firstDate)[0:4])
                    target_month=int(str(firstDate)[5:7])
                    
                    db.target_vs_achievement_field_force.insert(cid=cid,first_date=firstDate,target_year=target_year,target_month=target_month,rep_id=rep_id,rep_name=rep_name,achievement_amount=orderTotal)
...
#         return '<start>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<end>'
         return '<start>http://127.0.0.1:8000/mrepgh/syncmobile_order_to_delivery_gh/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<end>'
