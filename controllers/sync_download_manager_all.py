...
#        ====================if user type HO===============
        ...
        if(depot_mac==mac_get):
            query_data=db((db.sm_depot.cid==cid)).select(db.sm_depot.depot_id,db.sm_depot.name,db.sm_depot.op_balance,db.sm_depot.note,orderby=db.sm_depot.depot_id)                  
            return_value=''       
#            return db._lastsql
            for row_query_data in query_data:
                return_value=str(return_value)+str(row_query_data.depot_id)+"<fd>"+str(row_query_data.name)+"<fd>"+str(row_query_data.op_balance)+"<fd>"+str(row_query_data.note)+"<fd><rd>"                  
#            return db._lastsql
            return "<STARTSTART>"+str(return_value)+"<ENDEND>"
...
#                    query=qset((db.sm_order.cid==cid))
...
#        return sl
        creat_sl_query=sl.split(',',sl.count(','))
        total_sl=sl.count(',')
        i=0
        j=0
#                return sl
        ...
        sl_list=[]
        while i<total_sl+1:
            sl_get=creat_sl_query[i]
            sl_list.append(sl_get)
            
            i=i+1      
        
        if user_type_check=='HO':
            query=db((db.sm_requisition.cid==cid) & (db.sm_requisition.depot_id==set_depot) &(db.sm_requisition.sl.belongs(sl_list))).update(ho_status='1')
            query_head=db((db.sm_requisition_head.cid==cid) & (db.sm_requisition_head.depot_id==set_depot) &(db.sm_requisition_head.sl.belongs(sl_list))).update(ho_status='1')
         
            

            
        else:
            query=db((db.sm_requisition.cid==cid) & (db.sm_requisition.depot_id==set_depot) &(db.sm_requisition.sl.belongs(sl_list))).update(depot_status='1') 
            query_head=db((db.sm_requisition_head.cid==cid) & (db.sm_requisition_head.depot_id==set_depot) &(db.sm_requisition_head.sl.belongs(sl_list))).update(depot_status='1')

#        return query
#        return db._lastsql

        if ((query>0) and (query_head>0)):
            success="success";
            return "<STARTSTART>"+success+"<ENDEND>"
...
#                return sl
        ...
        if user_type_check=='HO':
            query=db((db.sm_issue.cid==cid) & (db.sm_issue.depot_id==set_depot) &(db.sm_issue.sl.belongs(sl_list))).update(ho_status='1')
            query_head=db((db.sm_issue_head.cid==cid) & (db.sm_issue_head.depot_id==set_depot) &(db.sm_issue_head.sl.belongs(sl_list))).update(ho_status='1')
         
            

            
        else:
            query=db((db.sm_issue.cid==cid) & (db.sm_issue.depot_id==set_depot) &(db.sm_issue.sl.belongs(sl_list))).update(depot_status='1') 
            query_head=db((db.sm_issue_head.cid==cid) & (db.sm_issue_head.depot_id==set_depot) &(db.sm_issue_head.sl.belongs(sl_list))).update(depot_status='1')
...
#                return sl
        ...
        if user_type_check=='HO':
            query=db((db.sm_receive.cid==cid) & (db.sm_receive.depot_id==set_depot) &(db.sm_receive.sl.belongs(sl_list))).update(ho_status='1')
            query_head=db((db.sm_receive_head.cid==cid) & (db.sm_receive_head.depot_id==set_depot) &(db.sm_receive_head.sl.belongs(sl_list))).update(ho_status='1')
         
            

            
        else:
            query=db((db.sm_receive.cid==cid) & (db.sm_receive.depot_id==set_depot) &(db.sm_receive.sl.belongs(sl_list))).update(depot_status='1') 
            query_head=db((db.sm_receive_head.cid==cid) & (db.sm_receive_head.depot_id==set_depot) &(db.sm_receive_head.sl.belongs(sl_list))).update(depot_status='1')
...
#                return sl
        ...
        if user_type_check=='HO':
            query=db((db.sm_damage.cid==cid) & (db.sm_damage.depot_id==set_depot) &(db.sm_damage.sl.belongs(sl_list))).update(ho_status='1')
            query_head=db((db.sm_damage_head.cid==cid) & (db.sm_damage_head.depot_id==set_depot) &(db.sm_damage_head.sl.belongs(sl_list))).update(ho_status='1')      
        else:
            query=db((db.sm_damage.cid==cid) & (db.sm_damage.depot_id==set_depot) &(db.sm_damage.sl.belongs(sl_list))).update(depot_status='1') 
            query_head=db((db.sm_damage_head.cid==cid) & (db.sm_damage_head.depot_id==set_depot) &(db.sm_damage_head.sl.belongs(sl_list))).update(depot_status='1')
...
#                return sl
        ...
        if user_type_check=='HO':
            query=db((db.sm_order.cid==cid) & (db.sm_order.depot_id==set_depot) &(db.sm_order.sl.belongs(sl_list))).update(ho_status='1')
            query_head=db((db.sm_order_head.cid==cid) & (db.sm_order_head.depot_id==set_depot) &(db.sm_order_head.sl.belongs(sl_list))).update(ho_status='1')      
        else:
            query=db((db.sm_order.cid==cid) & (db.sm_order.depot_id==set_depot) &(db.sm_order.sl.belongs(sl_list))).update(depot_status='1') 
            query_head=db((db.sm_order_head.cid==cid) & (db.sm_order_head.depot_id==set_depot) &(db.sm_order_head.sl.belongs(sl_list))).update(depot_status='1')
...
#                return sl
        ...
        try:
            if user_type_check=='HO':
                query=db((db.sm_invoice.cid==cid) & (db.sm_invoice.depot_id==set_depot) &(db.sm_invoice.sl.belongs(sl_list))).update(ho_status='1')
                query_head=db((db.sm_invoice_head.cid==cid) & (db.sm_invoice_head.depot_id==set_depot) &(db.sm_invoice_head.sl.belongs(sl_list))).update(ho_status='1')      
            else:
                query=db((db.sm_invoice.cid==cid) & (db.sm_invoice.depot_id==set_depot) &(db.sm_invoice.sl.belongs(sl_list))).update(depot_status='1') 
                query_head=db((db.sm_invoice_head.cid==cid) & (db.sm_invoice_head.depot_id==set_depot) &(db.sm_invoice_head.sl.belongs(sl_list))).update(depot_status='1')
            success="success";
            return "<STARTSTART>"+success+"<ENDEND>"
        ...
        try:
            ...
            while i<total_update_string_single+1: #while lopp based oncount total
               ...
               if user_type_check=='HO':
                    ...
                    query=db((db.sm_invoice.cid==cid) &  (db.sm_invoice_head.depot_id==update_id_sl[0]) & (db.sm_invoice.sl==update_id_sl[1])).update(depot_status='1') 
...
#            return query_data
            for row_query_data in query_data:
                return_value=str(return_value)+str(row_query_data.depot_id)+"<fd>"+str(row_query_data.sl)+"<fd>"+str(row_query_data.client_id)+"<fd>"+str(row_query_data.rep_id)+"<fd>"+str(row_query_data.return_date)+"<fd>"+str(row_query_data.order_sl)+"<fd>"+str(row_query_data.invoice_sl)+"<fd>"+str(row_query_data.area_id)+"<fd>"+str(row_query_data.status)+"<fd>"+str(row_query_data.order_media)+"<fd>"+str(row_query_data.discount)+"<fd>"+str(row_query_data.item_id)+"<fd>"+str(row_query_data.item_name)+"<fd>"+str(row_query_data.category_id)+"<fd>"+str(row_query_data.quantity)+"<fd>"+str(row_query_data.bonus_qty)+"<fd>"+str(row_query_data.price)+"<fd>"+str(row_query_data.short_note)+"<fd>"+str(row_query_data.note)+"<fd><rd>"
...
#                return sl
        ...
        if user_type_check=='HO':
            query=db((db.sm_return.cid==cid) & (db.sm_return.depot_id==set_depot) &(db.sm_return.sl.belongs(sl_list))).update(ho_status='1')
            query_head=db((db.sm_return_head.cid==cid) & (db.sm_return_head.depot_id==set_depot) &(db.sm_return_head.sl.belongs(sl_list))).update(ho_status='1')      
        else:
            query=db((db.sm_return.cid==cid) & (db.sm_return.depot_id==set_depot) &(db.sm_return.sl.belongs(sl_list))).update(depot_status='1') 
            query_head=db((db.sm_return_head.cid==cid) & (db.sm_return_head.depot_id==set_depot) &(db.sm_return_head.sl.belongs(sl_list))).update(depot_status='1')
...
#                return sl
        ...
        if user_type_check=='HO':
            query=db((db.sm_inbox.cid==cid) &(db.sm_inbox.sl.belongs(sl_list))).update(ho_status='1')
...
def delivery_data_new():
#    db((db.sm_invoice.cid=="cid") & (db.sm_invoice.updated_on<="2012-12-23 00:00:00") & (db.sm_invoice.ho_status=='0') ).select(db.sm_invoice.ALL,orderby=db.sm_invoice.updated_on|db.sm_invoice.depot_id|db.sm_invoice.sl,limitby=(0,100))
#    return db._lastsql
    my_str = request.args(0)
...
#            return date_download
            ...
            if user_type_check=='HO':
#                query_data=db((db.sm_invoice.cid==cid) & (db.sm_invoice.delivery_date<=date_download)  & (db.sm_invoice.ho_status=='0') & (db.sm_invoice.status.belongs(req_status))).select(db.sm_invoice.ALL,orderby=db.sm_invoice.delivery_date|db.sm_invoice.depot_id|db.sm_invoice.sl,limitby=limitby)
                query_data=db((db.sm_invoice.cid==cid) & (db.sm_invoice.updated_on<=date_download) & (db.sm_invoice.ho_status=='0') & (db.sm_invoice.status.belongs(req_status))).select(db.sm_invoice.ALL,orderby=db.sm_invoice.depot_id|db.sm_invoice.sl,limitby=limitby)
            else:
#                query_data=db((db.sm_invoice.cid==cid) & (db.sm_invoice.delivery_date<=date_download)  & (db.sm_invoice.depot_status=='0') & (db.sm_invoice.depot_id==user_id) & (db.sm_invoice.status.belongs(req_status))).select(db.sm_invoice.ALL,orderby=db.sm_invoice.delivery_date|db.sm_invoice.depot_id|db.sm_invoice.sl,limitby=limitby)
                query_data=db((db.sm_invoice.cid==cid) & (db.sm_invoice.updated_on<=date_download) & (db.sm_invoice.depot_status=='0') & (db.sm_invoice.depot_id==user_id) & (db.sm_invoice.status.belongs(req_status))).select(db.sm_invoice.ALL,orderby=db.sm_invoice.depot_id|db.sm_invoice.sl,limitby=limitby)
