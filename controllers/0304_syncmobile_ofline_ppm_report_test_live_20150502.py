...
#        ==== Order Count====================
        ...
        qset_oc=db()
        qset_oc=qset_vc((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) &     (db.sm_order_head.cid == cid) & (db.sm_order_head.area_id == db.sm_level.level_id ) &     (db.sm_order_head.field1 == 'ORDER') & (db.sm_order_head.order_datetime >= date_from) & (db.sm_order_head.order_datetime  < date_to)   )          
        ...
        if (se_market_report!="ALL"):
            qset_oc=qset_oc(db.sm_order_head.area_id == se_market_report)        
        if (rep_id!=rep_id_report):
            qset_oc=qset_oc(db.sm_order_head.rep_id == rep_id_report)

        reportRows_order_count=qset_oc.select(db.sm_order_head.sl.count()) 
        order_count='0'
        if reportRows_order_count:
            order_count=reportRows_order_count[0][db.sm_order_head.sl.count()]
...
#            =============area wise Value
        ...
        levelRows = db((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep)).select(db.sm_level.level_id, orderby=db.sm_level.level_id)
        for levelRow in levelRows:
            levelid = levelRow.level_id
            marketList=marketList+",'"+str(levelid)+"'"
        ...
        if (rep_id!=rep_id_report):
            condition=condition + "and sm_order.rep_id ="+ str(rep_id_report) 
        else:
            condition=condition + "and sm_order.area_id in ("+ str(marketList)+")"
...
#        ==============Order Count
        qset_oc=db()
        qset_oc=qset_oc((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) &     (db.sm_order_head.cid == cid) & (db.sm_order_head.area_id == db.sm_level.level_id ) & (db.sm_order_head.field1 == 'ORDER' ) & (db.sm_order_head.order_datetime >= date_from) & (db.sm_order_head.order_datetime  < date_to)   )          
...
#        levelRows = db((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) ).select(db.sm_level.is_leaf)
        ...
        for reportRow in reportRows:
            if (areawise_flag==0):
                ...
                areawise_flag=1
            areawise_str=areawise_str+str(reportRow[db.sm_level.level_name])+'('+str(reportRow[db.sm_doctor_visit.route_id])+') --'+str(reportRow[db.sm_doctor_visit.id.count()])+'</br>'
...
#        repRows = db((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) &     (db.sm_doctor_visit.cid == cid) & (db.sm_doctor_visit.route_id == db.sm_level.level_id ) & (db.sm_doctor_visit.visit_dtime >= date_from) & (db.sm_doctor_visit.visit_dtime  < date_to)).select(db.sm_doctor_visit.id.count(),db.sm_doctor_visit.rep_id,db.sm_doctor_visit.rep_name, groupby = db.sm_doctor_visit.rep_id)
        ...
        for repRow in repRows:
            ...
            repwise_str=repwise_str+str(repRow[db.sm_doctor_visit.rep_name])+'('+str(repRow[db.sm_doctor_visit.rep_id])+') --'+str(repRow[db.sm_doctor_visit.id.count()])+'</br>'
...
#         return db._lastsql
    ...
    if (user_type=='SUP'):
        ...
        qset_vc=db()
        qset_vc=qset_vc((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) &     (db.sm_prescription_head.cid == cid) & (db.sm_prescription_head.area_id == db.sm_level.level_id ) & (db.sm_prescription_head.created_on >= date_from) & (db.sm_prescription_head.created_on  < date_to))          
        if (se_market_report!="ALL"):
            qset_vc=qset_vc((db.sm_prescription_head.area_id == se_market_report))        
        if (rep_id!=rep_id_report):
...
#        =============area wise
        qset_ac=db()
        qset_ac=qset_ac((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) &     (db.sm_prescription_head.cid == cid) & (db.sm_prescription_head.area_id == db.sm_level.level_id ) & (db.sm_prescription_head.created_on >= date_from) & (db.sm_prescription_head.created_on  < date_to))          
        if (se_market_report!="ALL"):
...
#        RepWise=========================
        qset_rc=db()
        qset_rc=qset_rc((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) &     (db.sm_prescription_head.cid == cid) & (db.sm_prescription_head.area_id == db.sm_level.level_id ) & (db.sm_prescription_head.created_on >= date_from) & (db.sm_prescription_head.created_on  < date_to))          
        if (se_market_report!="ALL"):
...
#    Detail===========
        qset_detail=db()
        qset_detail=qset_detail((db.sm_level.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level] == level_rep) &     (db.sm_prescription_head.cid == cid) & (db.sm_prescription_head.area_id == db.sm_level.level_id ) & (db.sm_prescription_head.created_on >= date_from) & (db.sm_prescription_head.created_on  < date_to))  
...
#            clientRecords = db((db.sm_client.cid == cid) & (db.sm_client.client_id == client_id) & (db.sm_client.status == 'ACTIVE')).select(db.sm_client.name, db.sm_client.category_id, db.sm_client.area_id, db.sm_client.depot_id, limitby=(0, 1))
#            return db._lastsql
            clientRecords = db((db.sm_client.cid == cid) & (db.sm_client.client_id == client_id) & (db.sm_client.status == 'ACTIVE')).select(db.sm_client.name, db.sm_client.category_id, db.sm_client.area_id, db.sm_client.depot_id,db.sm_client.latitude,db.sm_client.longitude,db.sm_client.store_id,db.sm_client.store_name, limitby=(0, 1))
#            return db._lastsql
            client_lat=''
            client_long=''
            tracking_table_latlong="0,0"
...
#                Insert in tracking table====================
                ...
                for i in range(len(order_infoList)):
                    ...
                    if len(orderDataList) == 2:
                        ...
                        itemRow = db((db.sm_item.cid == cid) & (db.sm_item.item_id == itemId)).select(db.sm_item.name, db.sm_item.category_id,db.sm_item.category_id_sp, db.sm_item.price,db.sm_item.vat_amt, limitby=(0, 1))
                        if itemRow:
                            itemName = itemRow[0].name
                            itemCat = itemRow[0].category_id
                            itemCat_sp=itemRow[0].category_id_sp
                            itemPrice = itemRow[0].price
                            item_vat = itemRow[0].vat_amt
                            
                        #ins_dict = {'cid':cid, 'vsl':vsl, 'depot_id':depot_id, 'depot_name':depot_name, 'sl':ordSl,'store_id':store_id,'store_name':store_name,'item_id':itemId, 'item_name':itemName, 'category_id':itemCat,'category_id_sp':itemCat_sp, 'quantity':itemQty, 'price':itemPrice,'item_vat':item_vat}
                        
                        ins_dict = {'cid':cid, 'vsl':vsl, 'depot_id':depot_id, 'depot_name':depot_name, 'sl':ordSl, 'store_id':store_id,'store_name':store_name,'client_id':client_id, 'client_name':client_name, 'rep_id':rep_id, 'rep_name':rep_name, 'order_date':visit_date, 'order_datetime':visit_datetime, 'ym_date':firstDate,'delivery_date':delivery_date,'payment_mode':payment_mode,'collection_date':collection_date,
                                                                           'area_id':route_id, 'area_name':route_name, 'item_id':itemId, 'item_name':itemName, 'category_id':itemCat,'category_id_sp':itemCat_sp, 'quantity':itemQty, 'price':itemPrice,'item_vat':item_vat, 'order_media':'APP', 'status':'Submitted', 'level0_id' : level0_id,  'level0_name' : level0_name,'level1_id' : level1_id,'level1_name' : level1_name,'level2_id' : level2_id,'level2_name' : level2_name,'level3_id' : level3_id,'level3_name' : level3_name}
...
#    return errorFlag
    ...
    #             return giftStr
        insRes = db.sm_doctor_visit.insert(cid=cid, doc_id=doctorID, doc_name=docName, rep_id=rep_id, rep_name=rep_name, feedback=notesPart, ho_status='0', route_id=routeID, depot_id=depotID, visit_dtime=datetime_fixed, visit_date=current_date, giftnsample=itemngiftnsample,latitude = latitude, longitude = longitude, note=v_with, location_detail=str(location_detail),last_location=str(last_location))
...
#         return db._lastsql
        if not repRow:
           ...
           return retStatus
            ...
            retStatus = 'SUCCESS<SYNCDATA>Depot Stock'
            return retStatus
...
#     cid = str(request.vars.CID).strip().upper()
#     if (cid='BIOPHARMA'):
#        return '<start>http://c003.cloudapp.net/skf/syncmobile_ofline_ppm_report_test_live_20150502/<fd>http://c003.cloudapp.net/skf/static/<fd>http://c003.cloudapp.net/skf/syncmobile_ofline_ppm_02012015/<end>'
#         return '<start>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<end>'
          return '<start>http://127.0.0.1:8000/mrepskf/syncmobile_ofline_ppm_report_test_live_20150502/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<end>'
