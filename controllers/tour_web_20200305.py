...
#             return depthNo
        ...
        microList=[]
        microRows=db((db.sm_microunion.cid == cid) & (db.sm_microunion.area_id.belongs(areaList)) ).select(db.sm_microunion.microunion_id,orderby=db.sm_microunion.microunion_id)
        
        for microRows in microRows:
            microunionid = microRows.microunion_id            
            microList.append(microunionid) 
       
        if (session.depthNo_user) == 2:            
            pendingfo = db((db.sm_doctor_visit_plan.cid==cid)  &(db.sm_doctor_visit_plan.route_id.belongs(microList)) & (db.sm_doctor_visit_plan.status=='CReq') &(db.sm_doctor_visit_plan.schedule_date> current_date)).select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.status, orderby=db.sm_doctor_visit_plan.schedule_date, groupby=db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.schedule_date)
        else:
            pendingfo = db((db.sm_doctor_visit_plan.cid==cid)  &(db.sm_doctor_visit_plan.route_id.belongs(areaList)) & (db.sm_doctor_visit_plan.status=='CReq') &(db.sm_doctor_visit_plan.schedule_date> current_date)).select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.status, orderby=db.sm_doctor_visit_plan.schedule_date, groupby=db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.schedule_date)
