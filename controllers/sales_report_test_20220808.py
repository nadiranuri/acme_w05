    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter:
            if not(session.from_dt!='' and session.from_dt!=None and session.to_date!='' and session.to_date!=None):
                ...
                sql_str="SELECT  level1_id,  level1_name,area_id,  area_name,order_date FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND order_date >= '"+ str(session.from_dt)+"' AND order_date <= '"+ str(session.to_date) +"' "+ condition + " GROUP BY level1_id order by order_date desc;"
                # return sql_str
        ...
        #     records_ov=db.executesql(sql_str,as_dict = True)
            ...
            sql_str="SELECT level1_id,  level1_name,area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date = '"+ str(date_from) +"'  "+ condition + " GROUP BY level1_id order by order_date desc;"
            records_ov=db.executesql(sql_str,as_dict = True)
    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            if not(session.from_dt=='' or session.from_dt==None or session.to_date=='' or session.to_date==None):
                ...
                sql_str="SELECT  level2_id,  level2_name,area_id,  area_name,order_date FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND order_date >= '"+ str(session.from_dt) +"' AND order_date <= '"+ str(session.to_date) +"' AND level1_id = '"+ str(level1_id) +"' GROUP BY level2_id order by order_date desc;"
                # return sql_str
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass
            ...
            sql_str="SELECT level2_id,  level2_name,area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date = '"+ str(order_date) +"'  AND level1_id = '"+ str(level1_id) +"' GROUP BY level2_id order by order_date desc;"
            records_ov=db.executesql(sql_str,as_dict = True)
        # return sql_str
...
def sales_report_area_wise():
    ...
    if (user_type=='REP'):
        ...
        records_ov=[]
        sql_str="SELECT (sm_order.client_id) as client_id,(sm_order.client_name) as client_name,SUM((sm_order.price) * (sm_order.quantity)) as totalprice, sm_order.area_id as area_id, sm_order.vsl as vsl FROM sm_order WHERE sm_order.cid = '"+ str(cid)+"' " +condition+"  GROUP BY area_id,order_date,vsl ORDER BY `vsl` DESC  ;"
        # return sql_str
        records_ov=db.executesql(sql_str,as_dict = True)
