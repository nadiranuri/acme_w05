...
def home():
    ...
    btn_tour_plan=request.vars.btn_tour_plan
    btn_tour_planD=request.vars.btn_tour_planD
    
    btn_tour_plan_details=request.vars.btn_tour_plan_details
    btn_tour_plan_detailsD=request.vars.btn_tour_plan_detailsD
    ...
    if (btn_new_customer_info or btn_new_customer_info_D or btn_customer_type_wise_sales or btn_customer_type_wise_sales_D or btn_price_wise_product_sale or btn_price_wise_product_sale_D or btn_chk_incomp_inv or btn_chk_incomp_inv_D or btn_user_wise_inv or btn_user_wise_inv_D or btn_chk_dp_wise_doc or btn_chk_dp_wise_doc_D or btn_chk_dp_wise_inv_2 or btn_chk_dp_wise_inv_2_D or btn_chk_dp_wise_inv or btn_chk_dp_wise_inv_D or btn_tour_plan or btn_tour_planD or btn_tour_plan_details or btn_tour_plan_detailsD):
        ...
        if dateFlag==False:
            ...
            if dateDiff>31:
                ...
                if ((depot=='') | (store=='')):
                    ...
                    redirect(URL(c='report_tour',f='home'))
                else:
        ...
        #shima1226 start ============
                    if btn_tour_plan:
                        redirect (URL('tour_plan',vars=dict(date_from=date_from,date_to=date_to,pr_zone=pr_zone,pr_region=pr_region,pr_ff=pr_ff,pr_territory=pr_territory)))
                    if btn_tour_planD:
                        redirect (URL('tour_planD',vars=dict(date_from=date_from,date_to=date_to,pr_zone=pr_zone,pr_region=pr_region,pr_ff=pr_ff,pr_territory=pr_territory)))
                        
                    if btn_tour_plan_details:
                        redirect (URL('tour_plan_details',vars=dict(date_from=date_from,date_to=date_to,pr_zone=pr_zone,pr_region=pr_region,pr_ff=pr_ff,pr_territory=pr_territory)))
                    if btn_tour_plan_detailsD:
                        redirect (URL('tour_plan_detailsD',vars=dict(date_from=date_from,date_to=date_to,pr_zone=pr_zone,pr_region=pr_region,pr_ff=pr_ff,pr_territory=pr_territory)))
...
def tour_plan():
    ...
    if pr_ff!='':
        qset=qset(db.sm_doctor_visit_plan.rep_id == pr_ff)


    records=qset.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.note, orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.rep_id, groupby=db.sm_doctor_visit_plan.schedule_date)
...
#    Morning record
    ...
    qset_morning=db()
    qset_morning = qset_morning(db.sm_doctor_visit_plan.cid == cid)
    qset_morning = qset_morning(db.sm_doctor_visit_plan.schedule_date >= date_from)
    qset_morning = qset_morning(db.sm_doctor_visit_plan.schedule_date < date_to_m) 
    qset_morning = qset_morning(db.sm_doctor_visit_plan.note == 'Morning') 

    if pr_ff!='':
        qset_morning=qset_morning(db.sm_doctor_visit_plan.rep_id == pr_ff)
        
        
    records_morning=qset_morning.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.note, orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.rep_id)
#     return records_morning
    morningList=[]
    morningSearchList=[]
    morningSearch=''
    for records_morning in records_morning:
#         morningSearch=morningSearch+','+str(records_morning.schedule_date)+'|'+str(records_morning.rep_id)
        morningSearch=str(records_morning.schedule_date)+'|'+str(records_morning.rep_id)

        morningSearchList.append(str(morningSearch))
        morningList.append(str(records_morning.note))

    records_morningArea=qset_morning.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.note, orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.rep_id)
#     return records_morningArea
    morningAreaList=[]
    morningAreaSearchList=[]
    morsDatePast=''
    morrepPast=''
    morningAreaStr=''
    morrepList=[] 
    morrepPast_n=''
    morningSearch_area=''
    for records_morningArea in records_morningArea:
#         morningSearch_area=morningSearch_area+','+str(records_morningArea.schedule_date)+'|'+str(records_morningArea.rep_id)
        morningSearch_area=str(records_morningArea.schedule_date)+'|'+str(records_morningArea.rep_id)

        morningRStr=str(records_morningArea.rep_id)

        
        morningAreaStr=str(records_morningArea.route_name)

        if morsDatePast!=str(records_morningArea.schedule_date) and morrepPast!=str(records_morningArea.rep_id):
            morningAreaSearchList.append(morningSearch_area)
            morningAreaList.append(morningAreaStr)
            morningAreaStr=''

        morsDatePast=str(records_morningArea.schedule_date)
        morrepPast=str(records_morningArea.rep_id)
        
        
        morrepList.append(str(morningRStr))
...
#         
    ...
    qset_evening=db()
    qset_evening=qset_evening(db.sm_doctor_visit_plan.cid == cid)
    qset_evening = qset_evening(db.sm_doctor_visit_plan.schedule_date >= date_from)
    qset_evening = qset_evening(db.sm_doctor_visit_plan.schedule_date < date_to_m) 
    qset_evening = qset_evening(db.sm_doctor_visit_plan.note == 'Evening') 
   
           
    records_evening=qset_evening.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.note, orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.rep_id)
#     return records_evening
    eveningList=[]
    eveningSearchList=[]
    for records_evening in records_evening:
        eveningSearch=str(records_evening.schedule_date)+'|'+str(records_evening.rep_id)
        eveningSearchList.append(str(eveningSearch))
        eveningList.append(str(records_evening.note))
      
       
    records_eveningArea=qset_evening.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.note, orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.rep_id)
#     return records_eveningArea
    eveningAreaList=[]
    eveningAreaSearchList=[]
    sDatePast=''
    repPast=''
    eveningAreaStr=''
    for records_eveningArea in records_eveningArea:
        eveningSearch=str(records_evening.schedule_date)+'|'+str(records_evening.rep_id)
#         eveningAreaStr=eveningAreaStr+','+str(records_evening.route_name)
        eveningAreaStr=str(records_evening.route_name)
   
           
        if sDatePast!=str(records_evening.schedule_date) and repPast!=str(records_evening.rep_id):
            eveningAreaSearchList.append(eveningSearch)
            eveningAreaList.append(eveningAreaStr)
#             return eveningSearch+'  '+eveningAreaStr
            eveningAreaStr=''
           
        sDatePast=str(records_evening.schedule_date)
        repPast=str(records_evening.rep_id)


        
        
    return dict(records=records,eveningAreaSearchList=eveningAreaSearchList,eveningAreaList=eveningAreaList,eveningSearchList=eveningSearchList,eveningList=eveningList,morningSearchList=morningSearchList,morningList=morningList,morningAreaList=morningAreaList,morningAreaSearchList=morningAreaSearchList,date_from=date_from,date_to=date_to,pr_region=pr_region,pr_zone=pr_zone,pr_ff=pr_ff,pr_area=pr_area,pr_territory=pr_territory)
...
#     return records_evening
    ...
    return dict(records=records,eveningSearchList=eveningSearchList,eveningList=eveningList,morningSearchList=morningSearchList,morningList=morningList,date_from=date_from,date_to=date_to,pr_region=pr_region,pr_zone=pr_zone,pr_ff=pr_ff,pr_area=pr_area,pr_territory=pr_territory)
...
#     rep_newList=[]
    ...
    qset = qset(db.sm_doctor_day_plan.cid == cid)
    qset = qset(db.sm_doctor_day_plan.plan_date >= date_from)
    qset = qset(db.sm_doctor_day_plan.plan_date < date_to_m) 
    
    qset = qset(db.sm_client_day_plan.cid == cid)
    qset = qset(db.sm_client_day_plan.plan_date >= date_from)
    qset = qset(db.sm_client_day_plan.plan_date < date_to_m) 

    qset = qset((db.sm_doctor_day_plan.rep_id == db.sm_doctor_visit_plan.rep_id) and (db.sm_doctor_day_plan.plan_date == db.sm_doctor_visit_plan.schedule_date))
    
#     qset = qset((db.sm_client_day_plan.rep_id == db.sm_doctor_visit_plan.rep_id) and (db.sm_client_day_plan.plan_date == db.sm_doctor_visit_plan.schedule_date))

    if pr_ff!='':
        qset=qset(db.sm_doctor_visit_plan.rep_id == pr_ff)
        
#     records=qset.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.note,db.sm_doctor_day_plan.doc_name,db.sm_doctor_day_plan.plan_date,db.sm_doctor_day_plan.visit_time,db.sm_doctor_day_plan.rep_id,db.sm_client_day_plan.rep_id,  orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.rep_id|db.sm_doctor_day_plan.plan_date|db.sm_doctor_day_plan.rep_id, groupby=db.sm_doctor_visit_plan.schedule_date)


    records=qset.select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.note,db.sm_doctor_day_plan.doc_name,db.sm_doctor_day_plan.plan_date,db.sm_doctor_day_plan.visit_time,db.sm_doctor_day_plan.rep_id,db.sm_client_day_plan.plan_date,db.sm_client_day_plan.visit_time,db.sm_client_day_plan.rep_id, orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.rep_id|db.sm_doctor_day_plan.plan_date|db.sm_doctor_day_plan.rep_id|db.sm_client_day_plan.plan_date|db.sm_client_day_plan.rep_id, groupby=db.sm_doctor_visit_plan.schedule_date)
...
#         morningSearch_area=morningSearch_area+','+str(records_morningArea.schedule_date)+'|'+str(records_morningArea.rep_id)
        ...
        morrepList.append(str(morningRStr))
#     return morningAreaList
        
#    ================================

#    Morning Doctor record

    qset_mor_doc = db()
    qset_mor_doc = qset_mor_doc(db.sm_doctor_day_plan.cid == cid)
    qset_mor_doc = qset_mor_doc(db.sm_doctor_day_plan.plan_date >= date_from)
    qset_mor_doc = qset_mor_doc(db.sm_doctor_day_plan.plan_date < date_to_m) 
    qset_mor_doc = qset_mor_doc(db.sm_doctor_day_plan.visit_time == 'Morning')
    

    records_mor_doc=qset_mor_doc.select(db.sm_doctor_day_plan.doc_id,db.sm_doctor_day_plan.rep_id,db.sm_doctor_day_plan.doc_name,db.sm_doctor_day_plan.plan_date,db.sm_doctor_day_plan.visit_time, orderby=db.sm_doctor_day_plan.plan_date|db.sm_doctor_day_plan.rep_id)
#     return records_mor_doc
#     return db._lastsql
    strMorningDoc=''
    docCpast=''
    docName=''
    for records_mor_doc in records_mor_doc:
#         docName=''
        docVDate=str(records_mor_doc.plan_date)
        docRep=str(records_mor_doc.rep_id)
        docC=str(records_mor_doc.plan_date)+'|'+str(records_mor_doc.rep_id)
        morDocSearch=str(records_mor_doc.plan_date)+'|'+str(records_mor_doc.rep_id) 
        if docCpast!=docC:
            strMorningDoc=strMorningDoc+'<'+morDocSearch+'>'
#         docName=str(records_mor_doc.doc_name)+'|'+str(records_mor_doc.doc_id)+','
        if docName == '':
            docName=','+str(records_mor_doc.doc_name)+'|'+str(records_mor_doc.doc_id)
        else:
            docName=','+str(records_mor_doc.doc_name)+'|'+str(records_mor_doc.doc_id)
...
#         return docName
        ...
        strMorningDoc=strMorningDoc+docName
      
        docCpast=docC 

#    ================================

#    Morning Client record
    qset_mor_clnt = db()
    qset_mor_clnt = qset_mor_clnt(db.sm_client_day_plan.cid == cid)
    qset_mor_clnt = qset_mor_clnt(db.sm_client_day_plan.plan_date >= date_from)
    qset_mor_clnt = qset_mor_clnt(db.sm_client_day_plan.plan_date < date_to_m) 
    qset_mor_clnt = qset_mor_clnt(db.sm_client_day_plan.visit_time == 'Morning')
    
    records_mor_clnt=qset_mor_clnt.select(db.sm_client_day_plan.client_id,db.sm_client_day_plan.rep_id,db.sm_client_day_plan.client_name,db.sm_client_day_plan.plan_date,db.sm_client_day_plan.visit_time, orderby=db.sm_client_day_plan.plan_date|db.sm_client_day_plan.rep_id)
#     return records_mor_clnt
#     return db._lastsql
    strMorningClnt=''
    clntCpast=''
    clntName=''
    for records_mor_clnt in records_mor_clnt:
#         clntName=''
        clntVDate=str(records_mor_clnt.plan_date)
        clntRep=str(records_mor_clnt.rep_id)
        clntC=str(records_mor_clnt.plan_date)+'|'+str(records_mor_clnt.rep_id)
        morClntSearch=str(records_mor_clnt.plan_date)+'|'+str(records_mor_clnt.rep_id) 
        if clntCpast!=clntC:
            strMorningClnt=strMorningClnt+'<'+morClntSearch+'>'
#         clntName=str(records_mor_clnt.client_name)+'|'+str(records_mor_clnt.client_id)+','
        if clntName == '':
            clntName=','+str(records_mor_clnt.client_name)+'|'+str(records_mor_clnt.client_id)
        else:
            clntName=','+str(records_mor_clnt.client_name)+'|'+str(records_mor_clnt.client_id)
        ...
        strMorningClnt=strMorningClnt+clntName
      
        clntCpast=clntC 
#         return  strMorningClnt 
...
#         
    ...
    if pr_ff!='':
        qset_evening=qset_evening(db.sm_doctor_visit_plan.rep_id == pr_ff)
...
#    Evening Doctor record
    qset_evng_doc = db()
    qset_evng_doc = qset_evng_doc(db.sm_doctor_day_plan.cid == cid)
    qset_evng_doc = qset_evng_doc(db.sm_doctor_day_plan.plan_date >= date_from)
    qset_evng_doc = qset_evng_doc(db.sm_doctor_day_plan.plan_date < date_to_m) 
    qset_evng_doc = qset_evng_doc(db.sm_doctor_day_plan.visit_time == 'Evening')
    

    records_evng_doc=qset_evng_doc.select(db.sm_doctor_day_plan.doc_id,db.sm_doctor_day_plan.rep_id,db.sm_doctor_day_plan.doc_name,db.sm_doctor_day_plan.plan_date,db.sm_doctor_day_plan.visit_time, orderby=db.sm_doctor_day_plan.plan_date|db.sm_doctor_day_plan.rep_id)
#     return records_evng_doc
#     return db._lastsql
    strevngDoc=''
    docEpast=''
    docEName=''
    for records_evng_doc in records_evng_doc:
#         docEName=''
        docEVDate=str(records_evng_doc.plan_date)
        docERep=str(records_evng_doc.rep_id)
        docCE=str(records_evng_doc.plan_date)+'|'+str(records_evng_doc.rep_id)
        evngDocSearch=str(records_evng_doc.plan_date)+'|'+str(records_evng_doc.rep_id) 
        if docEpast!=docCE:
            strevngDoc=strevngDoc+'<'+evngDocSearch+'>'
#         docEName=str(records_evng_doc.doc_name)+'|'+str(records_evng_doc.doc_id)+','
        if docEName == '':
            docEName=','+str(records_evng_doc.doc_name)+'|'+str(records_evng_doc.doc_id)
        else:
            docEName=','+str(records_evng_doc.doc_name)+'|'+str(records_evng_doc.doc_id)

        strevngDoc=strevngDoc+docEName
...
#    Morning Client record
    qset_evng_clnt = db()
    qset_evng_clnt = qset_evng_clnt(db.sm_client_day_plan.cid == cid)
    qset_evng_clnt = qset_evng_clnt(db.sm_client_day_plan.plan_date >= date_from)
    qset_evng_clnt = qset_evng_clnt(db.sm_client_day_plan.plan_date < date_to_m) 
    qset_evng_clnt = qset_evng_clnt(db.sm_client_day_plan.visit_time == 'Evening')
    
    records_evng_clnt=qset_evng_clnt.select(db.sm_client_day_plan.client_id,db.sm_client_day_plan.rep_id,db.sm_client_day_plan.client_name,db.sm_client_day_plan.plan_date,db.sm_client_day_plan.visit_time, orderby=db.sm_client_day_plan.plan_date|db.sm_client_day_plan.rep_id)
#     return records_evng_clnt
#     return db._lastsql
    strevngClnt=''
    eclntCpast=''
    eclntName=''
    for records_evng_clnt in records_evng_clnt:
#         eclntName=''
        eclntVDate=str(records_evng_clnt.plan_date)
        eclntRep=str(records_evng_clnt.rep_id)
        eclntC=str(records_evng_clnt.plan_date)+'|'+str(records_evng_clnt.rep_id)
        evngClntSearch=str(records_evng_clnt.plan_date)+'|'+str(records_evng_clnt.rep_id) 
        if eclntCpast!=eclntC:
            strevngClnt=strevngClnt+'<'+evngClntSearch+'>'
#         eclntName=str(records_evng_clnt.client_name)+'|'+str(records_evng_clnt.client_id)+','
        if eclntName == '':
            eclntName=','+str(records_evng_clnt.client_name)+'|'+str(records_evng_clnt.client_id)
        else:
            eclntName=','+str(records_evng_clnt.client_name)+'|'+str(records_evng_clnt.client_id)
...
#         return eclntName
        ...
        strevngClnt=strevngClnt+eclntName
      
        eclntCpast=eclntC
        
        
    return dict(records=records,strMorningDoc=strMorningDoc,strMorningClnt=strMorningClnt,strevngClnt=strevngClnt,strevngDoc=strevngDoc,eveningSearchList=eveningSearchList,eveningList=eveningList,eveningAreaList=eveningAreaList,eveningAreaSearchList=eveningAreaSearchList,morningSearchList=morningSearchList,morningList=morningList,morningAreaList=morningAreaList,morningAreaSearchList=morningAreaSearchList,date_from=date_from,date_to=date_to,pr_region=pr_region,pr_zone=pr_zone,pr_ff=pr_ff,pr_area=pr_area,pr_territory=pr_territory)
