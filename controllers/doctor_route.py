...
def doctor_route_add():
    ...
    c_id = session.cid
    microunion=request.vars.microunion
    note=''
    field1=''
    if microunion!=None and microunion!='':
        note=str(microunion.split('|')[0]).upper()
        field1=str(microunion.split('|')[1]).upper()
    
    #   ---------------------
    form = SQLFORM(db.sm_doctor_area,
                  fields=['doc_id', 'area_id',  'address','field2'],
                  submit_button='Save'
                  )

    form.vars.cid = c_id
    form.vars.note = note
    form.vars.field1 = field1
    clientRow=db((db.sm_microunion.cid == c_id) & (db.sm_microunion.microunion_id == field1) & (db.sm_microunion.microunion_name == note)).select(db.sm_microunion.microunion_id, db.sm_microunion.microunion_name, limitby=(0, 1))
    if not clientRow:
        pass
    if clientRow:
    ...
    if (session.btn_filter_docroute):
        ...
        elif (session.searchType_docroute == 'MicrounionID'):
            searchValue=str(session.searchValue_docroute).split('|')[0]
            searchValueName=str(session.searchValue_docroute).split('|')[1]
            qset = qset(db.sm_doctor_area.field1==searchValue)
            qset = qset(db.sm_doctor_area.note==searchValueName)
...
def batch_upload_doctor_route():
    ...
    chk_upload = request.vars.chk_upload
    btn_update = request.vars.btn_update
    
    cleanRoutePass = request.vars.cleanRoutePass
    routeID = request.vars.cleanRouteID
    ...
    if btn_upload == 'Upload':
        ...
        for i in range(total_row):
            ...
            if len(coloum_list) == 6:
                ...
                district = str(coloum_list[4]).strip()
                thana = str(coloum_list[5]).strip()
...
#                             return validDoctor
                            if validDoctor == True:
                                ...
                                if validRoute == True:
                                    if client_id == '':
                                        insRes = db.sm_doctor_area.insert(cid=c_id, doc_id=doc_id, doc_name=docName, area_id=routeId, area_name=route_name, address=address, depot_id=depot_id, district=district, thana=thana)
                                        if insRes:
                                            count_inserted += 1
                                        else:
...
#     return chk_upload
#     ======================Update
    if btn_update == 'Route update' and chk_upload=='update':
#         return 'Nadira'
        excel_data = str(request.vars.excel_data)
        inserted_count = 0
        error_count = 0
            ...
            if len(coloum_list) == 6:
                ...
                if not(doc_id == '' or route_id == ''):
                    try:
                        ...
                        #----------- check duplicate
                        existRows = db((db.sm_doctor_area.cid == c_id) & (db.sm_doctor_area.area_id == route_id)).select(db.sm_doctor_area.doc_id, limitby=(0, 1))
#                         return db._lastsql
                        if existRows:
                            duplicate_doc = True
                            
                        #-----------------
                        if(duplicate_doc == True):
                            validDoctor = False
                            #----------- check doctor
                            for i in range(len(doc_list_valid)):
                                myRowData = doc_list_valid[i]
...
#                             return validDoctor
                            if validDoctor == True:
                                ...
                                if validRoute == True:
                                    if client_id == '':
                                        insRes = db((db.sm_doctor_area.cid==c_id) & (db.sm_doctor_area.doc_id==doc_id)).update(area_id=routeId, area_name=route_name)
#                                         return insRes
                                        if insRes:
                                            count_inserted += 1
                                        else:
                            ...
                            error_data = row_data + '(Doctor not available)\n'
                            error_str = error_str + error_data
                            count_error += 1
                            continue
                    except:
...
#     ======================Update
    if cleanRoutePass=='abnbl564321' and routeID!='':
        existRows = db((db.sm_doctor_area.cid == c_id) & (db.sm_doctor_area.area_id == routeID)).select(db.sm_doctor_area.doc_id, limitby=(0, 1))
        if existRows:
            insRes = db((db.sm_doctor_area.cid==c_id) & (db.sm_doctor_area.area_id==routeID)).delete()
            error_str = 'Clean Complete ' 
        else:
            error_str = 'Invaid Route'  

    else:
        error_str = 'Use valid pass and Route'   
