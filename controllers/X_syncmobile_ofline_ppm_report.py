...
#                         return retStatus
                        ...
                        for clientRow in clientRows:
                            ...
                            if start_flag == 0:
                                clientStr = clientStr + str(client_id) + '<fd>' + str(name) + '<fd>' + str(category_id)
                                start_flag = 1
                            else:
                                clientStr = clientStr + '<rd>' + str(client_id) + '<fd>' + str(name) + '<fd>' + str(category_id)
...
def s_call_order_summary():
    ...
    repRow = db((db.sm_rep.cid == cid) & (db.sm_rep.rep_id == rep_id) & (db.sm_rep.password == password) & (db.sm_rep.sync_code == synccode) & (db.sm_rep.status == 'ACTIVE')).select(db.sm_rep.id, limitby=(0, 1))
...
#    report_string=str(records)
    ...
    records_oc=qset_oc.select(db.sm_order_head.sl)
#    return db._lastsql
    slList=[]
    
    for records_oc in records_oc:
#        order_count=records_oc[0][db.sm_order_head.sl.count()]
        slList.append(records_oc.sl)
    
    
    
#    Sales Call====================
    qset_count=db()
    qset_count=qset((db.sm_order.cid == cid) & (db.sm_order.rep_id == rep_id) & (db.sm_order.sl.belongs(slList)))

    if (se_market_report!="ALL"):        
       qset_count=qset_count(db.sm_order.area_id==se_market_report)
    if (se_item_report!="ALL"):        
       qset_count=qset_count(db.sm_order.item_id==se_item_report)
    records_count=qset_count.select(db.sm_order_head.sl.count(), groupby=db.sm_order.sl)
#    return db._lastsql
#    report_string=str(records)
    order_count='0'
    if records_count:
        order_count=records_count[0][db.sm_order_head.sl.count()]
    
    report_string=str(order_count)
    


#  Order Value  
    qset_ov=db()
    qset_ov=qset_ov((db.sm_order.cid == cid) & (db.sm_order.rep_id == rep_id) & (db.sm_order.sl.belongs(slList)))
#    qset_ov=qset_ov((db.sm_order.order_date >= date_from) & (db.sm_order.order_date  < date_to))
#    return se_market_report
    if (se_market_report!="ALL"):        
       qset_ov=qset_ov(db.sm_order.area_id==se_market_report) 
    if (se_item_report!="ALL"):        
       qset_ov=qset_ov(db.sm_order.item_id==se_item_report) 
    records_ov=qset_ov.select(db.sm_order.price.sum(), groupby=db.sm_order.sl)
    order_value='0.0'
    if records_ov:
        order_value=records_ov[0][db.sm_order.price.sum()]
...
#    return db._lastsql
    report_string=str(sales_call)+ '<rd>' +str(order_count)+ '<rd>' +str(order_value)
...
#    return db._lastsql
#    if (se_item_report!="ALL"):        
#       qset_ol=qset_ol(db.sm_order_head.item_id==se_item_report) 
...
#    return order_sl
    ...
    if (order_sl != ''): 
        ...
        if (se_item_report!="ALL"):        
            qset_o=qset_o(db.sm_order.item_id==se_item_report) 
...
#            return db._lastsql
            ...
            if (ordeer_vsl_past!=vsl):
                if (start_flag_amount==0):
                    order_string="Visit SL:"+str(vsl)+ " Time:"+str(order_datetime)+"</br>"+str(c_name)+" ("+str(c_id)+" )</br>Order ="+str(order_amount)
                else:
...
#            Visit SL: 2546 Time: 12:35
#            Ma Medical Hall (code)
#            Order = 0
...
#        return records_o
    ...
    report_string=str(sales_call)+ '<rd>' +str(order_count)+ '<rd>' +str(order_value)+'<rd>'+str(order_string)
...
#  visit Count  
    ...
    #            return ppm_string
            ...
            if (start_flag==0):
                ...
                if (campaign_string!=''):
                    visit_string=visit_string+"</br>Campaign: "+str(campaign_string)
                if (sample_string!=''):
...
#          return '<start>http://127.0.0.1:8000/mrepnovelta/syncmobile_ofline_ppm_02012015/<fd>http://127.0.0.1:8000/mrepnovelta/static/<fd>http://127.0.0.1:8000/mrepnovelta/syncmobile_ofline_ppm_02012015/<end>'
         return '<start>http://e2.businesssolutionapps.com/mrepnovelta/syncmobile_ofline_ppm_report/<fd>http://e2.businesssolutionapps.com/mrepnovelta/static/<fd>http://e2.businesssolutionapps.com/mrepnovelta/syncmobile_ofline_ppm_report/<end>'
#          return '<start>http://127.0.0.1:8000/mrepnovelta/syncmobile_ofline_ppm_report/<fd>http://e2.businesssolutionapps.com/mrepnovelta/static/<fd>http://e2.businesssolutionapps.com/mrepnovelta/syncmobile_ofline_ppm_report/<end>'
