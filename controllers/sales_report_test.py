...
def salesDcr_report_zm_url():
    ...
    # se_market_report = str(request.vars.se_market_report).strip().upper()
    # level_id = str(request.vars.level_id).strip().upper()
    from_dt=str(request.vars.to_dt_2).strip()  
    to_date=str(request.vars.to_dt).strip()
...
def salesDcr_report_zm():
    ...
    if (user_type=='SUP'):
        ...
        if (from_dt or to_date)!='' and (from_dt or to_date)!=None and (from_dt or to_date)!='None':                   
            now = datetime.datetime.strptime(to_date, "%Y-%m-%d")
            to_date_1=now + datetime.timedelta(days = 1)
            to_date=str(to_date_1).split(' ')[0]
            qset=qset((db.sm_doctor_visit.visit_date >= from_dt) & (db.sm_doctor_visit.visit_date < to_date))
            # return to_date
        else:
            qset = qset((db.sm_doctor_visit.visit_date <= date_from) & (db.sm_doctor_visit.visit_date > date_to))
            session.from_dt=date_to
            session.to_date=date_from
        ...
        # return date_from
        # records = qset.select(db.sm_doctor_visit.doc_id,db.sm_doctor_visit.rep_id,db.sm_doctor_visit.doc_id,db.sm_doctor_visit.level0_id,db.sm_doctor_visit.level0_name,db.sm_doctor_visit.visit_date, orderby=~db.sm_doctor_visit.visit_date, groupby=db.sm_doctor_visit.level0_id)
    ...
    return dict(doc_id_list=doc_id_list,records=records,date_from=date_from,date_to=date_to,cid=cid,rep_id=rep_id,password=password,synccode=synccode,vChecklist=vChecklist,vCountList=vCountList,search_form=search_form)
...
def salesDcr_report_rsm():
    ...
    synccode =session.synccode 
    rep_id_report =session.rep_id_report 
    route_id_report =session.route_id
    ...
    if (user_type=='SUP'):
        ...
        if session.route_id!='' and session.route_id!='None' and session.route_id!=None:
        ...
        if (from_dt and to_date)!='' and (from_dt and to_date)!=None and (from_dt and to_date)!='None':                    
...
def salesDcr_report_fm_url():
    ...
    from_dt=str(request.vars.from_dt).strip()  
...
def salesDcr_report_detail():
    ...
    if (user_type=='REP'):
        ...
        records = qset.select(db.sm_doctor_visit.doc_id.count(),db.sm_doctor_visit.doc_id,db.sm_doctor_visit.rep_id,db.sm_doctor_visit.route_id,db.sm_doctor_visit.route_name,db.sm_doctor_visit.visit_date, orderby=db.sm_doctor_visit.visit_date, groupby=db.sm_doctor_visit.doc_id|db.sm_doctor_visit.rep_id|db.sm_doctor_visit.route_id|db.sm_doctor_visit.route_name|db.sm_doctor_visit.visit_date)
        return db._lastsql
        doc_id_list=[]
        visit_date_list=[]
        if records:
