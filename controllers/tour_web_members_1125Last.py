...
#             else :
#                 return '2'
            repRows = db((db.sm_rep_area.cid == cid) & (db.sm_level.is_leaf == '1') & (db.sm_level[level].belongs(level_idList) )).select(db.sm_rep_area.area_id, orderby=db.sm_rep_area.area_id)
#                 return repRows
            for repRow in repRows:
                territoryid = repRow.area_id            
                areaList.append(territoryid)
