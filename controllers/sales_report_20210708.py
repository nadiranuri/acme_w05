    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter:
            if (from_dt!='' and to_date!=''):
                date_from=to_date
                date_to=from_dt
                # sql_str="SELECT SUM((sm_order.price) * (sm_order.quantity)) as totalprice, sm_order.area_id as area_id, sm_order.area_name as area_name,sm_order.delivery_date as delivery_date,(SELECT count(sm_order_head.sl) FROM sm_order_head WHERE sm_order_head.cid = '"+ str(cid)+"'  AND sm_order_head.delivery_date = sm_order.delivery_date "+ condition + " ) as visit_count,(SELECT count(sm_order_head.sl) FROM sm_order_head WHERE sm_order_head.cid = '"+ str(cid)+"' AND sm_order_head.field1='ORDER' AND sm_order_head.delivery_date = sm_order.delivery_date "+ condition + " GROUP BY area_id,delivery_date order by delivery_date desc) as order_count FROM sm_order  WHERE sm_order.cid = '"+ str(cid) +"' AND sm_order.delivery_date > '"+ str(from_dt).split(' ')[0] +"' AND sm_order.delivery_date <= '"+ str(to_date) +"' "+ condition + " GROUP BY area_id,delivery_date order by delivery_date desc;"
                sql_str="SELECT  area_id,  area_name,delivery_date FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND delivery_date > '"+ str(from_dt).split(' ')[0] +"' AND delivery_date <= '"+ str(to_date) +"' "+ condition + " GROUP BY area_id,delivery_date order by delivery_date desc;"
                # return sql_str
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass
            ...
            sql_str="SELECT area_id,  area_name, delivery_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND delivery_date <= '"+ str(date_from) +"' AND delivery_date > '"+ str(date_to) +"' "+ condition + " GROUP BY area_id,delivery_date order by delivery_date desc;"
            # return sql_str
            records_ov=db.executesql(sql_str,as_dict = True)
...
def sales_report_area_wise():
    ...
    if (user_type=='SUP'):
        ...
        records_ov=[]
        sql_str="SELECT (sm_order.client_id) as client_id,(sm_order.client_name) as client_name,SUM((sm_order.price) * (sm_order.quantity)) as totalprice, sm_order.area_id as area_id, sm_order.vsl as vsl FROM sm_order WHERE sm_order.cid = '"+ str(cid) +"' AND sm_order.delivery_date = '"+ str(dlvry_date) +"' and sm_order.area_id='"+ area_id+"'   GROUP BY area_id,delivery_date,vsl ORDER BY vsl DESC  ;"
