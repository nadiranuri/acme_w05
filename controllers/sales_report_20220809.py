...
def salesDcr_report_detail():
    ...
    repRow = db((db.sm_rep.cid == cid) & (db.sm_rep.rep_id == rep_id) & (db.sm_rep.password == rep_pass) & (db.sm_rep.sync_code == device_id) & (db.sm_rep.status == 'ACTIVE')).select(db.sm_rep.ALL, limitby=(0, 1))
    # return repRow
    if not repRow:
...
def slsRptDcr_area_wise_url():
    ...
    redirect(URL(c='sales_report_dcr',f='slsRptDcr_area_wise'))
