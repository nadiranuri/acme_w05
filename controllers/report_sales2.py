...
def home():
    ...
    btn_inv_ret_details_ret=request.vars.btn_inv_ret_details_ret
    
    btn_inv_ret_details_D=request.vars.btn_inv_ret_details_D
    btn_inv_ret_itemwise=request.vars.btn_inv_ret_itemwise
    btn_inv_ret_itemwise_ret=request.vars.btn_inv_ret_itemwise_ret
    ...
    #--------------- Billal
    #     Billal
    if (btn_inv_ret_details or btn_inv_ret_details_ret or btn_inv_ret_details_D or btn_inv_ret_itemwise or btn_inv_ret_itemwise_ret or btn_inv_ret_itemwise_D or btn_inv_ret_checking or btn_inv_ret_checking_D or btn_discount_typewise_sale or btn_discount_typewise_sale_D or btn_discount_typewise_sale_ad or btn_discount_typewise_sale_with_value or btn_discount_typewise_sale_with_tp or btn_accounting_period_invoice_wise or btn_customer_type_wise_sales or btn_return_note_preview or btn_invoice_exclusive_bonus):
        date_from=request.vars.from_dt_2
        date_to=request.vars.to_dt_2
        
        from_dt_3=request.vars.from_dt_3
        to_dt_3=request.vars.to_dt_3
        ...
        if dateFlag==False:
            ...
            if dateDiff>92:
                ...
                date_from_ret=''
                date_to_ret=''
                if btn_inv_ret_details_ret or btn_inv_ret_itemwise_ret:
                    try:                
                        if not (from_dt_3=='' or to_dt_3==''):
                            date_from_ret=datetime.datetime.strptime(str(from_dt_3),'%Y-%m-%d')
                            date_to_ret=datetime.datetime.strptime(str(to_dt_3),'%Y-%m-%d')
                            if date_from_ret>date_to_ret:
                                dateFlagRet=False
                            else:
                                date_from_ret=date_from_ret.strftime('%Y-%m-%d')
                                date_to_ret=date_to_ret.strftime('%Y-%m-%d')
                    except:
                        dateFlagRet=False
                ...
                if dateFlagRet==False:
                    ...
                    if (depot=='' or store==''):
                        ...
                        elif btn_inv_ret_details_ret:                        
                            redirect (URL('invoice_return_details_ret',vars=dict(date_from=date_from,date_to=date_to,date_from_ret=date_from_ret,date_to_ret=date_to_ret,depot_id=depot_id,store_id=store_id,customer_id=customer_id,dman_id=dman_id,teritory_id=teritory_id,market_id=market_id,mso_id=mso_id)))
                        ...
                        elif btn_inv_ret_itemwise_ret:
                            redirect (URL('invoice_return_itemwise_ret',vars=dict(date_from=date_from,date_to=date_to,date_from_ret=date_from_ret,date_to_ret=date_to_ret,depot_id=depot_id,store_id=store_id,customer_id=customer_id,dman_id=dman_id,teritory_id=teritory_id,market_id=market_id,mso_id=mso_id)))
...
def invoice_return_details():
    ...
    response.title='30.1A Invoice and Return Details'
...
def invoice_return_details_download():
    ...
    #-------------
    myString='30.1A Invoice and Return Details\n'
    ...
    myString+='Inv.Date,Inv.Number,Cust.ID,Cust.Name,Retn.Date,Return Number,Sales,Discount (Reg+SP),Net Sales,Vat,Sales+Vat,Return Note,Discount (Reg+SP),Net Return Note,Vat,Return Note+Vat'+'\n'
...
def invoice_return_details_ret():
    c_id=session.cid
    
    response.title='30.1B Invoice and Return Details'
    
    fromDate=request.vars.date_from
    toDate=request.vars.date_to
    
    date_from_ret=request.vars.date_from_ret
    date_to_ret=request.vars.date_to_ret
    ...
    condStr=" (sm_return_head.cid = '"+c_id+"') AND (sm_return_head.depot_id='"+depot_id+"') AND (sm_return_head.store_id='"+store_id+"') AND ((sm_return_head.invoice_date >= '"+str(startDt)+"') AND (sm_return_head.invoice_date <= '"+str(endDt)+"')) AND (sm_return_head.status='Returned')"
    
    if not(date_from_ret=='' or date_to_ret==''):
        condStr+=" AND ((sm_return_head.return_date >= '"+str(date_from_ret)+"') AND (sm_return_head.return_date <= '"+str(date_to_ret)+"'))"   
        
    if customer_id!='':
        condStr+=" AND (sm_return_head.client_id='"+customer_id+"')"        
    if dman_id!='':
        condStr+=" AND (sm_return_head.d_man_id='"+dman_id+"')"        
    if territory_id!='':
        condStr+=" AND (sm_return_head.area_id='"+territory_id+"')"      
    if market_id!='':
        condStr+=" AND (sm_return_head.market_id='"+market_id+"')"      
    if mso_id!='':
        condStr+=" AND (sm_return_head.rep_id='"+mso_id+"')"        
        
    if startDt!='' and endDt!='': 
        dateRecords="SELECT sm_return_head.depot_id as depot_id,sm_return_head.invoice_date as invoice_date,sm_return_head.invoice_sl as invSl,sm_return_head.return_date as return_date,sm_return_head.sl as retSl,sm_return_head.client_id as client_id,sm_return_head.client_name as client_name,sm_return_head.inv_actual_total_tp as actualTpAmt,sm_return_head.inv_vat_total_amount as vatTotalAmt,sm_return_head.inv_discount as discAmt,sm_return_head.inv_sp_discount as spDiscAmt,sm_return_head.ret_actual_total_tp as retTpAmt,sm_return_head.vat_total_amount as retVatAmt,sm_return_head.discount as retDiscAmt,sm_return_head.sp_discount as retSpDiscAmt FROM sm_return_head WHERE ("+str(condStr)+") ORDER BY sm_return_head.invoice_sl"
    else:
        dateRecords=''
        response.flash='Required Date From and Date To'
    ...
    return dict(recordList=recordList,date_from_ret=date_from_ret,date_to_ret=date_to_ret,date_from=startDt,date_to=endDt,depot_id=depot_id,depotName=depot_name,store_id=store_id,storeName=store_name,dman_id=dman_id,delivery_man_name=delivery_man_name,territory_id=territory_id,territory_name=territory_name,mso_id=mso_id,mso_name=mso_name,customer_id=customer_id,customerName=customerName,market_id=market_id,market_name=market_name,page=page,items_per_page=items_per_page)    

def invoice_return_details_ret_download():
    c_id=session.cid
    ...
    #-------------
    myString='30.1B Invoice and Return Details\n'
    ...
    myString+='Inv. Date From:,'+str(startDt)+'\n'            
    myString+='Inv.To Date:'+','+str(endDt)+'\n'
    myString+='Ret. Date From:,'+str(date_from_ret)+'\n'            
    myString+='Ret.To Date:'+','+str(date_to_ret)+'\n'
    
    myString+='Document Details,,,,,,Invoice Details,,,,Return Details'+'\n'
    myString+='Inv.Date,Inv.Number,Cust.ID,Cust.Name,Retn.Date,Return Number,TP,VAT,Discount (Reg+SP),Net,TP,VAT,Discount (Reg+SP),Net'+'\n'
    ...
    for i in range(len(recordList)):    
        ...
        return_date=recData['return_date'] 
        retSl=recData['retSl'] 
        ...
        myString+=str(invoice_date)+','+str(invSl)+','+str(client_id)+','+str(client_name)+','+str(return_date)+','+str(retSl)+','+\
        str(invSaleAmt)+','+str(invVatAmt)+','+str(invDiscAmt)+','+str(invSaleVatNetAmt)+','+str(retSaleAmt)+','+str(retVatAmt)+','+str(retDiscAmt)+','+str(retSaleVatNetAmt)+'\n'
        
    myString+=str(invocieCount)+'Invoice(s),Total,,,,,'+str(round(invSaleTotal,2))+','+str(round(invVatTotal,2))+','+str(round(invDiscTotal,2))+','+str(round(invNetSaleVatTotal,2))+','+str(round(retSaleTotal,2))+','+str(round(retVatTotal,2))+','+str(round(retDiscTotal,2))+','+str(round(retNetSaleVatTotal,2))+'\n'
    
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=download_invoice_return_details_Return.csv'   
    return str(myString)
...
def invoice_return_itemwise():
    ...
    response.title='30.2A Invoice and Return Details Item wise'
...
def invoice_return_itemwise_download():
    ...
    #-------------
    myString='30.2A Invoice and Return Details Item wise\n'
...
def invoice_return_itemwise_ret():
    c_id=session.cid
    
    response.title='30.2B Invoice and Return Details Item wise'
    ...
    condStr=" (sm_return.cid = '"+c_id+"') AND (sm_return.depot_id='"+depot_id+"') AND (sm_return.store_id='"+store_id+"') AND ((sm_return.invoice_date >= '"+str(startDt)+"') AND (sm_return.invoice_date <= '"+str(endDt)+"')) AND  (sm_return.status='Returned') AND (sm_return.quantity>0 OR sm_return.bonus_qty>0)"
    
    if not(date_from_ret=='' or date_to_ret==''):
        condStr+=" AND ((sm_return.return_date >= '"+str(date_from_ret)+"') AND (sm_return.return_date <= '"+str(date_to_ret)+"'))"   
    
    if customer_id!='':
        condStr+=" AND (sm_return.client_id='"+customer_id+"')"        
    if dman_id!='':
        condStr+=" AND (sm_return.d_man_id='"+dman_id+"')"        
    if territory_id!='':
        condStr+=" AND (sm_return.area_id='"+territory_id+"')"      
    if market_id!='':
        condStr+=" AND (sm_return.market_id='"+market_id+"')"      
    if mso_id!='':
        condStr+=" AND (sm_return.rep_id='"+mso_id+"')"        
        
    if startDt!='' and endDt!='': 
        dateRecords="SELECT sm_return.item_id as item_id,MAX(sm_return.item_name) as item_name,MAX(sm_return.item_unit) as item_unit,sm_return.actual_tp as actual_tp, SUM(sm_return.quantity) as return_qty, SUM(sm_return.bonus_qty) as return_bonus_qty FROM sm_return WHERE ("+str(condStr)+") GROUP BY sm_return.item_id,sm_return.actual_tp ORDER BY sm_return.item_name asc,sm_return.actual_tp desc"
    else:
        dateRecords=''
        response.flash='Required Date From and Date To'
    ...
    return dict(recordList=recordList,date_from=startDt,date_to=endDt,date_from_ret=date_from_ret,date_to_ret=date_to_ret,depot_id=depot_id,depotName=depot_name,store_id=store_id,storeName=store_name,dman_id=dman_id,delivery_man_name=delivery_man_name,territory_id=territory_id,territory_name=territory_name,mso_id=mso_id,mso_name=mso_name,customer_id=customer_id,customerName=customerName,market_id=market_id,market_name=market_name,page=page,items_per_page=items_per_page)    
    
def invoice_return_itemwise_ret_download():
    c_id=session.cid
    ...
    #-------------
    myString='30.2B Invoice and Return Details Item wise\n'
    ...
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=download_invoice_return_itemWise_Return.csv'   
    return str(myString)
