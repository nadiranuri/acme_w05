...
def analysis():
    ...
    btn_product_performance = request.vars.btn_product_performance
    btn_product_performance_d = request.vars.btn_product_performance_d

    btn_outlet_coverage = request.vars.btn_outlet_coverage
    btn_outlet_coverage_d = request.vars.btn_outlet_coverage_d
    ...
    if (btn_new_client_summary_d or btn_new_client_summary or btn_new_client_d or btn_new_client or btn_productivity or btn_coverage or btn_daily_sales_item_wise or btn_customer_sales or btn_customer_sales_d or btn_customer_order or btn_customer_order_d or btn_product_performance or btn_product_performance_d or btn_outlet_coverage or btn_outlet_coverage_d or btn_zero_sales_outlet or btn_missed_outlet or btn_inactive_user or btn_sr_attendance or btn_sr_wise_sales or btn_sr_wise_sales_d or btn_sr_wise_visit or btn_sr_wise_visit_d or btn_dashboard or btn_market_research_level0_pres or btn_market_research_level1_pres or btn_market_research_rep_pres or btn_pres_survey_visit or btn_visit_list or btn_visit_count or btn_market_research_level0 or btn_market_research_level1 or btn_market_research_rep or btn_doctor_visit_summary or btn_schedule_and_completed_visit or btn_client_updated or visit_list_summary or btn_order_details_itemwise or btn_order_details_repwise or btn_order_details_depotwise or btn_sales_summary_itemwise or btn_sales_summary_repwise or btn_sales_summary_depotwise or btn_sales_details_download or btn_doctor_visit or btn_pres_survey_summary or btn_pres_survey_visit or btn_doctor_visit_individual):
        ...
        if dateFlag==False:
            ...
            if dateDiff>90:
                ...
                elif btn_product_performance:
                    redirect(URL('product_performance',vars=dict(fromDate=from_dt, toDate=to_dt, regionValue=regionValue, areaValue=areaValue,territoryValue=territoryValue, marketValue=marketValue, repCM=repCM)))
                elif btn_product_performance_d:
                    redirect(URL('product_performance_d',vars=dict(fromDate=from_dt, toDate=to_dt, regionValue=regionValue, areaValue=areaValue,territoryValue=territoryValue, marketValue=marketValue, repCM=repCM)))
                
                elif btn_outlet_coverage:
                    redirect(URL('outlet_coverage',vars=dict(fromDate=from_dt, toDate=to_dt, regionValue=regionValue, areaValue=areaValue,territoryValue=territoryValue, marketValue=marketValue, repCM=repCM)))
                elif btn_outlet_coverage_d:
                    redirect(URL('outlet_coverage_d',vars=dict(fromDate=from_dt, toDate=to_dt, regionValue=regionValue, areaValue=areaValue,territoryValue=territoryValue, marketValue=marketValue, repCM=repCM)))
    ...
    elif btn_target_vs_ach or btn_target_vs_ach_route_item:
        ...
        if rpt_year == '' or rpt_month == '':
            ...
            if btn_target_vs_ach:
                ...
                redirect(URL('target_vs_ach_route_item', vars=dict(rpt_year=rpt_year, rpt_month=rpt_month)))
...
def product_performance():
    c_id = session.cid
    response.title = 'Product Performance'
    ...
    records = "SELECT item_id,item_name,sum(quantity) as orderQty,sum(quantity * price) as orderAmount,count(distinct(sl)) as orderCount  FROM sm_order WHERE cid = '" + c_id + "' AND order_date >= '" + str(startDt) + "' AND order_date <= '" + str(endDt) + "'"+condition+" GROUP BY item_id;"
    records = db.executesql(records, as_dict=True)

    recordsSum = "SELECT sum(quantity) as orderQty,sum(quantity * price) as orderAmount,count(distinct(sl)) as orderCount  FROM sm_order WHERE cid = '" + c_id + "' AND order_date >= '" + str(startDt) + "' AND order_date <= '" + str(endDt) + "'"+condition+" ;"
    # return recordsSum
    recordsSum = db.executesql(recordsSum, as_dict=True)
...
def outlet_coverage():
    c_id = session.cid
    response.title = 'Outlet Coverage'
    ...
    repRrecords = "SELECT a.level0 as level0,a.level0_name as level0_name,a.level1 as level1,a.level1_name as level1_name,a.level2 as level2,a.level2_name as level2_name,a.level3 as level3,a.level3_name as level3_name,GROUP_CONCAT(level3) as level3_list FROM sm_level a WHERE a.level3!='' and a.cid = '" + c_id + "' AND  a.depth=3  " + condition + " GROUP BY a.level3;"
    # return repRrecords
    repRrecords = db.executesql(repRrecords, as_dict=True)
    
    qsetCount=db()
    qsetCount = qsetCount(db.sm_order_head.cid == c_id)
    qsetCount = qsetCount(db.sm_order_head.area_id != '')
    qsetCount = qsetCount(db.sm_order_head.order_date >= startDt)
    qsetCount = qsetCount(db.sm_order_head.order_date <= endDt)

    
    recordsCount = qsetCount.select(db.sm_order_head.client_id.count(),db.sm_order_head.area_id, orderby=db.sm_order_head.sl, groupby=db.sm_order_head.area_id)
    vChecklist=[]
    vCountList=[]
    for recordsCount in recordsCount:
        vCount=recordsCount[db.sm_order_head.client_id.count()]
        vCheck=str(recordsCount[db.sm_order_head.area_id])
        vChecklist.append(vCheck)
        vCountList.append(vCount)

    qsetOCount=db()
    qsetOCount = qsetOCount(db.sm_client.cid == c_id)
    qsetOCount = qsetOCount(db.sm_client.area_id != '')

    recordsOCount = qsetOCount.select(db.sm_client.area_id.count(),db.sm_client.area_id,  groupby=db.sm_client.area_id)
    OChecklist=[]
    OCountList=[]
    for recordsOCount in recordsOCount:
        OCount=recordsOCount[db.sm_client.area_id.count()]
        OCheck=str(recordsOCount[db.sm_client.area_id])
        OChecklist.append(OCheck)
        OCountList.append(OCount)


    return dict(records=repRrecords,fromDate=fromDate, toDate=toDate, regionValue=regionValue,regionValueShow=regionValueShow, areaValue=areaValue, territoryValue=territoryValue,areaValueShow=areaValueShow, territoryValueShow=territoryValueShow, marketValue=marketValue,marketValueShow=marketValueShow, repCM=repCM, page=page, items_per_page=items_per_page,vChecklist=vChecklist,vCountList=vCountList,OChecklist=OChecklist,OCountList=OCountList) 
