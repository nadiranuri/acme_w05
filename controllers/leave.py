    ...
    if btn_upload=='Upload':        
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)==11:
                ...
                error_data=row_data+'(11 columns need in a row)\n'
                error_str=error_str+error_data
                count_error+=1
                continue
...
def leave():
    ...
    response.title='Leave'
    
    #   ---------------------
    cid=session.cid 

    leave_type=str(request.vars.leave_type).strip()
    total_leave=str(request.vars.total_leave).strip()
    

    form =SQLFORM(db.sm_leave_type,
                  fields=['leave_type','total_leave'],
                  submit_button='Save'
                  )
   
    leave_type_l=request.vars.leave_type
    total_leave_t=request.vars.total_leave
    
     
    if ((leave_type_l!='') and (total_leave_t!=''))  :
       if form.accepts(request.vars,session,onvalidation=unit_validation):
          response.flash = 'Submitted Successfully'
    else:
...
#       redirect (URL('item','unit'))
    ...
    if btn_delete:
        record_id=request.args[1]
        db((db.sm_leave_type.id == record_id)).delete()
        # return record_id
        response.flash = 'Deleted Successfully'
    ...
    qset=qset(db.sm_leave_type.cid==cid)

    
    records=qset.select(db.sm_leave_type.ALL,orderby=~db.sm_leave_type.id,limitby=limitby)

    totalCount=qset.count()
    #return records
    return dict(form=form,records=records,page=page,totalCount=totalCount,items_per_page=items_per_page,access_permission=access_permission)
