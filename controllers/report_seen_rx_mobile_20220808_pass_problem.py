...
# http://127.0.0.1:8000/acme/report_seen_rx_mobile_test/index?cid=acme&rep_id=20496&password=1234&device_id=1234

def index():
    cid = request.vars.cid
    rep_id = request.vars.rep_id
    rep_pass = request.vars.password
    # return rep_pass
...
def home():
    ...
    if session.user_type=='rep':
        qset = qset(db.sm_prescription_seen_head.submit_by_id == session.rep_id)
...
def report_seen_rx_area_wise_url():
    ...
    redirect(URL(c='report_seen_rx_mobile_test',f='report_seen_rx_area_wise'))#,vars=dict(invoice_count=invoice_count)))
...
def report_seen_rx_area_wise():
    ...
    repRow = db((db.sm_rep.cid == cid) & (db.sm_rep.rep_id == rep_id) & (db.sm_rep.password == rep_pass) & (db.sm_rep.sync_code == sync_code) & (db.sm_rep.status == 'ACTIVE')).select(db.sm_rep.ALL, limitby=(0, 1))
    # return rep_id
    if not repRow:
...
def report_seen_rx_slWise_url():
    ...
    redirect(URL(c='report_seen_rx_mobile_test',f='report_seen_rx_slWise'))
