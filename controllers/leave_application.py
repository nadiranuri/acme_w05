...
def leave_application_submit():           
    ...
    available_leaveVal=request.vars.available_leaveVal
    # return available_leaveVal
    ...
    if (btn_leave_application) :
            if (leaveDayVal == 'Invalid Date Range'):
                session.flash='Invalid Date Range'
            else:               
                if (str(available_leaveVal)!='Not Available'):                    
                    if ((str(l_fromDate)!='') & (str(l_toDate)!='') & (str(leave_type)!='') & (str(leave_cause)!='') ):
                        try:                                                                                                
                            checkSettings=  db((db.sm_settings.cid == cid)  & (db.sm_settings.s_value == rep_id) &  (db.sm_settings.s_key == 'd_sales_director')  ).select(db.sm_settings.s_value,limitby=(0, 1))
                            if checkSettings:                            
                                if (int(available_leaveVal) > int(leaveDayVal)):     
                                    db.sm_leave_application.insert(cid=cid,from_date=l_fromDate,to_date=l_toDate,emp_id=rep_id,emp_name=emp_name,total_leave=leaveDayVal,leave_type=leave_type,note=leave_cause,field1=2)  
                                    session.flash='Submitted Successfully.' 
                                else:
                                    session.flash='Leave not available for this type'                    
                            else:  
                                checkLevel=  db((db.sm_supervisor_level.cid == cid) & (db.sm_supervisor_level.sup_id == repID)  & (db.sm_supervisor_level.level_depth_no == 0) ).select(db.sm_supervisor_level.sup_id,db.sm_supervisor_level.sup_name,limitby=(0, 1))
                                if  checkLevel:   
                                    if (int(available_leaveVal) > int(leaveDayVal)):   
                                        db.sm_leave_application.insert(cid=cid,from_date=l_fromDate,to_date=l_toDate,emp_id=rep_id,emp_name=emp_name,total_leave=leaveDayVal,leave_type=leave_type,note=leave_cause,field1=1)  
                                        session.flash='Submitted Successfully.' 
                                    else:
                                        session.flash='Leave not available for this type'
                                else:    
                        ...
                        session.flash='Please select Cause, Date & Leave type'
                else:
                     session.flash='Leave not available' 
                # else:
                    # session.flash='Leave not available for this type'  
    else:
...
def get_available_leave():
    ...
    checkLeave=  db ((db.sm_leave_application.cid == c_id) & (db.sm_leave_application.leave_type == leave_type)  & (db.sm_leave_application.emp_id == rep_id) & (db.sm_leave_application.director_marktng_status == 'Approved') ).select((db.sm_leave_application.total_leave).sum(), groupby=db.sm_leave_application.emp_id)        
    if checkLeave:
        for record in checkLeave:
            checkLeave=int(record[(db.sm_leave_application.total_leave).sum()])        
        
        if int(totalLeave)==0:
            available_leave='Not Available'

        else:
            try:
                available_leave=(int(totalLeave)-int(checkLeave))
                if int(available_leave)==0:
                    available_leave='Not Available'
                else:
                    pass
            except:
        ...
        available_leave=int(totalLeave)
...
def leave_application_approval():               
    ...
    for recordRow in recordRows:
        ...
        leaveType = recordRow.leave_type
    ...
    return dict(leave_type=leave_type,depthNo=depthNo,cid=cid,uid=uid,u_name=u_name,ared_head_id=ared_head_id,ared_head_name=ared_head_name,u_pass=u_pass,rep_id=rep_id,rep_name=rep_name,emp_id=emp_id,emp_name=emp_name,from_date=from_date,to_date=to_date,total_leave=total_leave,leaveType=leaveType,record_id=record_id,leave_req_reason=leave_req_reason)
...
def leave_application_confirm():              
    ...
    # leaveType=str(request.vars.leaveType)

    leaveType=str(request.vars.leave_type_name)
    # return leaveType

    checkLtype=  db((db.sm_leave_type.cid == cid) & (db.sm_leave_type.leave_type == leaveType)).select(db.sm_leave_type.ALL,limitby=(0, 1))    
    if checkLtype:
        totalLeave=checkLtype[0].total_leave
    
    checkLeave=  db ((db.sm_leave_application.cid == cid) & (db.sm_leave_application.leave_type == leaveType)  & (db.sm_leave_application.emp_id == memId_1) & (db.sm_leave_application.director_marktng_status == 'Approved') ).select((db.sm_leave_application.total_leave).sum(), groupby=db.sm_leave_application.emp_id)        
    if checkLeave:
        for record in checkLeave:
            checkLeave=int(record[(db.sm_leave_application.total_leave).sum()])        
    ...
    if checkRep: 
        ...
        if btn_approve_application:                                    
            ...
            if depth== '2'  :
                try:                                      
                    if (str(available_leaveVal)!='Not Available'):                          
                        if (int(available_leaveVal) > int(total_leave)):                            
                            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1) & (db.sm_leave_application.id == memId_1_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,ared_head_id=uid,ared_head_name=u_name,area_head_status='Approved')                             
                ...
                try:
                    if (int(available_leave) > int(total_leave)):                        
                        statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1) & (db.sm_leave_application.id == memId_1_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,ared_head_id=uid,ared_head_name=u_name,area_head_status='Approved')                             
                        session.flash='Approved Successfully.'
                    else:
                        session.flash='Leave not available'
            ...
            if depth== '1'  :                    
                try:                  
                    if (str(available_leaveVal)!='Not Available'):  
                        if (int(available_leaveVal) > int(total_leave)):
                            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1) & (db.sm_leave_application.id == memId_1_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,zonal_head_id=uid,zonal_head_name=u_name,zonal_head_status='Approved')            
                            session.flash='Approved Successfully.' 
                        else:
                            session.flash='Leave not available for this type'   
                    else:
            ...
            if depth== '0' :                
                try:                  
                    if (str(available_leaveVal)!='Not Available'):  
                        if (int(available_leaveVal) > int(total_leave)):
                            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1) & (db.sm_leave_application.id == memId_1_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,divisional_head_id=uid,divisional_head_name=u_name,divisional_head_status='Approved')            
                            session.flash='Approved Successfully.'
...
def leave_application_approval_p():               
    ...
    return dict(leaveType=leaveType,zonal_head_id=zonal_head_id,zonal_head_name=zonal_head_name,depthNo=depthNo,cid=cid,uid=uid,u_name=u_name,ared_head_id=ared_head_id,ared_head_name=ared_head_name,u_pass=u_pass,rep_id=rep_id,rep_name=rep_name,emp_id=emp_id,emp_name=emp_name,from_date=from_date,to_date=to_date,total_leave=total_leave,leave_type=leave_type,record_id=record_id,leave_req_reason=leave_req_reason)
...
def leave_application_confirm_p():              
    ...
    checkLeave=  db ((db.sm_leave_application.cid == cid) & (db.sm_leave_application.leave_type == leaveType)  & (db.sm_leave_application.emp_id == memId_r) & (db.sm_leave_application.director_marktng_status == 'Approved') ).select((db.sm_leave_application.total_leave).sum(), groupby=db.sm_leave_application.emp_id)        
    if checkLeave:
        for record in checkLeave:
            checkLeave=int(record[(db.sm_leave_application.total_leave).sum()])        
        ...
        if btn_approve_application:            
            ...
            if depth== '2'  :                  
                try:                  
                    if (str(available_leaveVal)!='Not Available'):  
                        if (int(available_leaveVal) > int(total_leave)):
                            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_r) & (db.sm_leave_application.id == memId_r_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,ared_head_id=uid,ared_head_name=u_name,area_head_status='Approved')                             
                            session.flash='Approved Successfully.'
                        else:
                            session.flash='Leave not available for this type'   
                    else:
            ...
            if depth== '1'  :                     
                try:                  
                    if (str(available_leaveVal)!='Not Available'):  
                        if (int(available_leaveVal) > int(total_leave)):
                            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_r) & (db.sm_leave_application.id == memId_r_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,zonal_head_id=uid,zonal_head_name=u_name,zonal_head_status='Approved')            
                            session.flash='Approved Successfully.'
                        else:
                            session.flash='Leave not available for this type'   
                    else:
            ...
            if depth== '0' :                
                try:                  
                    if (str(available_leaveVal)!='Not Available'):  
                        if (int(available_leaveVal) > int(total_leave)):
                            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_r) & (db.sm_leave_application.id == memId_r_id)).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,divisional_head_id=uid,divisional_head_name=u_name,divisional_head_status='Approved')            
                            session.flash='Approved Successfully.'
...
# http://127.0.0.1:8000/skf/leave_application/approveListDDM?cid=SKF&rep_id=1102&rep_pass=123
...
def approveListDDM():                  
    ...
    # session.d_director_id = d_director_id 
    d_pass=str(request.vars.rep_pass).strip().upper()
    # session.d_pass = d_pass
    ...
    if checkSettings:
        ...
        if checkRep: 
            ...
            app_req = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.field1 == '1')  & (db.sm_leave_application.ddm_status == '')  ).select(db.sm_leave_application.cid,db.sm_leave_application.emp_id, db.sm_leave_application.emp_name, db.sm_leave_application.from_date,db.sm_leave_application.to_date,db.sm_leave_application.total_leave, db.sm_leave_application.leave_type, db.sm_leave_application.id, orderby=db.sm_leave_application.emp_id|db.sm_leave_application.created_on) 
            if not app_req:
                session.msg_1 = 'Nothing pending for approval.'
            
            leave_req = db((db.sm_leave_application.cid == cid) & (db.sm_leave_application.ddm_status == '') & (db.sm_leave_application.divisional_head_status == 'Approved')).select(db.sm_leave_application.cid,db.sm_leave_application.emp_id, db.sm_leave_application.emp_name, db.sm_leave_application.from_date,db.sm_leave_application.to_date,db.sm_leave_application.total_leave, db.sm_leave_application.leave_type, db.sm_leave_application.id, orderby=db.sm_leave_application.emp_id|db.sm_leave_application.created_on)
            if not leave_req:
                session.msg_1 = 'Nothing pending for approval.'
        ...
        session.flash='Access is Denied !'        
    # return  app_req   
    return dict(app_req=app_req,leave_req=leave_req,director_id=director_id,director_pass=director_pass)
...
def leave_application_ddmConfirm():              
    ...
    if checkRep:  
        ...
        if btn_approve_application:                        
            ...
            try:                  
                if (str(available_leaveVal)!='Not Available'):  
                    if (int(available_leaveVal) > int(total_leave)):
                        # return available_leaveVal
                        statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1) & (db.sm_leave_application.id == memId_1_id) & (db.sm_leave_application.ddm_status == '')).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,ddm_id=director_id,ddm_name=u_name,ddm_status='Approved')            
                        session.flash='Approved Successfully.'
        ...
        if btn_reject_application: 
            ...
            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1)& (db.sm_leave_application.id == memId_1_id) ).update(ddm_id=director_id,ddm_name=u_name,ddm_status='Rejected')              
            session.flash='Rejected Successfully.'
...
def leave_application_approval_ddmP():               
    
    cid = str(request.vars.cid).strip().upper()
    uid= str(request.vars.memId_r)
    u_name= str(request.vars.memName_r)
    ...
    recordRows = db((db.sm_leave_application.cid == cid)  &  (db.sm_leave_application.emp_id==uid)  &  (db.sm_leave_application.id==memId_r_id)).select(db.sm_leave_application.ALL, orderby=db.sm_leave_application.emp_id)
    # return cid
    for recordRow in recordRows:
        record_id = recordRow.id        
        emp_id = recordRow.emp_id
...
def leave_application_ddmConfirmP():              

    cid = str(request.vars.cid).strip().upper()    
    director_id=str(request.vars.director_id).strip().upper()
    director_pass=str(request.vars.director_pass).strip().upper()
    
    memId_r=str(request.vars.emp_id).strip().upper()
    memName_r=str(request.vars.emp_name).strip().upper()
    memId_r_id=str(request.vars.record_id).strip()
    ...
    if checkRep: 
            ...
            try:                  
                if (str(available_leaveVal)!='Not Available'):  
                    if (int(available_leaveVal) > int(total_leave)):
                        ...
                        statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_r) & (db.sm_leave_application.id == memId_r_id) & (db.sm_leave_application.director_marktng_status == '')).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,ddm_id=director_id,ddm_name=u_name,ddm_status='Approved')            
                        session.flash='Approved Successfully.' 
                    else:
                        session.flash='Leave not available for this type'   
                else:
        ...
        if btn_reject_application: 
            ...
            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_r)& (db.sm_leave_application.id == memId_r_id) ).update(ddm_id=director_id,ddm_name=u_name,ddm_status='Rejected')              
            session.flash='Rejected Successfully.'
            
    else:
        session.flash='Invalid Authorization.'

    redirect(URL(c='leave_application',f='leave_application_approval_ddmP',vars=dict(cid=cid,director_id=director_id,director_pass=director_pass,memId_r=memId_r,memId_r_id=memId_r_id,memName_r=memName_r)))
...
def director_approveList():                      
    ...
    if checkSettings:
        ...
        if checkRep: 
            ...
            app_req = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.field1 == '2')  & (db.sm_leave_application.director_marktng_status == '')  ).select(db.sm_leave_application.cid,db.sm_leave_application.emp_id, db.sm_leave_application.emp_name, db.sm_leave_application.from_date,db.sm_leave_application.to_date,db.sm_leave_application.total_leave, db.sm_leave_application.leave_type, db.sm_leave_application.id, orderby=db.sm_leave_application.emp_id|db.sm_leave_application.created_on) 
            # return app_req
            if not app_req:
                session.msg_1 = 'Nothing pending for approval.'

            leave_req = db((db.sm_leave_application.cid == cid) & (db.sm_leave_application.ddm_status == 'Approved') & (db.sm_leave_application.director_marktng_status == '') ).select(db.sm_leave_application.cid,db.sm_leave_application.emp_id, db.sm_leave_application.emp_name, db.sm_leave_application.from_date,db.sm_leave_application.to_date,db.sm_leave_application.total_leave, db.sm_leave_application.leave_type, db.sm_leave_application.id, orderby=db.sm_leave_application.emp_id|db.sm_leave_application.created_on)                                   
            # return leave_req
            if not leave_req:
                session.msg_1 = 'Nothing pending for approval.' 
    else :
    ...
    if checkRep: 
        ...
        if btn_approve_application:            
            ...
            try:                  
                if (str(available_leaveVal)!='Not Available'):  
                    if (int(available_leaveVal) > int(total_leave)):
                        statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1) & (db.sm_leave_application.id == memId_1_id) & (db.sm_leave_application.director_marktng_status == '')).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,director_id=director_id,director_name=u_name,director_marktng_status='Approved')            
                        session.flash='Approved Successfully.' 
                    else:
                        session.flash='Leave not available for this type'   
                else:
        ...
        if btn_reject_application:            
            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_1)& (db.sm_leave_application.id == memId_1_id) ).update(director_id=director_id,director_name=u_name,director_marktng_status='Rejected')              
            session.flash='Rejected Successfully.'
...
def leave_application_approval_dp():               
...
def leave_application_dConfirmP():              
    # return '8'
    cid = str(request.vars.cid).strip().upper()    
    director_id=str(request.vars.director_id).strip().upper()
    director_pass=str(request.vars.director_pass).strip().upper()
    ...
    if checkRep: 
        ...
        if btn_approve_application:            
            ...
            try:                  
                if (str(available_leaveVal)!='Not Available'):  
                    if (int(available_leaveVal) > int(total_leave)):
                        statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_r) & (db.sm_leave_application.id == memId_r_id) & (db.sm_leave_application.director_marktng_status == '')).update(from_date=from_dt_2,to_date=to_dt_2,total_leave=total_leave,leave_type=leaveType,director_id=director_id,director_name=u_name,director_marktng_status='Approved')            
                        session.flash='Approved Successfully.' 
        ...
        if btn_reject_application:            
            statusConfirm = db((db.sm_leave_application.cid == cid)  & (db.sm_leave_application.emp_id == memId_r)& (db.sm_leave_application.id == memId_r_id) ).update(director_id=director_id,director_name=u_name,director_marktng_status='Rejected')              
            session.flash='Rejected Successfully.'
            
    else:
        session.flash='Invalid Authorization.'  

    redirect(URL(c='leave_application',f='leave_application_approval_dp',vars=dict(cid=cid,director_id=director_id,director_pass=director_pass,memId_r=memId_r,memId_r_id=memId_r_id,memName_1=memName_r)))
...
def approve_reject_notice():               
    ...
    recordRows = db((db.sm_leave_application.cid == cid)  &  (db.sm_leave_application.emp_id==rep_id )).select(db.sm_leave_application.ALL,orderby=~db.sm_leave_application.id,limitby=(0, 20))
