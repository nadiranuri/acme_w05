...
def download_tracking():
    ...
    # Nazma Azam 2020-01-22 start
    myString='Tracking List\n\n'


    if ((session.search_type_rep != '') and (session.search_value_rep != '')):
        # return session.search_value_rep
        myString +=  str(session.search_type_rep)+ ': ,' + str(session.search_value_rep)+ '\n\n'
        # return myString
    if ((session.from_dt != '') and (session.to_dt != '')):
        myString +=  'From   ,' + str(session.from_dt)+', To,'+ str(session.to_dt)  + '\n\n\n'

    # return myString
    # Nazma Azam 2020-01-22 end
    ...
    for rec in records:
        ...
        link_location = 'https://maps.google.com/?q=' + str(rec.visited_latlong).replace(',','+')
#         location_g='https://maps.google.com/?q='+str(rec.latLong).replace(',','+')
#         a_link=""" <a href=""" + link_location + """ target="_blank"></a>"""
        # return a_link
        myString+=str(visit_date)+','+str(visit_time)+','+str(rep_id)+','+str(rep_name)+','+str(call_type)+','+str(visited_id)+','+str(visited_name)+','+str(link_location)+'\n'
        # Nazma Azam 2020-01-22 end
