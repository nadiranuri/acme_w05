...
def home():
    ...
    #--------------- Billal
    #     Billal
    if (btn_inv_ret_details or btn_inv_ret_details_D or btn_inv_ret_itemwise or btn_inv_ret_itemwise_D or btn_inv_ret_checking or btn_inv_ret_checking_D or btn_discount_typewise_sale or btn_discount_typewise_sale_D or btn_discount_typewise_sale_ad or btn_discount_typewise_sale_with_value or btn_discount_typewise_sale_with_tp or btn_accounting_period_invoice_wise or btn_customer_type_wise_sales or btn_return_note_preview):
        ...
        if dateFlag==False:
            ...
            if dateDiff>92:
                ...
                if (depot=='' or store==''):
                    ...
                    elif btn_inv_ret_details_D:
                        response.flash='inv_ret_details_D'
                    ...
                    elif btn_inv_ret_itemwise_D:
                        response.flash='inv_ret_itemwise_D'
                    ...
                    elif btn_inv_ret_checking_D:
                        response.flash='inv_ret_checking_D'
                    ...
                    elif btn_discount_typewise_sale_D:
                        response.flash='discount_typewise_sale_D'
...
def discount_type_wise_sales_with_tp():
    ...
    if discount_type=='REGULAR':        
        dateRecords="SELECT sm_invoice_head.depot_id as depot_id,sm_invoice_head.invoice_date as invoice_date,sm_invoice_head.sl as invSl,sm_invoice_head.client_id as client_id,sm_invoice_head.client_name as client_name,sm_invoice_head.area_id as area_id,sm_invoice_head.market_name as market_name,sm_invoice_head.discount_precent as discount_precent,(sm_invoice_head.regular_disc_tp-sm_invoice_head.ret_regular_disc_tp) as tp_amt,(sm_invoice_head.discount-sm_invoice_head.return_discount) as disc_value FROM sm_invoice_head WHERE ("+str(condStr)+") ORDER BY sm_invoice_head.sl"
...
def discount_type_wise_sales_with_value_2():
    c_id=session.cid
    ...
    condStr=" (sm_invoice.cid = '"+c_id+"') AND (sm_invoice.depot_id='"+depot_id+"') AND (sm_invoice.store_id='"+store_id+"') AND ((sm_invoice.invoice_date >= '"+str(startDt)+"') AND (sm_invoice.invoice_date <= '"+str(endDt)+"')) AND  (sm_invoice.status='Invoiced')"
    ...
    if startDt!='' and endDt!='': 
        dateRecords="SELECT sm_invoice.depot_id as depot_id,sm_invoice.sl as invSl,sm_invoice.invoice_date as invoice_date,sm_invoice.client_id as client_id,sm_invoice.client_name as client_name,sm_invoice.area_id as area_id,sm_invoice.market_name as market_name,sm_invoice.discount_type as discount_type,SUM(sm_invoice.actual_tp*sm_invoice.quantity) as actualTpAmt,SUM(sm_invoice.actual_vat*sm_invoice.quantity) as vatTotalAmt,SUM(sm_invoice.item_discount) as item_discount,SUM(sm_invoice.actual_tp*sm_invoice.return_qty) as retTpAmt,SUM(sm_invoice.actual_vat*sm_invoice.return_qty) as retVatAmt,sm_invoice.return_discount as retDiscAmt,sm_invoice.return_sp_discount as retSpDiscAmt,sm_invoice_head.discount_precent as discount_precent FROM sm_invoice_head WHERE ("+str(condStr)+") ORDER BY sm_invoice_head.sl"
    else:
        dateRecords=''
        response.flash='Required Date From and Date To'
