...
def sales_report_fm_url(): 
    ...
    # to_date = str(request.vars.to_date).strip().upper()
    # return to_date
    session.cid = cid 
    session.rep_id = rep_id 
    session.user_type = user_type 
...
def sales_report_detail(): 
    ...
    if user_type == 'SUP':  
        ...
        levelRows = db((db.sm_rep_area.cid == cid)  & (db.sm_rep_area.rep_id == rep_id)).select(db.sm_rep_area.area_id, db.sm_rep_area.area_name, groupby=db.sm_rep_area.area_id ,orderby=db.sm_rep_area.area_name)
        # return levelRows
