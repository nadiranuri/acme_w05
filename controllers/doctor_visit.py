...
#         return from_date +'        '+to_date
        ...
        if search_month!='': 
            ...
            records = qset.select(db.sm_prescription_head.ALL,db.sm_prescription_details.ALL, orderby=db.sm_prescription_head.sl)
    #         return records
#             return db._lastsql
            myString='Prescription,'+str(from_date)+','+str(to_date)+' \n\n'
            myString+='Date  ,SL,  regID,  regName, areaID, areaName ,teritoryID, teritoryName , DoctorID, DoctorName,  SubmitByID, SubmitByName, medicineID, medicineName, medType,\n'
            for record in records: 
                myString+=str(record.sm_prescription_head.submit_date)+','+str(record.sm_prescription_head.sl)+','+str(record.sm_prescription_head.reg_id)+','+str(record.sm_prescription_head.reg_name)+','+str(record.sm_prescription_head.tl_id)+','+str(record.sm_prescription_head.tl_name)+','+str(record.sm_prescription_head.area_id)+','+str(record.sm_prescription_head.area_name)+','+str(record.sm_prescription_head.doctor_id)+','+str(record.sm_prescription_head.doctor_name)+','+str(record.sm_prescription_head.submit_by_id)+','+str(record.sm_prescription_head.submit_by_name)+','+str(record.sm_prescription_details.medicine_id)+','+str(record.sm_prescription_details.medicine_name)+','+str(record.sm_prescription_details.med_type)+'\n'
                #-----------
#             return myString
            import gluon.contenttype
            response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
...
def doctor_visit_listshow():
    c_id=session.cid
    
    response.title='Doctor Visit List'
    
    fromDate=request.vars.vsitdt
    
    to_dt2=datetime.datetime.strptime(str(fromDate),'%Y-%m-%d')   
    toDate=to_dt2 + datetime.timedelta(days = 1)   
#     return toDate
    ...
    repCMName=str(request.vars.repCMName).strip()
#     return repCM
    if areaValue=='None':
        areaValue=''
...
#             qset=qset(db.sm_level.level0==regionValue)
        if areaValue!='':
            qset=qset(db.sm_doctor_visit.route_id==areaValue)
...
#     return records
    return dict(records=records,fromDate=fromDate,toDate=toDate,areaValue=areaValue,areaValueShow=areaValueShow,repCM=repCM,repCMName=repCMName,page=page,items_per_page=items_per_page)
