...
def rep_validation(form):
    ...
    if ((rep_id!='') and (session.cid!='')and (mobile_no!='')):
        ...
        if compMobRecords:
            ...
            if not mobRows:
                ...
                if rows_check:
                    ...
                    if (status=='INACTIVE'):
                        form.vars.mobile_no=''
                    elif ((status=='ACTIVE') and (mobile_no!='')):
    ...
    elif ((rep_id!='') and (session.cid!='')and (mobile_no=='') and (status=='INACTIVE')):
        rows_check=db((db.sm_rep.cid==c_id) & (db.sm_rep.rep_id==rep_id)).select(db.sm_rep.rep_id,limitby=(0,1))
        if rows_check:
            form.errors.rep_id=''
            response.flash = 'please choose a new '
...
def rep_edit_validation(form):
    ...
    if (status=='INACTIVE'):
        ...
        if ((current_rep!='') and (session.cid!='')and (mobile_no!='')):
            compMobRecords=db((db.sm_comp_mobile.cid==c_id) & (db.sm_comp_mobile.mobile_no == mobile_no)).select(db.sm_comp_mobile.mobile_no,limitby=(0,1))
            #Check mobile number already used or not
            if compMobRecords:
    ...
    if rows_check:
        ...
        if status=='ACTIVE':
            ...
            if not compMobRecords:
                ...
                #Check mobile no used or not
                mobRows=db((db.sm_rep.cid==c_id) & (db.sm_rep.mobile_no==mobile_no)).select(db.sm_rep.mobile_no,limitby=(0,1))                    
                if mobRows:
                    form.errors.mobile_no='mobile no already exist,please choose a new'
                else:
                    form.vars.rep_id=rep_id
...
def supervisor_edit_validation(form):
    ...
    if (status=='INACTIVE'):
        ...
        if ((current_sup!='') and (c_id!='')and (mobile_no!='')):
            compMobRecords=db((db.sm_comp_mobile.cid==c_id) & (db.sm_comp_mobile.mobile_no == mobile_no)).select(db.sm_comp_mobile.mobile_no,limitby=(0,1))
            if compMobRecords:
