...
def sales_report_fm():
    ...
    if (user_type=='REP'):
        ...
        #     records_ov=db.executesql(sql_str,as_dict = True)
            ...
            sql_str="SELECT area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date = '"+ str(order_date) +"' "+ condition + " GROUP BY area_id,order_date order by order_date desc;"
            records_ov=db.executesql(sql_str,as_dict = True)
    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter: 
            if not(session.from_dt=='' or session.from_dt==None or session.to_date=='' or session.to_date==None):
                ...
                sql_str="SELECT  level3_id,  level3_name,area_id,  area_name,order_date FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND order_date >= '"+ str(session.from_dt) +"' AND order_date <= '"+ str(session.to_date) +"' AND level2_id = '"+ str(level2_id) +"' GROUP BY level3_id,order_date order by order_date desc;"
                # return sql_str
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass
            ...
            sql_str="SELECT level3_id,  level3_name,area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date  =  '"+ str(order_date) +"'  AND level2_id = '"+ str(level2_id) +"' GROUP BY level3_id,order_date order by order_date desc;"
            records_ov=db.executesql(sql_str,as_dict = True)
    ...
    if (user_type=='REP'):
        ...
        if session.btn_filter:
            ...
            if not(session.from_dt=='' or session.from_dt==None or session.to_date=='' or session.to_date==None):
                ...
                sql_str="SELECT  area_id,  area_name,order_date FROM sm_order_head  WHERE cid = '"+ str(cid) +"' AND order_date >= '"+ str(session.from_dt).split(' ')[0] +"' AND order_date <= '"+ str(session.to_date) +"' "+ condition + " GROUP BY area_id,order_date order by order_date desc;"
                # return sql_str
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass
            ...
            sql_str="SELECT area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date  =  '"+ str(session.from_dt)  +"' "+ condition + " GROUP BY area_id,order_date order by order_date desc;"
            # return sql_str
            records_ov=db.executesql(sql_str,as_dict = True)
        else:
            sql_str="SELECT area_id,  area_name, order_date  FROM sm_order_head WHERE cid = '"+ str(cid) +"'  AND order_date =  '"+ str(date_from) +"'  "+ condition + " GROUP BY area_id,order_date order by order_date desc;"
            # return sql_str
            records_ov=db.executesql(sql_str,as_dict = True)
    ...
    if (user_type=='SUP'):
        ...
        if session.btn_filter:
            ...
            if not(session.levelIdstr=='' or session.levelIdstr==None):
                ...
                sql_str="SELECT  area_id,  area_name,order_date FROM sm_order_head  WHERE cid = '"+ str(cid) +"'  AND  area_id='"+ str(levelIdstr) +"'  AND order_date >= '"+ str(session.from_dt).split(' ')[0] +"' AND order_date <= '"+ str(session.to_date) +"' GROUP BY area_id,order_date order by order_date desc;"
                return sql_str
                records_ov=db.executesql(sql_str,as_dict = True)
            else:
                pass
