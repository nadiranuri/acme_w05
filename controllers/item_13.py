...
def unit_validation(form):
    unit_type=str(request.vars.cat_type_id).strip().upper()
...
#             else:
        if price < 0 :
            ...
            if vat_amt < 0 :
                ...
                total_amt=float(price)+float(vat_amt) #TP+VAT
...
def item():
    ...
    form =SQLFORM(db.sm_item,
                  fields=['item_id','name','des','category_id','category_id_sp','unit_type','manufacturer','price','vat_amt'],       
                  submit_button='Save'
                  )
...
def item_edit():
    ...
    form =SQLFORM(db.sm_item,
                  ...
                  linkto=None,
                  upload=None,
                  fields=['name','des','category_id','category_id_sp','unit_type','manufacturer','price','vat_amt'],
    ...
    if btn_upload=='Upload':        
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)==9:
                ...
                name=str(coloum_list[1]).strip().upper()
                ...
                unit_type=str(coloum_list[5]).strip().upper()
                manufacturer=str(coloum_list[6]).strip()
                price=str(coloum_list[7]).strip()  #Distributor Price
                vat_amt=str(coloum_list[8]).strip()       #Retailer Price
                    ...
                    if item_id=='' or name=='' or category_id=='' or unit_type=='':
                        ...
                        if valid_category==True and valid_category_sp==True: # check unit type                                                         
                            for i in range(len(unit_list)):
                                ...
                                if (str(u_type).strip()==str(unit_type).strip().upper()):
                                    valid_unit=True
                                    break
                                        ...
                                        if duplicate_item==True:
                                            ...
                                            if price<0 or vat_amt<0:
                                                ...
                                                if (itemListStr.find(temp_item)== (-1)):
                                                    ...
                                                    ins_dict= {'cid':c_id,'item_id':item_id,'name':name,'des':des,'category_id':category_id,'category_id_sp':category_id_sp,'unit_type':unit_type,'manufacturer':manufacturer,'price':price,'vat_amt':vat_amt,'total_amt':total_amt}
                ...
                error_data=row_data+'(9 columns need in a row)\n'
                error_str=error_str+error_data
                count_error+=1
                continue
...
def download_item():
    ...
    myString='Item List\n'
    myString+='Item ID,Name,Description,Group/Category,Base Group/Category,Unit Type,Manufacturer,TP Amount,VAT Amount,Total Amount\n'
    ...
    for rec in records:
        ...
        if category_id_sp==None:
            category_id_sp=''
        myString+=str(item_id)+','+str(name)+','+str(des)+','+str(category_id)+','+str(category_id_sp)+','+str(unit_type)+','+str(manufacturer)+','+str(price)+','+str(vat_amt)+','+str(total_amt)+'\n'
...
def download_item_batch():
    ...
    myString+='Item ID,Name,Batch ID,Expiary Date\n'
    #Replace coma from records. cause coma means new Column    
    for rec in records:
        item_id=rec.item_id
        name=str(rec.name).replace(',', ' ')
            ...
            if len(coloum_list)!=3:
                ...
                if item_id=='' or batch_id=='' or expiaryDate=='':
                    ...
                    if valid_item==False:
                        ...
                        if dateFlag==False:
                            error_data=row_data+'(Invalid Expiary Date)\n'
                            error_str=error_str+error_data
                            count_error+=1
                            continue
                        else:                            
...
def process_change_item_manual():
    ...
    if not processRecords:
        ...
        itemTransfer="insert into sm_item(cid,item_id,name,des,category_id,category_id_sp,unit_type,manufacturer,price,dist_price,vat_amt,total_amt,field1,field2,note,created_on,created_by,updated_on,updated_by) (select cid,item_id,name,des,category_id,category_id_sp,unit_type,manufacturer,price,dist_price,vat_amt,total_amt,field1,field2,note,created_on,created_by,updated_on,updated_by from sm_item_temp where sm_item_temp.cid='"+cid+"')" 
